          ...
          {submissions.map((item: any) => {
            ...
            return (
              <article className={`reviews-submission-card ${statusClass(item.status)}`} key={item.id}>
                ...
                <div className="reviews-row-actions"><a href={`/admin/avaliacoes/${item.id}`}>Avaliar</a><form action={`/admin/avaliacoes/${item.id}/excluir`} method="post"><input type="hidden" name="return_to" value={withQuery(status)} /><button className="delete" type="submit">Excluir</button></form></div>
              </article>
            );
          })}
