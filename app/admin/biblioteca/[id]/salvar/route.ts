...
async function uploadModuleCover(file: File, moduleId: string) {
  ...
  if (!file.type.startsWith('image/')) return { ok: false, error: 'Arquivo de capa invalido.' };
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const storage_provider = String(formData.get('storage_provider') || 'drive') === 'r2' ? 'r2' : 'drive';
    const r2_prefix = String(formData.get('r2_prefix') || '').trim();

    if (!title) return redirectOrJson(request, id, { ok: false, error: 'Informe o titulo do modulo.' }, 400);
