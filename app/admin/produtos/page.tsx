...
import { dailyTrainingSteps, getDailyTrainingExercise, getExercisesByCategory, trainingCategories, trainingExercises } from '@/lib/training-center';
import { accessLabel, getCentralAccessRows, type CentralAccessLevel } from '@/lib/central-access';
...
const compactProductsCss = `.admin-products-compact .admin-clean-hero{min-height:220px;padding:34px 36px}.admin-products-compact .admin-clean-hero h1{font-size:clamp(54px,7vw,86px);line-height:.88}.admin-products-compact .admin-course-grid{display:grid!important;grid-template-columns:repeat(auto-fill,minmax(340px,1fr))!important;gap:24px!important;align-items:stretch!important}.admin-products-compact .admin-course-card{min-height:0!important;height:auto!important;display:flex!important;flex-direction:column!important;border-radius:28px!important;overflow:visible!important;background:linear-gradient(180deg,rgba(255,255,255,.055),rgba(255,255,255,.024))!important}.admin-products-compact .admin-course-cover{aspect-ratio:16/10!important;height:auto!important;min-height:0!important;position:relative!important;overflow:hidden!important;border-radius:28px 28px 0 0!important;background:#09090d!important}.admin-products-compact .admin-course-cover img{width:100%!important;height:100%!important;object-fit:cover!important;display:block!important}.admin-products-compact .admin-course-cover span{height:100%;display:grid;place-items:center;font-size:38px;color:#f5c76b}.admin-products-compact .admin-course-body{flex:1!important;padding:18px 20px 20px!important;display:flex!important;flex-direction:column!important;gap:11px!important}.admin-products-compact .admin-course-body h2{font-size:26px!important;line-height:1.04!important;margin:0!important;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.admin-products-compact .admin-course-body p{font-size:14px!important;line-height:1.38!important;margin:0!important;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.admin-products-compact .product-meta-line{margin-top:auto!important;padding-top:12px!important}.admin-products-compact .admin-clean-actions{margin-top:6px!important;display:flex!important;gap:10px!important;flex-wrap:wrap!important}.admin-products-compact .admin-clean-button{padding:11px 16px!important;border-radius:14px!important}.admin-products-compact .product-manager-row{display:flex;gap:8px;flex-wrap:wrap;margin-top:2px}.admin-products-compact .product-manager-row form{display:inline-flex}.admin-products-compact .product-danger{border-color:rgba(255,95,95,.36)!important;color:#ffb4b4!important}.admin-products-compact .product-access-note{font-size:12px;color:rgba(255,255,255,.58)}.central-access-panel{margin:34px 0 0;border:1px solid rgba(245,199,107,.18);border-radius:30px;background:linear-gradient(135deg,rgba(255,255,255,.055),rgba(255,255,255,.02));padding:26px}.central-access-head{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;margin-bottom:20px}.central-access-head h2{margin:0;font-size:34px;letter-spacing:-.04em}.central-access-head p{margin:8px 0 0;color:rgba(255,255,255,.62);max-width:720px}.central-access-grid{display:grid;gap:18px}.central-access-group{border:1px solid rgba(255,255,255,.09);border-radius:24px;background:rgba(0,0,0,.18);padding:18px}.central-access-group>h3{margin:0 0 14px;color:#f5c76b;text-transform:uppercase;letter-spacing:.12em;font-size:13px}.central-access-row{display:grid;grid-template-columns:minmax(0,1fr) 180px;gap:14px;align-items:center;border-top:1px solid rgba(255,255,255,.07);padding:14px 0}.central-access-row:first-of-type{border-top:0}.central-access-row strong{display:block;color:#fff;font-size:16px}.central-access-row small{display:block;margin-top:4px;color:rgba(255,255,255,.52);line-height:1.35}.central-access-row select{width:100%;height:44px;border-radius:14px;border:1px solid rgba(245,199,107,.22);background:#08090d;color:#fff;padding:0 12px;font-weight:900}.central-access-row button{height:44px;border:0;border-radius:14px;background:#f5c76b;color:#130d04;font-weight:950;padding:0 12px}.central-access-control{display:grid;grid-template-columns:1fr auto;gap:8px}.level-pill{display:inline-flex;align-items:center;border:1px solid rgba(255,255,255,.12);border-radius:999px;padding:4px 8px;margin-left:8px;color:rgba(255,255,255,.68);font-size:11px;text-transform:uppercase}.central-access-nested{margin-left:16px;padding-left:16px;border-left:1px solid rgba(245,199,107,.14)}@media(max-width:720px){.admin-products-compact .admin-clean-hero{padding:28px 22px}.admin-products-compact .admin-course-grid{grid-template-columns:1fr!important;gap:16px!important}.admin-products-compact .admin-course-cover{aspect-ratio:16/9!important}.admin-products-compact .admin-course-body{padding:15px!important}.admin-products-compact .admin-course-body h2{font-size:22px!important}.central-access-panel{padding:18px;border-radius:24px}.central-access-head{display:grid}.central-access-row{grid-template-columns:1fr}.central-access-control{grid-template-columns:1fr}.central-access-nested{margin-left:8px;padding-left:10px}}`;
...
async function setCentralAccessRule(formData: FormData) {
  ...
  const level = String(formData.get('level') || 'open') as CentralAccessLevel;
  const note = String(formData.get('note') || '').trim();
  if (!key || !['open', 'subscriber', 'vip', 'locked'].includes(level)) return;
  await supabase.from('central_access_rules').upsert({ key, level, note, updated_at: new Date().toISOString() }, { onConflict: 'key' });
...
function AccessControl({ itemKey, title, description, rules }: { itemKey: string; title: string; description: string; rules: Record<string, CentralAccessLevel> }) {
  ...
  return <form className="central-access-row" action={setCentralAccessRule}><div><strong>{title}<span className="level-pill">{accessLabel(value)}</span></strong><small>{description}</small></div><div className="central-access-control"><input type="hidden" name="key" value={itemKey} /><input type="hidden" name="note" value={description} /><select name="level" defaultValue={value}><option value="open">Aberto</option><option value="subscriber">Assinantes</option><option value="vip">VIP</option><option value="locked">Bloqueado</option></select><button type="submit">Salvar</button></div></form>;
}
...
export default async function AdminProductsPage() {
  ...
  const [{ data }, accessRows] = await Promise.all([
    supabase.from('products').select('id,name,slug,description,type,status,cover_url,price_cents,billing_type,courses(id,slug,sort_order)').neq('status', 'archived').order('created_at', { ascending: true }),
  ...
  return (
    <main className="admin-page-clean admin-products-compact">
      ...
      <section className="central-access-panel">
        <div className="central-access-head"><div><span className="admin-clean-eyebrow">Controle granular</span><h2>Central de Atividades</h2><p>Controle a liberação externa e interna da Central. Você pode bloquear a Central inteira, Diários, Personalizados, categorias e exercícios individuais.</p></div><a className="admin-clean-button secondary" href="/aluno/central">Prévia da Central</a></div>
        <div className="central-access-grid">
          <div className="central-access-group"><h3>Entradas principais</h3><AccessControl itemKey="central" title="Central inteira" description="Controla o acesso geral à Central de Treinamento." rules={rules} /><AccessControl itemKey="daily" title="Exercícios Diários" description="Controla o card e a entrada dos desafios diários." rules={rules} /><AccessControl itemKey="personalized" title="Personalizados" description="Controla o card e a entrada do treino por objetivo." rules={rules} /><AccessControl itemKey="repertoire" title="Estude seu Repertório" description="Controla o card de estudo de repertório dentro da Central." rules={rules} /></div>
          <div className="central-access-group"><h3>Diários · dias e exercícios internos</h3>{dailyTrainingSteps.map((step) => { const exercise = getDailyTrainingExercise(step); return <div className="central-access-nested" key={step.exerciseNumber}><AccessControl itemKey={`daily_step_${step.exerciseNumber}`} title={`Dia ${step.day} · Atividade ${step.exerciseNumber}`} description={step.title} rules={rules} />{exercise ? <AccessControl itemKey={`exercise_${exercise.slug}`} title={`Exercício interno · ${exercise.title}`} description={exercise.description} rules={rules} /> : null}</div>; })}</div>
          <div className="central-access-group"><h3>Personalizados · categorias e exercícios</h3>{trainingCategories.map((category) => <div className="central-access-nested" key={category.slug}><AccessControl itemKey={`custom_category_${category.slug}`} title={category.title} description={category.description} rules={rules} />{getExercisesByCategory(category.slug).map((exercise) => <AccessControl itemKey={`exercise_${exercise.slug}`} title={exercise.title} description={exercise.description} rules={rules} key={exercise.slug} />)}</div>)}</div>
        </div>
      </section>
