...
export default async function AdminProductsPage() {
  ...
  return (
    <main className="admin-page-clean admin-products-compact">
      ...
      <section className="admin-course-grid">{products.map((product) => <article className="admin-course-card" key={product.id}><div className="admin-course-cover">{product.cover_url ? <img src={product.cover_url} alt={product.name} /> : <span>{product.name.slice(0, 2).toUpperCase()}</span>}</div><div className="admin-course-body"><span className="admin-clean-pill">{product.billing_type === 'recurring' ? 'Assinatura' : 'Pagamento único'} · {product.status || 'draft'}</span><h2>{product.name}</h2><p className="admin-clean-muted">{product.description || 'Sem descrição cadastrada.'}</p><div className="product-meta-line"><strong>{money(product.price_cents)}</strong><small>{product.courses?.length || 0} área vinculada</small></div><div className="admin-clean-actions"><a className="admin-clean-button primary" href={`/admin/produtos/${product.id}`}>Gerenciar</a><a className="admin-clean-button secondary" href={`/admin/produtos/${product.id}/destino`}>Destino</a><a className="admin-clean-button secondary" href="/aluno/biblioteca">Prévia</a></div></div></article>)}</section>
