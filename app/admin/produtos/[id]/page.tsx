...
export default async function ProductEditPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams?: Promise<Search> }) {
  ...
  const supabase = createAdminClient();
  const [{ data: product }, { data: course }] = await Promise.all([supabase.from('products').select('*').eq('id', id).maybeSingle(), supabase.from('courses').select('*').eq('product_id', id).order('created_at', { ascending: true }).limit(1).maybeSingle()]);
  ...
  return (
    <main className="admin-page-clean">
      <section className="admin-clean-hero"><div><span className="admin-clean-eyebrow">Gerenciar produto</span><h1>{product.name}</h1><p>{product.description || 'Configure produto, preco, capa, modulos, aulas, alunos e comentarios.'}</p></div><a className="admin-clean-button secondary" href="/admin/produtos">Voltar</a></section>
        ...
        <section className="admin-clean-section">
          ...
          <div className="admin-member-modules">
            {modules.map((module, index) => {
              ...
              const missingStreamCount = lessons.filter((lesson) => !lesson.stream_uid).length;
                ...
                <article className="admin-member-module" key={module.id}>
                  <div className="admin-member-module-head">
                    ...
                    <div className="admin-clean-actions">
                      ...
                      <AdminContentStreamTools productId={product.id} moduleId={module.id} moduleTitle={module.title || 'Módulo'} missingInitial={missingStreamCount} />
