...
export default async function CourseManagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const supabase = createAdminClient();
  const [courseResult, productsResult, modulesResult, legacyResult] = await Promise.all([
  ...
  const currentModules = (modulesResult.data || []) as AnyRow[];
  const legacyModules = ((legacyResult.data || []) as AnyRow[]).map((module) => ({ ...module, legacy: true, lessons: module.exercises || [] }));
  const visibleModules: AnyRow[] = currentModules.length > 0 ? currentModules : legacyModules;
  ...
  return (
    <main className="page admin-shell school-admin-shell">
      <section className="admin-hero school-hero compact-hero">
        <div><p className="eyebrow">Gerenciar curso</p><h1>{course.title}</h1><p className="muted">{product?.name ? `Liberado por: ${product.name}` : 'Curso ainda sem produto de acesso vinculado.'}</p></div>
      ...
      <section className="school-two-column">
        <div className="stack">
          ...
          <section className="card premium-panel">
            <div className="section-heading"><div><p className="eyebrow">Conteúdo</p><h2>Módulos e aulas</h2></div></div>
            <div className="admin-list">
              {visibleModules.length ? visibleModules.map((module) => (
                <article className="module-board-card" key={module.id}>
                  <div className="module-board-head"><div><span className="pill">{module.legacy ? 'Biblioteca atual' : `Ordem ${module.sort_order || 0}`}</span><h3>{module.title}</h3><p className="muted">{module.description || 'Sem descrição.'}</p></div></div>
                  <div className="lesson-mini-list">
                    {((module.lessons || []) as AnyRow[]).sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0)).map((lesson) => <div className="lesson-mini-row" key={lesson.id}><span>{lesson.lesson_type || lesson.media_type || 'atividade'}</span><strong>{lesson.title}</strong><small>{module.legacy ? 'Legado VIP' : `${lesson.allow_submission ? 'Com envio' : 'Sem envio'} · ${lesson.allow_community_post ? 'Comunidade' : 'Privado'}`}</small></div>)}
        ...
        <aside className="stack sticky-panel">
          <form className="card premium-panel premium-form" action={createModule}>
            <input type="hidden" name="course_id" value={course.id} /><p className="eyebrow">Novo módulo</p><h2>Criar módulo</h2>
            <label><span>Título</span><input name="title" placeholder="Ex: Primeiros duetos" required /></label><label><span>Descrição</span><textarea name="description" /></label><label><span>Ordem</span><input name="sort_order" type="number" defaultValue="0" /></label>
          ...
          <form className="card premium-panel premium-form" action={createLesson}>
            <input type="hidden" name="course_id" value={course.id} /><p className="eyebrow">Nova aula</p><h2>Criar aula ou atividade</h2>
            <label><span>Módulo</span><select name="module_id" defaultValue="" required><option value="">Selecionar módulo</option>{currentModules.map((module) => <option key={module.id} value={module.id}>{module.title}</option>)}</select></label>
            <label><span>Título</span><input name="title" placeholder="Ex: Treino de segunda voz" required /></label><label><span>Slug</span><input name="slug" /></label><label><span>Descrição</span><textarea name="description" /></label>
            <div className="form-grid-two"><label><span>Tipo</span><select name="lesson_type" defaultValue="lesson"><option value="lesson">Aula</option><option value="activity">Atividade</option><option value="material">Material</option><option value="live">Ao vivo</option></select></label><label><span>Ordem</span><input name="sort_order" type="number" defaultValue="0" /></label></div>
            <label><span>URL do vídeo</span><input name="video_url" /></label><label><span>URL da capa</span><input name="cover_url" /></label>
            <label className="premium-check"><input type="checkbox" name="allow_submission" /><span>Permitir envio de atividade</span></label><label className="premium-check"><input type="checkbox" name="allow_community_post" /><span>Permitir publicar na comunidade</span></label>
