...
class RuntimeScheduler implements VoiceStudioRuntimeScheduler {
  ...
  cancelAll(): void {
    this.#handles.forEach(handle => this.#clearTimeout(handle));
    this.#handles.clear();
  }
...
class RuntimePlaybackClock implements VoiceStudioPlaybackClock {
  ...
  get position(): number {
    ...
    return Math.max(0, this.#position + (this.#now() - this.#startedAt));
  }
  ...
  seek(position: number): void {
    this.#position = Math.max(0, position);
    if (this.#running) this.#startedAt = this.#now();
  }
...
export class VoiceStudioRuntime {
  ...
  objectURLSnapshot(): Readonly<Record<string, string>> {
    return Object.fromEntries(this.#objectUrls);
  }
  ...
  cacheDecodedAudio(assetId: string, audioBuffer: AudioBuffer): void {
    this.#decodedAudio.set(assetId, audioBuffer);
  }

  getDecodedAudio(assetId: string): AudioBuffer | undefined {
    return this.#decodedAudio.get(assetId);
  }

  deleteDecodedAudio(assetId: string): boolean {
    return this.#decodedAudio.delete(assetId);
  }

  cacheWaveform(assetId: string, waveform: VoiceStudioWaveformData): void {
    this.#waveforms.set(assetId, waveform);
  }

  getWaveform(assetId: string): VoiceStudioWaveformData | undefined {
    return this.#waveforms.get(assetId);
  }

  deleteWaveform(assetId: string): boolean {
    return this.#waveforms.delete(assetId);
  }
