...
export class VoiceStudioTransportController {
  ...
  #playback: VoiceStudioPlaybackEngine | null = null;
  ...
  constructor(options: CreateVoiceStudioTransportOptions = {}) {
    ...
    this.#snapshot = {
      ...
      countBeat: 0,
      loop: {
        enabled: options.loop?.enabled ?? false,
        start: clampTime(options.loop?.start ?? 0),
        end: clampTime(options.loop?.end ?? 0),
      },
      punch: {
        enabled: options.punch?.enabled ?? false,
        in: options.punch?.in == null ? null : clampTime(options.punch.in),
        out: options.punch?.out == null ? null : clampTime(options.punch.out),
      },
    };
  }
  ...
  attachPlayback(playback: VoiceStudioPlaybackEngine): void {
    if (this.#playback && this.#playback !== playback) throw new Error('Playback is already attached to this Transport.');
    this.#playback = playback;
  }

  async play(snapshot: VoiceStudioPlaybackSnapshot): Promise<void> {
    const playback = this.#requirePlayback();
    const offset = clampTime(snapshot.offset);
    this.#patch({ status: 'playing', playhead: offset, countBeat: 0 });
    try {
      await playback.play({
        ...snapshot,
        offset,
        loop: snapshot.mode === 'loop' || this.#snapshot.loop.enabled,
      });
    } catch (error) {
  ...
  setCountInBars(countInBars: number): void {
    this.#patch({ countInBars: Math.max(0, Math.floor(countInBars)), countBeat: 0 });
  }

  beginCountIn(): void {
    this.#patch({ status: 'countin', countBeat: 0 });
  }

  setCountBeat(countBeat: number): void {
    this.#patch({ countBeat: Math.max(0, Math.floor(countBeat)) });
  }

  beginRecording(): void {
    this.#patch({ status: 'recording', countBeat: 0 });
  }
  ...
  setTempo(tempo: number): void {
    const next = clampTempo(tempo);
    this.#patch({ tempo: next, bpm: next });
  }
  ...
  handlePlaybackTick(time: number): void {
    if (this.#snapshot.status !== 'playing') return;
    this.#patch({ playhead: clampTime(time) });
  }

  handlePlaybackEnded(time: number, reason: VoiceStudioPlaybackEndReason): void {
    if (reason === 'loop') {
      this.#patch({ status: 'playing', playhead: clampTime(time) });
      return;
    }
  ...
  #requirePlayback(): VoiceStudioPlaybackEngine {
...
export function createVoiceStudioTransportController(
  options: CreateVoiceStudioTransportOptions = {},
