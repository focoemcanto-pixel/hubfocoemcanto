...
import { ArrowLeft, Check, Cloud, Download, Folder, FolderOpen, FolderUp, HardDrive, LogIn, Play, Square, X } from 'lucide-react';
...
export default function LiveRecordingCenterRuntime() {
  ...
  async function uploadDrive(){if(!blob)return;if(!driveConnected){setStatus('Conecte o Google Drive antes de salvar.');return}setUploading(true);setStatus('Preparando pasta no Google Drive…');try{const fileName=`${safe(theme)} — ${new Date().toLocaleDateString('pt-BR').replaceAll('/','-')} — ${new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}).replace(':','h')}.webm`;const session=await fetch('/api/live/recordings/drive-session',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({fileName,mimeType:blob.type||'video/webm',theme:safe(theme),dateLabel:new Date().toLocaleDateString('pt-BR').replaceAll('/','-'),destinationFolderId:selectedFolder.id})});const data=await session.json();if(!session.ok)throw new Error(data.error||'Falha ao conectar ao Drive');setStatus('Enviando gravação ao Google Drive…');const upload=await fetch(data.uploadUrl,{method:'PUT',headers:{'content-type':blob.type||'video/webm','content-length':String(blob.size)},body:blob});if(!upload.ok)throw new Error('O envio ao Google Drive falhou.');setStatus(`Gravação salva em ${selectedFolder.name}.`);setBlob(null)}catch(error){setStatus(error instanceof Error?error.message:'Erro ao enviar ao Google Drive.')}finally{setUploading(false)}}
  ...
  return <>
    ...
    {open&&<div className="fl-recording-center-backdrop" onPointerDown={()=>setOpen(false)}><section className="fl-recording-center" onPointerDown={e=>e.stopPropagation()}>
      ...
      {blob&&<div className="fl-recording-result"><b>Arquivo pronto</b><small>{(blob.size/1024/1024).toFixed(1)} MB</small><div><button onClick={download}><Download/> Baixar</button><button disabled={uploading||!driveConnected} className="drive" onClick={uploadDrive}><FolderUp/> {uploading?'Enviando…':'Salvar no Google Drive'}</button></div></div>}
      {status&&<p className="fl-recording-status">{status}</p>}
      <footer>{mode==='daily'?'A disponibilidade depende do plano e da configuração da sua conta Daily.':'A pasta escolhida fica como destino desta gravação; dentro dela o Foco Live organiza ano, mês, tema e data.'}</footer>
    </section></div>}
  </>;
}
