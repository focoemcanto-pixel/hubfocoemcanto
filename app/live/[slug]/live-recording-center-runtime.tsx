...
import { Cloud, Download, FolderUp, HardDrive, Pause, Play, Square, X } from 'lucide-react';
...
export default function LiveRecordingCenterRuntime() {
  ...
  async function startDaily(){
    try{
      ...
      startedRef.current=Date.now();setElapsed(0);setRecording(true);setStatus('Gravação na nuvem iniciada.');
  ...
  async function stopDaily(){
    try{ await (window as LiveWindow).__FOCO_LIVE_CALL__?.stopRecording?.(); setRecording(false); setStatus('Gravação enviada para processamento na Daily.'); }
    catch{setStatus('Não foi possível encerrar a gravação Daily.');}
  }
  async function startLocal(){
    try{
      ...
      recorder.onstop=()=>{const result=new Blob(chunksRef.current,{type:recorder.mimeType||'video/webm'});setBlob(result);setRecording(false);stream.getTracks().forEach(t=>t.stop());setStatus('Gravação pronta para baixar ou salvar no Google Drive.');};
      stream.getVideoTracks()[0]?.addEventListener('ended',()=>recorder.state!=='inactive'&&recorder.stop(),{once:true});
      recorder.start(1000);startedRef.current=Date.now();setElapsed(0);setRecording(true);setStatus('Gravando pelo Foco Live.');
  ...
  return <div className="fl-recording-center-backdrop" onPointerDown={()=>!recording&&setOpen(false)}>
    <section className="fl-recording-center" onPointerDown={e=>e.stopPropagation()}>
      <header><div><small>FOCO LIVE</small><strong>Central de gravação</strong></div><button disabled={recording} onClick={()=>setOpen(false)}><X/></button></header>
      ...
      <footer>{mode==='daily'?'A disponibilidade depende do plano e da configuração da sua conta Daily.':'O vídeo é enviado diretamente do navegador ao seu Google Drive.'}</footer>
    </section>
  </div>;
}
