...
export default function VoiceStudioDaw({ readOnly }: { readOnly: boolean }) {
  ...
  return <div className="vs-daw" style={{ '--vs-zoom': project.view.zoom, '--vs-track-height': `${Math.round(92 * verticalZoom)}px` } as React.CSSProperties}>
    ...
    <header className="vs-transport">
      ...
      <div className="vs-main-controls"><button onClick={() => playAll()} disabled={!projectHasContent(project) || status === 'recording' || status === 'countin'}>{status === 'playing' ? <Pause/> : <Play/>}</button><button className={status === 'recording' ? 'recording' : 'record'} onClick={status === 'recording' ? stopRecording : beginRecord} disabled={readOnly || status === 'countin' || status === 'playing'}>{status === 'recording' ? <Square/> : <Circle fill="currentColor"/>}</button><button title="Parar" onClick={() => stopPlayback(true)} disabled={status !== 'playing'}><Square/></button><button title="Tocar seleção" onClick={() => playAll('selection')} disabled={!selectionRange || status !== 'idle'}>SEL</button><button className={project.loop.enabled ? 'active' : ''} title="Loop" onClick={() => patchProject({ loop: { ...project.loop, enabled: !project.loop.enabled, ...(selectionRange && !project.loop.enabled ? { start: selectionRange.start, end: selectionRange.end } : {}) } })}>LOOP</button><time>{timeLabel(elapsed)}</time></div>
