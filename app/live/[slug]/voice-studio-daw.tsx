...
export default function VoiceStudioDaw({ readOnly }: { readOnly: boolean }) {
  ...
  const timeline = useVoiceStudioTimeline({
    ...
    onViewChange: view => {
      setProject(current => ({ ...current, view: { ...view, playhead: current.view.playhead } }));
    },
  });
  ...
  function moveDrag(event: React.PointerEvent) { const drag = dragRef.current; if (!drag) return; const location = findClip(drag.initialProject, drag.clipId); if (!location) return; const delta = timeline.timeFromClientX(event.clientX) - timeline.timeFromClientX(drag.startX); if (drag.mode === 'move') setProject(moveClip(drag.initialProject, drag.clipId, drag.trackId, quantize(location.clip.start + delta))); else if (drag.mode === 'trim-right') setProject(resizeClip(drag.initialProject, drag.clipId, 'right', quantize(location.clip.duration + delta))); else setProject(resizeClip(drag.initialProject, drag.clipId, 'left', quantize(location.clip.start + delta))); }
