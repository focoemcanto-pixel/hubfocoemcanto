...
import { TIMELINE_ZOOM_LIMITS, timelinePixelsToTime, timelineSnapTime, timelineTrackHeight } from './voice-studio-timeline-engine';
...
export default function VoiceStudioDaw({ readOnly }: { readOnly: boolean }) {
  ...
  function seekTimeline(event: React.MouseEvent<HTMLElement>) { if (status !== 'idle' || (event.target as HTMLElement).closest('.vs-clip')) return; timeline.seekAtClientX(event.clientX, quantize); setSelectedIds(new Set()); }
  ...
  function moveDrag(event: React.PointerEvent) { const drag = dragRef.current; if (!drag) return; const location = findClip(drag.initialProject, drag.clipId); if (!location) return; const delta = timelinePixelsToTime(event.clientX - drag.startX, project.view.zoom); if (drag.mode === 'move') setProject(moveClip(drag.initialProject, drag.clipId, drag.trackId, quantize(location.clip.start + delta))); else if (drag.mode === 'trim-right') setProject(resizeClip(drag.initialProject, drag.clipId, 'right', quantize(location.clip.duration + delta))); else setProject(resizeClip(drag.initialProject, drag.clipId, 'left', quantize(location.clip.start + delta))); }
  ...
  const fitProject = useCallback(() => timeline.setZoom(Math.max(TIMELINE_ZOOM_LIMITS.minimum, Math.min(TIMELINE_ZOOM_LIMITS.maximum, timeline.viewport.width / Math.max(1, duration * 56)))), [duration, timeline]);
  const fitSelection = useCallback(() => {
    if (!selected.length) return fitProject();
    const start = Math.min(...selected.map(item => item.clip.start));
    const end = Math.max(...selected.map(item => item.clip.start + item.clip.duration));
    const span = Math.max(1, end - start);
    timeline.setZoom(Math.max(TIMELINE_ZOOM_LIMITS.minimum, Math.min(TIMELINE_ZOOM_LIMITS.maximum, timeline.viewport.width / (span * 56))));
    window.requestAnimationFrame(() => timeline.ensureTimeVisible(start, true));
  }, [fitProject, selected, timeline]);
  const setVerticalZoom = useCallback((verticalZoom: number) => patchProject({ view: { ...project.view, verticalZoom } }), [project.view]);
  const trackHeight = timelineTrackHeight(project.view.verticalZoom);

  return <div className="vs-daw" style={{ '--vs-zoom': project.view.zoom, '--vs-track-height': `${trackHeight}px` } as React.CSSProperties}>
    ...
    <header className="vs-transport">
      ...
      <div className="vs-edit-tools"><button title="Desfazer" disabled={!history.length || status !== 'idle'} onClick={undo}><Undo2/></button><button title="Refazer" disabled={!future.length || status !== 'idle'} onClick={redo}><Redo2/></button><button title="Dividir clip no playhead (S)" disabled={!selected.length || status !== 'idle'} onClick={splitSelected}><Scissors/></button><button title="Duplicar clip" disabled={!selected.length || status !== 'idle'} onClick={duplicateSelected}><Copy/></button><button className={project.settings.snapping ? 'active' : ''} title="Snap" onClick={() => patchProject({ settings: { ...project.settings, snapping: !project.settings.snapping } })}><Magnet/></button><button title="Reduzir zoom horizontal" onClick={() => timeline.setZoom(project.view.zoom - 0.25)}><ZoomOut/></button><button title="Aumentar zoom horizontal" onClick={() => timeline.setZoom(project.view.zoom + 0.25)}><ZoomIn/></button><button title="Reduzir zoom vertical" onClick={() => setVerticalZoom(Math.max(TIMELINE_ZOOM_LIMITS.verticalMinimum, project.view.verticalZoom - 0.1))}>V−</button><button title="Aumentar zoom vertical" onClick={() => setVerticalZoom(Math.min(TIMELINE_ZOOM_LIMITS.verticalMaximum, project.view.verticalZoom + 0.1))}>V+</button><button title="Fit Project" onClick={fitProject}>FIT</button><button title="Fit Selection" disabled={!selected.length} onClick={fitSelection}>SEL</button></div>
