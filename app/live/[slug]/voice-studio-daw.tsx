...
export default function VoiceStudioDaw({readOnly}:{readOnly:boolean}){
  ...
  function cleanup(){if(timerRef.current)window.clearInterval(timerRef.current);if(metroRef.current)window.clearInterval(metroRef.current);if(rafRef.current)cancelAnimationFrame(rafRef.current);streamRef.current?.getTracks().forEach(t=>t.stop());playAudiosRef.current.forEach(a=>{a.pause();a.src='';});playAudiosRef.current=[];scheduledNodesRef.current.forEach(node=>{try{node.disconnect();}catch{}});scheduledNodesRef.current=[];unbindMidiInput();void contextRef.current?.close().catch(()=>undefined);}
  ...
  function handleMidiMessage(event:any){
    const [command,note,velocity]=event.data as [number,number,number];const type=command&0xf0;const noteOn=type===0x90&&velocity>0;const noteOff=type===0x80||(type===0x90&&velocity===0);
    if(noteOn){playLiveNote(note,velocity,armed.instrument);if(status==='recording'&&armed.kind==='midi')activeMidiRef.current.set(note,{start:(performance.now()-startAtRef.current)/1000,velocity});}
    if(noteOff){stopLiveNote(note);const active=activeMidiRef.current.get(note);if(active&&status==='recording'&&armed.kind==='midi'){midiNotesRef.current.push({id:crypto.randomUUID(),note,velocity:active.velocity,start:active.start,duration:Math.max(.04,(performance.now()-startAtRef.current)/1000-active.start)});activeMidiRef.current.delete(note);}}
  }
  ...
  async function beginRecord(){if(readOnly||status!=='idle')return;setError('');setElapsed(0);setLivePeaks([]);livePeaksRef.current=[];midiNotesRef.current=[];activeMidiRef.current.clear();try{await audioContext().resume();if(armed.kind==='audio')await prepareAudio();else if(!selectedMidiInput())throw new Error('Conecte e selecione um teclado MIDI.');await countIn();if(armed.kind==='audio')startAudioRecorder();else startMidiRecorder();}catch(reason){setStatus('idle');setError(reason instanceof Error?reason.message:'Não foi possível iniciar a gravação.');cleanupCapture();}}
  ...
  function playAll(){if(status==='playing'){stopPlayback();return;}if(!tracks.length)return;const ctx=audioContext();void ctx.resume();const playable=tracks.filter(t=>!t.muted&&(!soloed||t.solo));playAudiosRef.current=playable.filter(t=>t.kind==='audio'&&t.url).map(t=>{const a=new Audio(t.url);a.volume=t.volume;void a.play();return a;});playable.filter(t=>t.kind==='midi').forEach(track=>scheduleMidiTrack(track,ctx.currentTime));startAtRef.current=performance.now();setStatus('playing');timerRef.current=window.setInterval(()=>{const next=(performance.now()-startAtRef.current)/1000;setElapsed(next);if(next>=projectDuration)stopPlayback();},50);}
  function scheduleMidiTrack(track:Track,base:number){const ctx=audioContext();track.notes.forEach(note=>{const osc=ctx.createOscillator();const gain=ctx.createGain();osc.type=instrumentWave(track.instrument);osc.frequency.value=midiFrequency(note.note);const start=base+note.start;const end=start+note.duration;gain.gain.setValueAtTime(0,start);gain.gain.linearRampToValueAtTime((note.velocity/127)*.16*track.volume,start+.01);gain.gain.setTargetAtTime(.0001,end,.04);osc.connect(gain).connect(ctx.destination);osc.start(start);osc.stop(end+.2);scheduledNodesRef.current.push(osc,gain);});}
  function stopPlayback(){if(timerRef.current)window.clearInterval(timerRef.current);timerRef.current=null;playAudiosRef.current.forEach(a=>a.pause());playAudiosRef.current=[];scheduledNodesRef.current.forEach(node=>{try{if('stop'in node)(node as OscillatorNode).stop();node.disconnect();}catch{}});scheduledNodesRef.current=[];setElapsed(0);setStatus('idle');}
