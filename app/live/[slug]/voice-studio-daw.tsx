...
export default function VoiceStudioDaw({ readOnly }: { readOnly: boolean }) {
  ...
  function scheduleAudioClip(track: VoiceStudioTrack, clip: VoiceStudioClip, asset: VoiceStudioAsset, offset: number) { const url = objectUrlsRef.current[asset.id]; if (!url) return; const delay = Math.max(0, clip.start - offset); const elapsedInsideClip = Math.max(0, offset - clip.start); const sourceTime = clip.sourceOffset + elapsedInsideClip; if (sourceTime >= clip.sourceOffset + clip.duration) return; const audio = new Audio(url); const baseVolume = Math.min(1, Math.max(0, track.volume * clip.gain)); audio.volume = clip.fadeIn > 0 && elapsedInsideClip < clip.fadeIn ? baseVolume * Math.max(0, elapsedInsideClip / clip.fadeIn) : baseVolume; audio.currentTime = sourceTime; playAudiosRef.current.push(audio); const start = () => { void audio.play(); const remaining = Math.max(0.01, clip.duration - elapsedInsideClip); if (clip.fadeIn > 0 && elapsedInsideClip < clip.fadeIn) { const fadeStart = performance.now(); const fadeFrom = audio.volume; const fadeDuration = Math.max(0.01, clip.fadeIn - elapsedInsideClip); const tickFadeIn = () => { const progress = Math.min(1, (performance.now() - fadeStart) / (fadeDuration * 1000)); audio.volume = fadeFrom + (baseVolume - fadeFrom) * progress; if (progress < 1 && !audio.paused) playbackTimersRef.current.push(window.setTimeout(tickFadeIn, 16)); }; tickFadeIn(); } if (clip.fadeOut > 0) { const fadeDelay = Math.max(0, remaining - clip.fadeOut); playbackTimersRef.current.push(window.setTimeout(() => { const fadeStart = performance.now(); const tickFadeOut = () => { const progress = Math.min(1, (performance.now() - fadeStart) / (clip.fadeOut * 1000)); audio.volume = baseVolume * (1 - progress); if (progress < 1 && !audio.paused) playbackTimersRef.current.push(window.setTimeout(tickFadeOut, 16)); }; tickFadeOut(); }, fadeDelay * 1000)); } playbackTimersRef.current.push(window.setTimeout(() => audio.pause(), remaining * 1000)); }; if (delay > 0.005) playbackTimersRef.current.push(window.setTimeout(start, delay * 1000)); else start(); }
