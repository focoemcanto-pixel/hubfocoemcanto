...
function fixture(): { project: VoiceStudioProject; clipId: string; firstTrackId: string; secondTrackId: string } {
  const asset: VoiceStudioAsset = {
    id: 'asset-actions', kind: 'audio', duration: 12, createdAt: '2026-07-19T00:00:00.000Z', peaks: [0.1, 0.5], midiNotes: [],
  };
  ...
  return { project, clipId: project.tracks[0].clips[0].id, firstTrackId: firstTrack.id, secondTrackId: secondTrack.id };
}

describe('VoiceStudioProjectActions', () => {
  it('routes clip mutations through Commands, History and PROJECT_CHANGED', () => {
    ...
    const changed = vi.fn();
    eventBus.subscribe('PROJECT_CHANGED', changed);
  ...
  it('preserves undo and redo through the Actions facade', () => {
    ...
    const eventBus = createVoiceStudioEventBus();
    const actions = createVoiceStudioProjectActions(project, new VoiceStudioHistoryEngine(), eventBus);
  ...
  it('publishes TRACK_UPDATED for track actions', () => {
    const { project, clipId, firstTrackId, secondTrackId } = fixture();
    const eventBus = createVoiceStudioEventBus();
    const trackUpdated = vi.fn();
    eventBus.subscribe('TRACK_UPDATED', trackUpdated);
    const actions = createVoiceStudioProjectActions(project, new VoiceStudioHistoryEngine(), eventBus);
    ...
    expect(trackUpdated).toHaveBeenCalledTimes(2);
  });

  it('merges continuous fade gestures into a single history command', () => {
    const { project, clipId } = fixture();
    const history = new VoiceStudioHistoryEngine();
    const actions = createVoiceStudioProjectActions(project, history, createVoiceStudioEventBus());
