...
export default function LiveCameraControlsRuntime() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) setTransform({ ...DEFAULT_TRANSFORM, ...JSON.parse(saved) });
    } catch {}
  ...
  }, [camera, isHost, pipActive, ready, sceneActive, selected, transform]);
  ...
  useEffect(() => {
    ...
    if (room?.classList.contains('foco-studio-broadcasting')) {
      (window as StudioWindow).__FOCO_LIVE_CALL__?.sendAppMessage?.({ type: 'foco-camera-transform', transform }, '*');
    }
  }, [isHost, room, transform]);

  useEffect(() => {
    if (!isHost || !camera || !sceneActive || !pipActive) return;
    const select = (event: Event) => {
      if ((event.target as HTMLElement).closest('.fl-camera-object-controls')) return;
      event.stopPropagation();
      setSelected(true);
    };
    camera.addEventListener('click', select);
    ...
    return () => { camera.removeEventListener('click', select); window.removeEventListener('pointerdown', clear); };
  }, [camera, isHost, pipActive, sceneActive]);
  ...
  function moveDrag(event: ReactPointerEvent) {
    ...
    if (drag.mode === 'move') {
      ...
      if (next.shape !== 'wide') {
        const squareHeight = (next.width * rect.width) / rect.height;
        next.height = clamp(squareHeight, 10, 75);
      }
  ...
  function setShape(shape: CameraShape) {
    ...
    setTransform((current) => {
      if (shape === 'wide') return { ...current, shape, width: Math.max(current.width, 24), height: Math.max(16, current.height * .62), radius: Math.min(current.radius, 28) };
      const height = (current.width * rect.width) / rect.height;
      return { ...current, shape, height: clamp(height, 10, 75), radius: shape === 'round' ? 50 : 18 };
    });
  }
  ...
  return <>
    {createPortal(<div className="fl-camera-object-controls" onPointerMove={moveDrag} onPointerUp={stopDrag}>
      <button className="fl-camera-drag-surface" aria-label="Mover câmera" onPointerDown={(event) => startDrag(event, 'move')} />
      {selected && <>
        <button className="fl-camera-settings-trigger" aria-label="Configurações da câmera" onClick={(event) => { event.stopPropagation(); setSettingsOpen((value) => !value); }}><Settings2 size={16} /></button>
        {(['resize-nw','resize-ne','resize-sw','resize-se'] as DragMode[]).map((mode) => <button key={mode} className={`fl-camera-resize ${mode}`} aria-label="Redimensionar câmera" onPointerDown={(event) => startDrag(event, mode)} />)}
      </>}
    </div>, camera)}
    ...
    {settingsOpen && createPortal(<section className="fl-camera-settings-panel">
      ...
      <div className="fl-camera-shapes">
        ...
        <button className={transform.shape === 'square' ? 'active' : ''} onClick={() => setShape('square')}><Square size={20} /><span>Quadrada</span></button>
