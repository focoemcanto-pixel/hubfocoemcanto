...
function fakeAudioBuffer(): AudioBuffer {
  return { duration: 2.5, getChannelData: () => new Float32Array([0, 0.25, -0.8, 0.4]) } as unknown as AudioBuffer;
}
...
describe('VoiceStudioAssetStore', () => {
  it('imports audio through Runtime and publishes ASSET_IMPORTED', async () => {
    ...
    eventBus.subscribe('ASSET_IMPORTED', importedEvent);
    ...
    const imported = await store.importAudio(blob, {
      assetId: 'asset-a', fileName: 'voz.wav', createdAt: '2026-07-19T00:00:00.000Z', waveformPoints: 4,
    });

    expect(context.decodeAudioData).toHaveBeenCalledTimes(1);
    expect(imported.asset).toMatchObject({ id: 'asset-a', kind: 'audio', fileName: 'voz.wav', mimeType: 'audio/wav', duration: 2.5 });
    ...
    expect(importedEvent).toHaveBeenCalledWith({ asset: imported.asset, blob });
  });

  it('registers existing assets and publishes them', () => {
    ...
    const asset = { id: 'asset-b', kind: 'audio' as const, duration: 1, createdAt: '2026-07-19T00:00:00.000Z', peaks: [0.2], midiNotes: [] };
    ...
    expect(importedEvent).toHaveBeenCalledWith({ asset, blob });
  });
  ...
  it('removes and disposes resources through Runtime', async () => {
    ...
    expect(() => store.registerAsset({ id: 'late', kind: 'audio', duration: 1, createdAt: '', peaks: [], midiNotes: [] })).toThrow('VoiceStudioAssetStore has been disposed.');
  });
});
