...
export default function VoiceStudioDaw({ readOnly }: { readOnly: boolean }) {
  ...
  function startAudioRecorder() { const stream = streamRef.current; if (!stream) return; const capture = createAudioCapture(stream); captureRef.current = capture; recorderRef.current = capture.recorder; chunksRef.current = capture.chunks; capture.recorder.onstop = () => { void finishAudio(capture); }; capture.recorder.start(100); startClock(); }
  ...
  function cleanupCapture() { if (timerRef.current) window.clearInterval(timerRef.current); timerRef.current = null; stopMetronome(); clearPlayback(); if (rafRef.current) cancelAnimationFrame(rafRef.current); rafRef.current = null; try { inputSourceRef.current?.disconnect(); } catch {} try { monitorGainRef.current?.disconnect(); } catch {} inputSourceRef.current = null; monitorGainRef.current = null; streamRef.current?.getTracks().forEach(track => track.stop()); streamRef.current = null; captureRef.current = null; recorderRef.current = null; chunksRef.current = []; setMeter(0); }
