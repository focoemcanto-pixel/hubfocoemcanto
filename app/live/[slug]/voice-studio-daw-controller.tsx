...
export default function VoiceStudioDaw({ readOnly }: { readOnly: boolean }) {
  ...
  function cleanup() { if (timerRef.current) window.clearInterval(timerRef.current); if (metroRef.current) window.clearInterval(metroRef.current); if (countInTimerRef.current) window.clearInterval(countInTimerRef.current); if (audioCaptureSlot.rafRef.current) cancelAnimationFrame(audioCaptureSlot.rafRef.current); cleanupCapture(); liveOscRef.current.forEach((_, note) => stopLiveNote(note)); clearPlayback(); unbindMidiInput(); revokeObjectUrls(objectUrlsRef.current); void contextRef.current?.close().catch(() => undefined); }
  ...
  async function beginRecord() { if (readOnly || status !== 'idle') return; setError(''); const track = armed.trackId ? project.tracks.find(item => item.id === armed.trackId) : null; if (!track || track.kind !== armed.kind) { setError('Arme uma track compatível antes de gravar.'); return; } recordStartRef.current = punch.enabled && punch.in !== null ? punch.in : (elapsed >= duration ? 0 : elapsed); setLivePeaks([]); audioCaptureSlot.livePeaksRef.current = []; midiNotesRef.current = []; activeMidiRef.current.clear(); recordingSessionRef.current = createRecordingSession({ trackId: track.id, kind: armed.kind, start: recordStartRef.current, latencyCompensation: latencyCompensationMs / 1000, punch }); try { await audioContext().resume(); if (armed.kind === 'audio') await prepareAudio(); else if (!selectedMidiInput()) throw new Error('Conecte e selecione um teclado MIDI.'); await countIn(); startBackingTracks(recordStartRef.current); if (armed.kind === 'audio') startAudioRecorder(); else startClock(); } catch (reason) { recordingSessionRef.current = null; setStatus('idle'); setError(reason instanceof Error ? reason.message : 'Não foi possível iniciar a gravação.'); cleanupCapture(); } }
  async function prepareAudio() { const deviceId = localStorage.getItem('foco-live-microphone-device'); const stream = await navigator.mediaDevices.getUserMedia({ audio: { deviceId: deviceId ? { exact: deviceId } : undefined, echoCancellation: false, noiseSuppression: false, autoGainControl: false } }); audioCaptureSlot.streamRef.current = stream; const context = audioContext(); const source = context.createMediaStreamSource(stream); const analyser = context.createAnalyser(); analyser.fftSize = 512; source.connect(analyser); audioCaptureSlot.inputSourceRef.current = source; audioCaptureSlot.analyserRef.current = analyser; if (monitorInput) { const gain = context.createGain(); gain.gain.value = 0.75; source.connect(gain).connect(context.destination); audioCaptureSlot.monitorGainRef.current = gain; } watchInput(); }
  ...
  function startAudioRecorder() { const stream = audioCaptureSlot.streamRef.current; if (!stream) return; const capture = createAudioCapture(stream); audioCaptureSlot.captureRef.current = capture; audioCaptureSlot.recorderRef.current = capture.recorder; audioCaptureSlot.chunksRef.current = capture.chunks; capture.recorder.onstop = () => { void finishAudio(capture); }; capture.recorder.start(100); startClock(); }
  function watchInput() { const analyser = audioCaptureSlot.analyserRef.current; if (!analyser) return; const data = new Uint8Array(analyser.frequencyBinCount); const draw = () => { analyser.getByteTimeDomainData(data); let sum = 0; let max = 0; for (const value of data) { const normalized = Math.abs((value - 128) / 128); sum += normalized * normalized; max = Math.max(max, normalized); } setMeter(Math.min(1, Math.sqrt(sum / data.length) * 3)); if (audioCaptureSlot.recorderRef.current?.state === 'recording') { audioCaptureSlot.livePeaksRef.current.push(Math.max(0.03, max)); if (audioCaptureSlot.livePeaksRef.current.length > 220) audioCaptureSlot.livePeaksRef.current.shift(); setLivePeaks([...audioCaptureSlot.livePeaksRef.current]); } audioCaptureSlot.rafRef.current = requestAnimationFrame(draw); }; draw(); }
  ...
  function stopRecording() { if (armed.kind === 'audio' && audioCaptureSlot.recorderRef.current?.state === 'recording') { try { audioCaptureSlot.recorderRef.current.requestData(); } catch {} audioCaptureSlot.recorderRef.current.stop(); } else if (armed.kind === 'midi') finishMidi(); }
  function cancelRecording() { if (status !== 'recording' && status !== 'countin') return; if (countInTimerRef.current) window.clearInterval(countInTimerRef.current); countInTimerRef.current = null; const recorder = audioCaptureSlot.recorderRef.current; if (recorder?.state === 'recording') recorder.onstop = null; try { if (recorder?.state === 'recording') recorder.stop(); } catch {} recordingSessionRef.current = null; audioCaptureSlot.chunksRef.current = []; cleanupCapture(); setLivePeaks([]); audioCaptureSlot.livePeaksRef.current = []; setElapsed(recordStartRef.current); setStatus('idle'); }
  async function finishAudio(capture: VoiceStudioAudioCapture) { const session = recordingSessionRef.current; if (!session) { cleanupCapture(); setStatus('idle'); return; } const clipDuration = Math.max(MIN_CLIP, (performance.now() - startAtRef.current) / 1000); const blob = new Blob(capture.chunks, { type: capture.recorder.mimeType || capture.mimeType || 'audio/webm' }); let peaks = audioCaptureSlot.livePeaksRef.current; try { const context = new AudioContext(); const buffer = await context.decodeAudioData(await blob.arrayBuffer()); peaks = makePeaks(buffer.getChannelData(0)); await context.close().catch(() => undefined); } catch {} const asset = buildRecordedAudioAsset({ blob, duration: clipDuration, peaks, fileName: `voz-${Date.now()}.webm` }); addRecordedAsset(asset, blob, `Voz ${project.tracks.filter(track => track.kind === 'audio').length + 1}`, session); recordingSessionRef.current = null; cleanupCapture(); setElapsed(recordStartRef.current); setStatus('idle'); }
