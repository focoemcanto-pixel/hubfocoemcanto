...
const EMPTY_WAVEFORM = Object.freeze(Array.from({ length: 80 }, () => 0.04)) as number[];
...
const TimelineLane = memo(function TimelineLane({ track, assets, zoom, height }: { track: VoiceStudioTrack; assets: Record<string, VoiceStudioAsset>; zoom: number; height: number }) {
...
const Wave = memo(function Wave({ peaks }: { peaks: number[] }) {
  const values = peaks.length ? peaks : EMPTY_WAVEFORM;
