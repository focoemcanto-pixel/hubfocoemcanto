...
export type VoiceStudioLegacyPlaybackMode = 'project' | 'loop' | 'selection';
export type VoiceStudioLegacyPlaybackEndReason = 'stop' | 'pause' | 'ended' | 'loop';

export type VoiceStudioLegacyPlaybackSnapshot = {
  project: VoiceStudioProject;
  objectUrls: Record<string, string>;
  offset: number;
  end: number;
  mode: VoiceStudioLegacyPlaybackMode;
  loop: boolean;
};

export type VoiceStudioLegacyPlaybackCallbacks = {
  getAudioContext: () => AudioContext;
  onTick: (time: number, drift: number) => void;
  onEnded: (time: number, reason: VoiceStudioLegacyPlaybackEndReason) => void;
...
export function legacyPlaybackSelectionRange(project: VoiceStudioProject, clipIds: Iterable<string>) {
...
export class VoiceStudioLegacyPlaybackEngine {
  private callbacks: VoiceStudioLegacyPlaybackCallbacks;
  private snapshot: VoiceStudioLegacyPlaybackSnapshot | null = null;
  ...
  constructor(callbacks: VoiceStudioLegacyPlaybackCallbacks) { this.callbacks = callbacks; }
  get isPlaying() { return !this.stopped && Boolean(this.snapshot); }

  async play(snapshot: VoiceStudioLegacyPlaybackSnapshot) {
  ...
  stop(reset = false, reason: VoiceStudioLegacyPlaybackEndReason = 'stop') {
