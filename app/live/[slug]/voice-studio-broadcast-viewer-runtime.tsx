...
const VISUAL_MESSAGE = 'foco-voice-studio-visual';
...
type VisualSnapshot = { html: string; width: number; height: number; sequence: number };
...
function visualSource() {
  return document.querySelector<HTMLElement>(
    '.fl-studio-scene.app-voice .vs-manager-shell, .fl-studio-scene.app-voice .vs-daw-runtime, .fl-studio-scene.app-voice .vs-daw',
  );
}

function visualHtml(source: HTMLElement) {
  const clone = source.cloneNode(true) as HTMLElement;
  clone.querySelectorAll('audio,video,canvas').forEach(element => {
...
export default function VoiceStudioBroadcastViewerRuntime() {
  useEffect(() => {
    ...
    const requestProject = () => {
      if (!isHost) call()?.sendAppMessage?.({ type: REQUEST_MESSAGE }, '*');
    };
    ...
    const sendVisual = (force = false) => {
      if (!isHost || !voiceSceneOpen() || !broadcasting()) return;
      const source = visualSource();
      if (!source) return;
      const rect = source.getBoundingClientRect();
      const html = visualHtml(source);
      if (!force && html === lastVisual) return;
      lastVisual = html;
      call()?.sendAppMessage?.({
        type: VISUAL_MESSAGE,
        snapshot: {
    ...
    const attachCall = () => {
      ...
      current.on?.('app-message', event => {
        ...
        if (data.type === VISUAL_MESSAGE && !isHost && data.snapshot) {
          window.dispatchEvent(new CustomEvent(VISUAL_EVENT, { detail: data.snapshot }));
        }
    ...
    visualTimer = window.setInterval(() => sendVisual(), 120);
    attachCall();
