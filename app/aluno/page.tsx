...
export default async function StudentPage() {
  ...
  }).filter((item) => item.href !== '/aluno/aula/') : modules.slice(0, 5).map((module, index) => ({ id: module.id, href: `/aluno/biblioteca/${module.slug}`, title: module.title, moduleTitle: 'Sala VIP', cover: module.cover_url || covers[index % covers.length], lessons: module.exercises?.length || 0, percent: index === 0 ? 12 : 0, badge: index === 0 ? 'Começar aqui' : 'Módulo', resume: 'ainda não iniciado' }));
  ...
  return (
    <AppShell>
      <main className="page app-home premium-student-home">
        ...
        <section className="premium-continue-panel"><div className="premium-section-heading"><h2>Continue de onde parou</h2><Link href="/aluno/biblioteca" prefetch>Ver todos →</Link></div><div className="premium-course-row">{continueItems.map((item) => <Link className="premium-course-card" key={item.id} href={item.href} prefetch><div className="course-cover" style={item.cover?.startsWith('radial-gradient') ? { background: item.cover } : { backgroundImage: `url(${item.cover})` }}><span className="course-badge">{item.badge}</span><strong>{item.title}</strong></div><div className="course-resume">{item.moduleTitle} · {item.resume}</div><div className="course-meta"><span>{item.lessons} aulas</span><span>{item.percent}%</span></div><div className="progress"><span style={{ width: `${item.percent}%` }} /></div></Link>)}</div></section>
