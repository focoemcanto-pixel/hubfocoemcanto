  ...
  return (
    <AppShell>
      <main className="page app-home premium-student-home">
        ...
        <section className="premium-hero"><div className="premium-hero-copy"><p className="eyebrow">Grupo VIP Foco em Harmonia ★</p><h1>Olá, {firstName}.<br />Escolha seu treino de hoje.</h1><p>Biblioteca premium com aulas, áudios e duetos organizados por objetivo.</p><div className="hero-actions"><Link className="premium-button gold" href="/aluno/biblioteca" prefetch>▶ Abrir biblioteca</Link><Link className="premium-button dark" href="/aluno/perfil" prefetch>Ver avaliações</Link></div></div><div className="premium-hero-photo" aria-hidden="true" style={{ '--student-hero-image': `url(${studentHeroImage})` } as React.CSSProperties} /><div className="hero-icons"><span>🔔</span><span>{firstName[0]}</span></div></section>
        <section className="premium-continue-panel"><div className="premium-section-heading"><h2>Continue evoluindo</h2><Link href="/aluno/biblioteca" prefetch>Ver todos →</Link></div><div className="premium-course-row">{modules.slice(0, 6).map((module: any, index: number) => { const progress = progressFor(index); return <Link className="premium-course-card" key={module.id} href={`/aluno/biblioteca/${module.slug}`} prefetch><div className="course-cover" style={module.cover_url ? { backgroundImage: `linear-gradient(180deg, transparent 0%, rgba(0,0,0,.35) 42%, rgba(0,0,0,.88) 100%), url(${module.cover_url})` } : { background: fallbackCovers[index % fallbackCovers.length] }}>{index === 0 ? <span className="course-badge">Em andamento</span> : null}<strong>{module.title}</strong></div><div className="course-meta"><span>{module.exercises?.length || 0} aulas</span><span>{progress}%</span></div><div className="progress"><span style={{ width: `${progress}%` }} /></div></Link>; })}</div></section>
