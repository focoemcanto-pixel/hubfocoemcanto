...
function productHref(product: Product, unlocked: boolean) { return isVipProduct(product) ? '/aluno/biblioteca#sala-vip' : unlocked ? `/aluno/biblioteca/${product.slug}` : (product?.sales_url || product?.checkout_url || '/aluno/biblioteca'); }
...
export default async function StudentPage() {
  ...
  const feedPosts = (postsResult.data || []).map((post: any) => ({ id: post.id, authorId: post.profile_id, authorName: post.profiles?.name || 'Aluno VIP', authorAvatarUrl: post.profiles?.avatar_url || null, createdAt: post.created_at, exerciseTitle: post.exercises?.title || 'Atividade da comunidade', exerciseSlug: post.exercises?.slug || null, caption: post.caption || 'Compartilhou uma prática.', mediaUrl: post.media_url || post.submissions?.file_url || null, likesCount: post.likes_count || 0, commentsCount: post.comments_count || 0 }));
  return (
    <AppShell>
