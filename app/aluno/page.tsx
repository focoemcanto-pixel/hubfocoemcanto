...
export default async function StudentPage() {
  ...
  const [{ data: rawModules }, { data: posts }, { data: communitySubmissions }, { data: products }, { data: subscriptions }] = await Promise.all([
  ...
  const courseCards = [
    ...
    { title: 'Foco em Harmonia', description: 'Curso completo de segunda voz e divisão vocal.', unlocked: hasHarmonia, href: hasHarmonia ? productLink(findProduct(['harmonia']), links.harmonia) : productLink(findProduct(['harmonia']), links.harmonia), cover: findProduct(['harmonia'])?.cover_url || covers[1], action: hasHarmonia ? 'Acessar curso' : 'Comprar curso' },
    { title: 'Foco em Canto', description: 'Técnica, extensão, afinação e performance vocal.', unlocked: false, href: productLink(findProduct(['canto']), links.canto), cover: findProduct(['canto'])?.cover_url || covers[2], action: 'Comprar curso' },
    { title: 'Ebooks e Guias', description: 'Materiais complementares para acelerar seus estudos.', unlocked: false, href: productLink(findProduct(['ebook', 'guia']), links.ebooks), cover: findProduct(['ebook', 'guia'])?.cover_url || covers[3], action: 'Comprar acesso' },
  ];
