...
export default async function StudentPage() {
  ...
  const [{ data: rawModules }, { data: profile }, { data: posts }] = await Promise.all([
    ...
    supabase.from('community_posts').select('id,caption,media_url,likes_count,comments_count,created_at,profiles(name),exercises(title,slug)').order('created_at', { ascending: false }).limit(10),
  ]);
                ...
                return (
                  <article className="home-insta-post" key={post.id}>
                    <header className="home-post-head">
                      ...
                      <a href="/aluno/comunidade">•••</a>
                    </header>

                    <a className="home-post-media" href="/aluno/comunidade">
                      {post.media_url ? <video src={post.media_url} controls playsInline preload="metadata" /> : <div className="home-post-placeholder"><div><span>▶</span><strong>{exercise?.title || 'Publicação da comunidade'}</strong></div></div>}
                    </a>

                    <div className="home-post-actions">
                      <form action="/api/community/likes" method="post"><input type="hidden" name="post_id" value={post.id} /><input type="hidden" name="return_to" value="/aluno" /><button type="submit">♡ Curtir</button></form>
                      <a href="/aluno/comunidade">💬 Comentar</a>
                      {exercise?.slug ? <a href={`/aluno/aula/${exercise.slug}`}>Abrir aula</a> : null}
                      {exercise?.slug ? <a href={`/aluno/atividade/${exercise.slug}`}>Realizar atividade</a> : null}
                    </div>
                    ...
                    <form className="home-comment-form" action="/api/community/comments" method="post">
                      <input type="hidden" name="post_id" value={post.id} />
                      <input type="hidden" name="return_to" value="/aluno" />
