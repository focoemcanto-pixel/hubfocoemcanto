...
export default async function StudentPage() {
  ...
  const [{ data: rawModules }, { data: profile }, { data: posts }, { data: communitySubmissions }] = await Promise.all([
    ...
    supabase.from('community_posts').select('id,profile_id,exercise_id,caption,media_url,likes_count,comments_count,created_at,profiles(name),exercises(title,slug),submissions(file_url)').order('created_at', { ascending: false }).limit(10),
