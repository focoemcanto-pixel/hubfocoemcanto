...
export default async function StudentPage() {
  ...
  const [{ data: rawModules }, { data: profile }, { data: posts }] = await Promise.all([
    ...
    supabase.from('community_posts').select('id,caption,media_url,likes_count,comments_count,created_at,profiles(name),exercises(title,slug),submissions(file_url)').order('created_at', { ascending: false }).limit(10),
  ]);
    ...
    <AppShell>
      <main className="page app-home premium-student-home">
        ...
        <section className="feed-layout premium-community-feed">
          ...
          {feedItems.length > 0 ? <div className="home-insta-feed">{feedItems.map((post: any) => {
            const exercise = getRelated(post.exercises);
            const author = getRelated(post.profiles);
            const submission = getRelated(post.submissions);
            const mediaUrl = post.media_url || submission?.file_url || '';
            return <article className="home-insta-post" key={post.id}>
              <header className="home-post-head"><div className="avatar">{initials(author?.name)}</div><div><strong>{author?.name || 'Aluno VIP'}</strong><span>{exercise?.title || 'Atividade da comunidade'}</span></div><a className="home-post-menu" href="/aluno/comunidade">•••</a></header>
              <div className="home-post-media">{mediaUrl ? <video src={mediaUrl} controls playsInline preload="metadata" /> : <div className="home-post-placeholder"><div><span>▶</span><strong>{exercise?.title || 'Publicação da comunidade'}</strong></div></div>}</div>
              <div className="home-icon-actions"><form action="/api/community/likes" method="post"><input type="hidden" name="post_id" value={post.id} /><input type="hidden" name="return_to" value="/aluno" /><button type="submit" aria-label="Curtir">♡</button></form><a href="/aluno/comunidade" aria-label="Comentar">💬</a><a href="/aluno/comunidade" aria-label="Compartilhar">↗</a></div>
              <div className="home-lesson-actions">{exercise?.slug ? <a href={`/aluno/aula/${exercise.slug}`}>Abrir aula</a> : null}{exercise?.slug ? <a className="primary" href={`/aluno/atividade/${exercise.slug}`}>Realizar atividade</a> : null}</div>
              <p className="home-post-caption"><strong>{author?.name || 'Aluno VIP'}</strong>{post.caption || 'Compartilhou uma prática.'}</p>
              <div className="home-post-meta"><span>{post.likes_count || 0} curtidas</span><span>{post.comments_count || 0} comentários</span></div>
              <form className="home-comment-form" action="/api/community/comments" method="post"><input type="hidden" name="post_id" value={post.id} /><input type="hidden" name="return_to" value="/aluno" /><input name="comment" placeholder="Adicionar comentário..." /><button type="submit">Publicar</button></form>
            </article>;
          })}</div> : <div className="empty-community-feed"><h3>Nenhuma postagem real ainda.</h3><p>Quando os alunos publicarem exercícios na comunidade, eles aparecerão aqui automaticamente.</p><a className="premium-button gold" href="/aluno/comunidade">Criar primeira postagem</a></div>}
        </section>
      </main>
    </AppShell>
  );
