...
const links = {
  vip: process.env.NEXT_PUBLIC_VIP_CHECKOUT_URL || 'https://pay.kiwify.com.br/HHr4eyM',
...
function isFreeTuningModule(module: any) { const value = `${module?.title || ''} ${module?.slug || ''}`.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''); return value.includes('firmando') && value.includes('afinacao'); }

function ModuleCard({ module, index, hasVip, vipUrl }: { module: any; index: number; hasVip: boolean; vipUrl: string }) {
  ...
  const label = hasVip ? (progress ? 'Em andamento' : 'Módulo') : isFree ? 'Liberado grátis' : '🔒 VIP';
  return (
    <a className={`module-vertical-card ${unlocked ? isFree ? 'free' : '' : 'frozen'}`} href={unlocked ? `/aluno/biblioteca/${module.slug}` : vipUrl}>
      ...
      {!unlocked ? <div className="module-lock-layer"><span>Assine o VIP para desbloquear este módulo</span></div> : null}
      <div className="module-vertical-copy">
        <h3>{module.title}</h3>
        <div className="module-card-meta"><small>{module.exercises?.length || 0} aulas</small><strong>{unlocked ? `${progress}%` : 'VIP'}</strong></div>
        <div className="stream-progress"><i style={{ width: `${Math.max(8, unlocked ? progress : 8)}%` }} /></div>
      </div>
    </a>
  );
}
...
export default async function StudentLibraryPage() {
  ...
  const vipUrl = productLink(vipProduct, links.vip);
  ...
  const courseCards = [
    { title: 'Sala de Atividades VIP', description: hasVip ? 'Atividades, duetos e comunidade.' : 'Experimente Firmando a Afinação grátis. Assine para liberar tudo.', unlocked: true, href: '#sala-vip', cover: vipProduct?.cover_url || covers[0], action: hasVip ? 'Abrir módulos' : 'Explorar grátis' },
    { title: 'Foco em Harmonia', description: 'Curso completo. Até migrar, o acesso segue pela Kiwify.', unlocked: hasHarmonia, href: productLink(harmoniaProduct, links.harmonia), cover: harmoniaProduct?.cover_url || covers[1], action: hasHarmonia ? 'Acessar na Kiwify' : 'Comprar curso' },
    { title: 'Foco em Canto', description: 'Técnica, extensão e performance.', unlocked: hasCanto, href: hasCanto ? productLink(cantoProduct, links.canto) : productLink(cantoProduct, links.cantoCheckout), cover: cantoProduct?.cover_url || covers[2], action: hasCanto ? 'Acessar curso' : 'Comprar curso' },
    { title: 'Foco em Melismas', description: 'Agilidade, riffs, runs e ornamentações.', unlocked: hasMelismas, href: productLink(melismasProduct, links.melismas), cover: melismasProduct?.cover_url || covers[3], action: hasMelismas ? 'Acessar curso' : 'Comprar curso' },
    { title: 'Ebooks e Guias', description: 'Materiais premium complementares.', unlocked: hasEbooks, href: productLink(ebookProduct, links.ebooks), cover: ebookProduct?.cover_url || covers[4], action: hasEbooks ? 'Acessar materiais' : 'Comprar acesso' },
  ];
    ...
    <AppShell>
      <main className="page school-library-page">
        ...
        <section className="school-library-hero">
          ...
          <p>Experimente a Sala de Atividades com o módulo Firmando a Afinação. O VIP libera todos os módulos, downloads e avaliações do professor.</p>
        </section>
        ...
        <section className="school-section">
          ...
          <div className="course-access-grid">{courseCards.map((course) => <a className={`course-access-card ${course.unlocked ? 'unlocked' : 'locked'}`} href={course.href} key={course.title}><span className="course-access-badge">{course.unlocked ? 'Liberado' : '🔒 Bloqueado'}</span><div className="course-access-bg" style={styleForCover(course.cover)} /><div className="course-access-copy"><h3>{course.title}</h3><p>{course.description}</p><span className="course-access-action">{course.action}</span></div></a>)}</div>
        </section>

        <section id="sala-vip" className="school-section course-room">
          <div className="course-room-header"><div><span className="eyebrow">Grupo VIP</span><h2>Sala de Atividades VIP</h2><p>{hasVip ? 'Todos os módulos estão liberados para você.' : 'O módulo Firmando a Afinação está liberado grátis. Os demais ficam congelados para assinantes VIP.'}</p></div><span className="course-room-pill">{modules.length} módulos</span></div>
          {!hasVip ? <div className="free-callout"><strong>Modo gratuito ativo.</strong><p>Você pode assistir, gravar duetos e postar na comunidade no módulo Firmando a Afinação. Avaliação do professor e downloads são recursos VIP.</p></div> : null}
          <div className="module-vertical-grid">{modules.map((module: any, index: number) => <ModuleCard module={module} index={index} hasVip={hasVip} vipUrl={vipUrl} key={module.id} />)}</div>
        </section>
      </main>
    </AppShell>
  );
