...
function ModuleCard({ module, index, hasVip }: { module: any; index: number; hasVip: boolean }) {
  ...
  return (
    <a className={`module-vertical-card ${unlocked ? isFree ? 'free' : '' : 'frozen'}`} href={`/aluno/biblioteca/${module.slug}`}>
      ...
      {!unlocked ? <div className="module-lock-layer"><span>Toque para ver os benefícios VIP</span></div> : null}
  ...
  return (
    <AppShell>
      <main className="page school-library-page">
        ...
        <section className="school-library-hero"><p className="eyebrow">Biblioteca da escola</p><h1>Escolha seu curso vocal.</h1><p>Experimente a Sala de Atividades com o módulo Firmando a Afinação. O VIP libera todos os módulos, downloads e avaliações do professor.</p></section>
