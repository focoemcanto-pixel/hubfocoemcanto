...
function lockedProductLink(product: any) { if (isFocoEmCanto(product)) return FOCO_EM_CANTO_URL; if (isFocoEmHarmonia(product)) return FOCO_EM_HARMONIA_URL; return productLink(product, VIP_CHECKOUT_URL); }
function lockedAction(product: any) { if (isFocoEmCanto(product) || isFocoEmHarmonia(product)) return 'Conhecer curso'; return 'Liberar acesso'; }
...
export default async function StudentLibraryPage() {
  ...
  const courseCards = productList.map((product, index) => {
    ...
    const description = vip ? (hasVip ? 'Atividades, duetos e comunidade.' : 'Módulo 1 aberto para começar grátis. Os demais ficam liberados no VIP.') : (unlocked ? (product.description || 'Acesso liberado.') : 'Bloqueado. Acesse a página do curso para liberar sua matrícula.');
    const href = unlocked ? (vip ? '#sala-vip' : productLink(product, `/aluno/biblioteca/${product.slug}`)) : lockedProductLink(product);
    const action = unlocked ? (vip ? 'Abrir sala' : 'Acessar curso') : lockedAction(product);
