...
const FOCO_EM_CANTO_URL = process.env.NEXT_PUBLIC_FOCO_EM_CANTO_URL || 'https://focoemcanto.com';
const FOCO_EM_HARMONIA_URL = process.env.NEXT_PUBLIC_FOCO_EM_HARMONIA_URL || 'https://harmonia.focoemcanto.com';
...
function productIdentity(product: any) { return `${product?.name || ''} ${product?.slug || ''}`.toLowerCase(); }
...
function isVip(product: any) { return productIdentity(product).includes('vip'); }
...
function productHref(product: any, unlocked: boolean, vip: boolean) {
  ...
  if (unlocked && isExternalKiwifyCourse(product)) return KIWIFY_LOGIN_URL;
  if (!unlocked && isFocoEmCanto(product)) return FOCO_EM_CANTO_URL;
  if (!unlocked && isFocoEmHarmonia(product)) return FOCO_EM_HARMONIA_URL;
  if (unlocked) return `/aluno/biblioteca/${product.slug}`;
  return VIP_CHECKOUT_URL;
}
function productAction(product: any, unlocked: boolean, vip: boolean) {
  const destination = productDestination(product);
  if (vip) return 'Abrir sala';
  if (unlocked && isExternalKiwifyCourse(product)) return 'Acessar na Kiwify';
  if (unlocked && destination) return 'Acessar destino';
  if (unlocked) return 'Acessar curso';
  if (destination || isExternalKiwifyCourse(product)) return 'Conhecer curso';
  return 'Liberar no VIP';
}
...
function ModuleCard({ module, index, hasVip }: { module: any; index: number; hasVip: boolean }) {
  ...
  const unlocked = hasVip || index === 0;
  const label = unlocked ? (index === 0 && !hasVip ? 'Grátis' : progress ? 'Em andamento' : 'Módulo') : 'VIP';
  return (
...
export default async function StudentLibraryPage() {
  ...
  const courseCards = productList.map((product, index) => {
    ...
    const href = productHref(product, unlocked, vip);
    const action = productAction(product, unlocked, vip);
    return { title, description, unlocked, href, cover: productCover(product, covers[index % covers.length]), action, destination };
  });
  ...
  return (
    <AppShell>
      <main className="page school-library-page">
        ...
        <section className="school-library-hero"><p className="eyebrow">Biblioteca da escola</p><h1>Escolha seu curso vocal.</h1><p>A Sala de Atividades mantém o Módulo 1 aberto para começar grátis. Os demais módulos, duetos, downloads e avaliações ficam exclusivos para alunos VIP.</p></section>
