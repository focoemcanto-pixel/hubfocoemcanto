...
function ModuleCard({ module, index, hasVip }: { module: any; index: number; hasVip: boolean }) {
  ...
  const label = hasVip ? (progress ? 'Em andamento' : 'Módulo') : isFree ? 'Liberado grátis' : '🔒 Acesso VIP';
  return (
...
export default async function StudentLibraryPage() {
  ...
  const courseCards = [
    ...
    { title: 'Foco em Harmonia', description: 'Curso completo. Até migrar, o acesso segue pela Kiwify.', unlocked: hasHarmonia, href: productLink(harmoniaProduct, 'https://dashboard.kiwify.com.br'), cover: harmoniaProduct?.cover_url || covers[1], action: hasHarmonia ? 'Acessar na Kiwify' : 'Comprar curso' },
    { title: 'Foco em Canto', description: 'Técnica, extensão e performance.', unlocked: hasCanto, href: productLink(cantoProduct, 'https://dashboard.kiwify.com.br'), cover: cantoProduct?.cover_url || covers[2], action: hasCanto ? 'Acessar curso' : 'Comprar curso' },
    { title: 'Foco em Melismas', description: 'Agilidade, riffs, runs e ornamentações.', unlocked: hasMelismas, href: productLink(melismasProduct, 'https://dashboard.kiwify.com.br'), cover: melismasProduct?.cover_url || covers[3], action: hasMelismas ? 'Acessar curso' : 'Comprar curso' },
