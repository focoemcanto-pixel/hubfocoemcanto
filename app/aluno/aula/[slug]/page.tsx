    ...
    <main className="premium-lesson-page route-surface vip-lesson-locked-page">
      <section className="vip-lock-backdrop static">
        <section className="vip-lock-modal">
          ...
          <h3>Essa aula é para assinantes da Sala de Atividades VIP</h3>
          <p>O módulo Firmando a Afinação está liberado grátis. Assine para acessar todos os módulos, downloads e avaliação do professor.</p>
          <ul>
            <li>Exercícios guiados</li>
            <li>Envio para avaliação</li>
            <li>Duetos na comunidade</li>
            <li>Correções do professor</li>
          </ul>
          <a className="vip-lock-cta" href={VIP_CHECKOUT_URL}>Assinar e desbloquear agora</a>
          <Link className="vip-lock-later" href="/aluno/biblioteca#sala-vip" prefetch={false}>Voltar para a sala VIP</Link>
        </section>
      </section>
    </main>
  );
...
export default async function StudentLessonPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [settings, lessonResult, profileResult] = await Promise.all([
    getAdminSettings(),
    supabase
      .from('exercises')
  ...
  return (
    <main className="premium-lesson-page route-surface">
      <style dangerouslySetInnerHTML={{ __html: vipListCss }} />
      <aside className="premium-lesson-leftnav" aria-label="Navegação do aluno">
        <Link className="premium-back-link" href={module?.slug ? `/aluno/biblioteca/${module.slug}` : '/aluno/biblioteca'} prefetch={false}><ChevronLeft size={18} />Voltar ao módulo</Link>
        <nav className="premium-nav-stack">
          {navItems.map((item) => {
            const Icon = item.icon;
            const isAnchor = item.href.startsWith('#');
            return isAnchor ? (
              <a className={item.active ? 'premium-nav-item active' : 'premium-nav-item'} href={item.href} key={item.label}><Icon size={22} /><span>{item.label}</span></a>
            ) : (
              <Link className={item.active ? 'premium-nav-item active' : 'premium-nav-item'} href={item.href} prefetch={false} key={item.label}><Icon size={22} /><span>{item.label}</span></Link>
            );
          })}
        </nav>
      ...
      <section className="premium-lesson-main">
        <header className="premium-lesson-topbar">
          <div />
          <Link className="premium-brand" href="/aluno" prefetch={false} aria-label="Ir para o início"><DynamicBrandLogo settings={settings} compact /></Link>
          <div className="premium-top-actions">
            <Link href="/aluno" prefetch={false}><Home size={16} /> Início</Link>
            <Link href="/aluno/perfil" prefetch={false}><Trophy size={16} /> Conquistas</Link>
            <a href="#lesson-notes"><HelpCircle size={16} /> Ajuda</a>
            <Link href="/aluno/perfil" className="premium-avatar premium-avatar-photo" prefetch={false} aria-label={`Abrir perfil de ${avatarLabel}`}>
              {profile?.avatar_url ? <img src={profile.avatar_url} alt={avatarLabel} /> : <span>{initials(profile?.name, profile?.email || email)}</span>}
            </Link>
          </div>
        </header>
        ...
        <div className="premium-content-grid">
          <section className="premium-watch-column">
            <p className="premium-breadcrumb">
              <Link href="/aluno" prefetch={false}>Hub VIP</Link> › {module?.slug ? <Link href={`/aluno/biblioteca/${module.slug}`} prefetch={false}>{module.title}</Link> : module?.title || 'Módulo'}
            </p>
            <div className="premium-player-card">
              <div className="premium-player-frame">
                <ContentPlayer
                  ...
                  trimStartSeconds={lesson?.trim_start_seconds}
                  trimEndSeconds={lesson?.trim_end_seconds}
            ...
            <section className="premium-lesson-details" id="lesson-action">
              <div className="premium-lesson-header-row">
                <div>
                  ...
                  <p>{description}</p>
                  {savedPosition > 5 && !completed ? <small className="muted">Retomando de aproximadamente {Math.floor(savedPosition / 60)}min {savedPosition % 60}s.</small> : null}
                </div>
                {lesson?.id ? <LessonProgressButton exerciseId={lesson.id} initialCompleted={completed} /> : null}
              </div>

              <div className="premium-progress-block">
                <div className="premium-progress-head"><span>Progresso do módulo</span><strong>{currentPosition} de {totalLessons} aulas</strong></div>
                <div className="premium-progress"><span style={{ width: `${progress}%` }} /></div>
              </div>

              <div className="premium-action-row compact">
                <Link className="premium-primary-button" href={`/aluno/atividade/${lesson?.slug || ''}`} prefetch={false}><Headphones size={18} />Realizar atividade</Link>
                <div className="premium-next-actions">
                  {previousLesson ? <Link className="premium-round" href={`/aluno/aula/${previousLesson.slug}`} prefetch={false} aria-label="Aula anterior"><ArrowLeft size={20} /></Link> : <span className="premium-round disabled"><ArrowLeft size={20} /></span>}
                  {nextLesson ? <Link className="premium-round" href={`/aluno/aula/${nextLesson.slug}`} prefetch={false} aria-label="Próxima aula"><ArrowRight size={20} /></Link> : <span className="premium-round disabled"><ArrowRight size={20} /></span>}
                </div>
              </div>
          ...
          <aside className="premium-modules-panel">
            <div className="premium-modules-head">
              <Link href="/aluno/biblioteca" prefetch={false}><ArrowLeft size={18} /> Módulos</Link>
              <Link href="/aluno" prefetch={false}><X size={18} /></Link>
            </div>
            <div className="premium-module-list">
              {modules.map((mod: any) => {
                const lessons = (mod.exercises || []).sort((a: any, b: any) => (a.sort_order || 0) - (b.sort_order || 0));
                const moduleOpen = hasVip || isFreeTuningModule(mod);
                return (
                  <section className={`premium-module-group ${moduleOpen ? '' : 'locked-vip'}`} key={mod.id}>
                    <div className="premium-module-title"><strong>{mod.title}</strong><span>{lessons.length} aulas</span>{!moduleOpen ? <em className="vip-list-badge"><Lock size={11} /> VIP</em> : null}</div>
                    <div className="premium-lessons-list">
                      {lessons.map((item: any, index: number) => {
                        const active = item.slug === lesson?.slug;
                        return (
                          <Link className={`${active ? 'premium-lesson-item active no-thumb' : 'premium-lesson-item no-thumb'} ${moduleOpen ? '' : 'locked-vip-lesson'}`} href={moduleOpen ? `/aluno/aula/${item.slug}` : `/aluno/biblioteca/${mod.slug}`} prefetch={false} key={item.id}>
                            <span className={active ? 'premium-check active' : 'premium-check'}>{active ? <Check size={14} /> : !moduleOpen ? <Lock size={13} /> : null}</span>
                            <span className="premium-thumb premium-generated-thumb"><span>{String(index + 1).padStart(2, '0')}</span><Play size={18} /></span>
                            <span className="premium-lesson-copy"><strong>{item.title}</strong><small>{moduleOpen ? `Aula ${String(index + 1).padStart(2, '0')}` : 'Exclusivo VIP'}</small></span>
                          </Link>
                        );
                      })}
                    </div>
