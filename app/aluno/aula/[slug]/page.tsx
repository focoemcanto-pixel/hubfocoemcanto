...
const vipListCss = `${focoAcademyLogoCss}${dynamicBrandLogoCss}.premium-module-group.locked-vip{position:relative;border:1px solid rgba(245,199,107,.26);border-radius:22px;background:rgba(245,199,107,.04);padding:10px}.premium-module-group.locked-vip .premium-lessons-list{opacity:.68;filter:saturate(.75)}.premium-module-group.locked-vip .premium-module-title strong{color:rgba(255,255,255,.58)}.vip-list-badge{margin-left:auto;border:1px solid rgba(245,199,107,.45);border-radius:999px;padding:5px 9px;color:#f5c76b;background:rgba(245,199,107,.1);font-size:11px;font-style:normal;font-weight:900}.premium-lesson-item.locked-vip-lesson{border:1px solid rgba(245,199,107,.14)}.premium-lesson-item.locked-vip-lesson .premium-lesson-copy small{color:#f5c76b}.premium-avatar-photo{overflow:hidden;padding:0!important}.premium-avatar-photo img,.premium-avatar-photo span{width:100%;height:100%;border-radius:inherit;object-fit:cover;display:grid;place-items:center}.premium-avatar-photo img{display:block}.premium-avatar-photo span{font-size:inherit;font-weight:inherit}.premium-brand .foco-academy-logo.compact svg{width:46px;height:36px}.premium-brand .dynamic-brand-logo.compact img{height:46px;max-width:190px}.premium-brand .brand-wave{display:none!important}`;
...
export default async function StudentLessonPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: lesson } = await supabase.from('exercises').select('id,title,slug,description,objective,media_type,difficulty,drive_url,media_url,audio_url,module_id,modules(title,slug,description)').eq('slug', slug).single();
