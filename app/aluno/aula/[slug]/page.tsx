  ...
  return (
    <main className="premium-lesson-page route-surface">
      <aside className="premium-lesson-leftnav" aria-label="Navegação do aluno">
        <Link className="premium-back-link" href={module?.slug ? `/aluno/biblioteca/${module.slug}` : '/aluno/biblioteca'} prefetch><ChevronLeft size={18} />Voltar ao módulo</Link>
        <nav className="premium-nav-stack">{navItems.map((item) => { const Icon = item.icon; const isAnchor = item.href.startsWith('#'); return isAnchor ? <a className={item.active ? 'premium-nav-item active' : 'premium-nav-item'} href={item.href} key={item.label}><Icon size={22} /><span>{item.label}</span></a> : <Link className={item.active ? 'premium-nav-item active' : 'premium-nav-item'} href={item.href} prefetch key={item.label}><Icon size={22} /><span>{item.label}</span></Link>; })}</nav>
        ...
        <div className="premium-content-grid">
          <section className="premium-watch-column">
            ...
            <section className="premium-lesson-details" id="lesson-action">
              <div className="premium-lesson-header-row"><div><p className="premium-module-label"><Sparkles size={16} /> {module?.title || 'Biblioteca VIP'}</p><h1>{lesson?.title || 'Aula'}</h1><p>{description}</p>{!hasVip ? <small className="muted">Modo gratuito: grave duetos e poste na comunidade. Avaliação do professor é VIP.</small> : null}{savedPosition > 5 && !completed ? <small className="muted">Retomando de aproximadamente {Math.floor(savedPosition / 60)}min {savedPosition % 60}s.</small> : null}</div>{lesson?.id ? <LessonProgressButton exerciseId={lesson.id} initialCompleted={completed} /> : null}</div>
