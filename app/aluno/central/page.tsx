    ...
    <AppShell>
      <main className="page training-center">
        ...
        <section className="training-hero">
          ...
          <div className="today-progress"><div><strong>—</strong><span>Sequência</span></div><div><strong>0/{dailyTrainingSteps.length}</strong><span>Treinos</span></div><div><strong>0</strong><span>Pontos</span></div></div>
        </section>
        <section className="daily-preview">
          ...
          <div className="daily-track">{dailyTrainingSteps.map((step, index) => <><div className="daily-dot current" key={`dot-${step.exerciseNumber}`}>{step.exerciseNumber}</div>{index < dailyTrainingSteps.length - 1 ? <div className="daily-line" key={`line-${step.exerciseNumber}`} /> : null}</>)}</div>
