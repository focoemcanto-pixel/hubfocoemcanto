...
function isRealModule(module: any) { const title = String(module?.title || '').toLowerCase(); const description = String(module?.description || '').toLowerCase(); return Boolean(module?.is_active) && title !== 'biblioteca geral' && !description.includes('importados da pasta'); }
...
export default async function CommunityPage() {
  ...
  const exercises = modules.flatMap((module: any) => (module.exercises || []).filter((exercise: any) => exercise.is_active).sort((a: any, b: any) => (a.sort_order || 0) - (b.sort_order || 0)).map((exercise: any) => ({ ...exercise, moduleTitle: module.title })));
    ...
    <AppShell>
      <main className="community-instagram-page">
        ...
        <section className="community-main-feed">
          <header className="community-feed-topbar"><div><p className="eyebrow">Comunidade VIP</p><h1>Compartilhe sua evolução.</h1><p>Publique sua prática, receba apoio dos alunos e acompanhe o crescimento do grupo.</p></div><div className="community-top-actions"><button type="button" aria-label="Notificações"><Bell size={20} /></button><Avatar className="community-current-avatar" name={firstName} url={currentAvatarUrl} /></div></header>
          <section className="community-create-strip"><div className="community-tabs"><a className="active" href="/aluno/comunidade">Para você</a><a href="/aluno/comunidade">Recentes</a><a href="/aluno/comunidade">Seguindo</a></div><a className="new-post-button" href="#nova-publicacao"><Plus size={30} /><span>Nova publicação</span></a></section>
          <section id="nova-publicacao" className="community-composer-instagram"><div className="composer-topline"><div className="avatar-ring mini"><Avatar name={firstName} url={currentAvatarUrl} /></div><strong>{firstName}</strong><small>Aluno VIP</small></div><form action="/api/community/posts" method="post"><textarea name="caption" placeholder="O que você treinou hoje? Compartilhe sua prática, dificuldade ou conquista..." /><select name="exercise_id" defaultValue=""><option value="">Vincular somente a aula publicada no Hub</option>{exercises.map((item: any) => <option value={item.id} key={item.id}>{item.moduleTitle} — {item.title}</option>)}</select><div className="composer-actions-instagram"><span>Publique práticas gravadas pelo envio da atividade.</span><button type="submit"><Send size={18} /> Publicar</button></div></form></section>
