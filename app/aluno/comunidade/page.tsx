...
const createPostCss = `.community-main-feed,.community-create-strip{overflow:visible!important}.new-post-menu{position:relative;z-index:120}.new-post-menu summary{list-style:none;cursor:pointer}.new-post-menu summary::-webkit-details-marker{display:none}.new-post-menu .new-post-options{display:none}.new-post-menu[open] .new-post-options{position:absolute;right:0;top:calc(100% + 10px);z-index:9999;width:285px;display:grid;gap:8px;border:1px solid rgba(245,199,107,.34);border-radius:20px;background:linear-gradient(145deg,rgba(17,17,24,.99),rgba(7,7,11,.99));box-shadow:0 22px 80px rgba(0,0,0,.75);padding:10px;backdrop-filter:blur(18px);opacity:1!important;visibility:visible!important;animation:newPostMini .14s ease-out}.new-post-menu[open] .new-post-options:after{content:'';position:absolute;right:24px;top:-7px;width:14px;height:14px;transform:rotate(45deg);background:rgba(17,17,24,.99);border-left:1px solid rgba(245,199,107,.34);border-top:1px solid rgba(245,199,107,.34)}.new-post-options a{position:relative;z-index:1;display:grid;grid-template-columns:34px 1fr;gap:2px 10px;align-items:center;border:1px solid rgba(255,255,255,.14);border-radius:16px;background:rgba(255,255,255,.075);padding:12px;color:#fff;text-decoration:none}.new-post-options a svg{grid-row:span 2;color:#f5c76b}.new-post-options a strong{font-size:16px;line-height:1}.new-post-options a small{color:rgba(255,255,255,.70);font-size:12px;line-height:1.25}.new-post-options a:hover{border-color:rgba(245,199,107,.55);background:rgba(245,199,107,.12)}.community-empty-filter{border:1px solid rgba(255,255,255,.12);border-radius:24px;background:rgba(255,255,255,.04);padding:26px;margin-top:18px;color:#fff}.community-empty-filter h3{margin:0 0 8px;font-size:24px}.community-empty-filter p{margin:0;color:rgba(255,255,255,.64);line-height:1.45}@keyframes newPostMini{from{opacity:0;transform:translateY(-6px) scale(.98)}to{opacity:1;transform:translateY(0) scale(1)}}@media(max-width:520px){.new-post-menu[open] .new-post-options{position:fixed;right:16px;top:312px;width:min(78vw,292px);border-radius:20px;background:linear-gradient(145deg,#171720,#07070c);box-shadow:0 24px 90px rgba(0,0,0,.88);border-color:rgba(245,199,107,.42)}.new-post-menu[open] .new-post-options:after{display:none}.new-post-options a{padding:13px;background:rgba(255,255,255,.085)}.new-post-options a strong{font-size:15px}.new-post-options a small{font-size:11px;color:rgba(255,255,255,.74)}}`;
    ...
    <AppShell>
      <main className="community-instagram-page">
        ...
        <section className="community-main-feed">
          ...
          <section className="community-create-strip"><div className="community-tabs"><Link className={mode === 'voce' ? 'active' : ''} href="/aluno/comunidade#feed-comunidade">Para você</Link><Link className={mode === 'recentes' ? 'active' : ''} href="/aluno/comunidade?feed=recentes#feed-comunidade">Recentes</Link><Link className={mode === 'seguindo' ? 'active' : ''} href="/aluno/comunidade?feed=seguindo#feed-comunidade">Seguindo</Link></div><details className="new-post-menu"><summary><Plus size={30} /><span>Nova publicação</span></summary><div className="new-post-options"><a href="#nova-publicacao"><FileText size={22} /><strong>Texto</strong><small>Compartilhe uma ideia, dúvida ou conquista.</small></a><Link href="/aluno/biblioteca"><Video size={22} /><strong>Dueto/atividade</strong><small>Escolha o módulo, grave e publique pelo envio.</small></Link></div></details></section>
          ...
          <section id="feed-comunidade" className="community-social-feed">{feedPosts.length ? <HomeCommunityFeed initialPosts={feedPosts} hasVipAccess={hasVipAccess} vipCheckoutUrl={VIP_CHECKOUT_URL} /> : <div className="community-empty-filter"><h3>Nenhuma publicação em {feedModeLabel(mode)}.</h3><p>{mode === 'seguindo' ? 'Você ainda não segue nenhum aluno. Toque em Seguir em alguma publicação para montar esse feed.' : 'Quando houver novas publicações, elas aparecerão aqui.'}</p></div>}</section>
        </section>
      </main>
    </AppShell>
  );
