  ...
  return (
    <AppShell>
      <main className="ig-profile-page">
        <header className="ig-profile-topbar"><strong>@{handle}</strong><span>Aluno VIP</span></header>
        <section className="ig-profile-header"><Link className="ig-avatar" href="/aluno/perfil/editar" aria-label="Editar foto de perfil">{profileAny?.avatar_url ? <img src={profileAny.avatar_url} alt={name} /> : <span>{initials(name)}</span>}<b>+</b></Link><div className="ig-profile-stats"><Link href="/aluno/comunidade"><strong>{postsCount || 0}</strong><span>publicações</span></Link><Link href="/aluno/perfil/seguidores"><strong>{followersCount || 0}</strong><span>seguidores</span></Link><Link href="/aluno/perfil/seguindo"><strong>{followingCount || 0}</strong><span>seguindo</span></Link></div></section>
        <section className="ig-profile-bio"><h1>{name}</h1><span className="ig-vip-pill">★ Aluno VIP</span>{bio ? <p>{bio}</p> : null}<small>Aluno desde 2026 · Foco em Harmonia</small></section>
        <section className="ig-profile-actions"><Link href="/aluno/perfil/editar">Editar perfil</Link><Link href="/aluno/avaliacoes">Avaliações</Link></section>
        <section className="ig-profile-shortcuts"><Link href="/aluno/avaliacoes"><div><strong>Minhas avaliações</strong><span>{reviewsCount || 0} recebidas · {pendingCount || 0} aguardando</span></div><b>›</b></Link><Link href="/aluno/salvos"><div><strong>Salvos</strong><span>Posts favoritos da comunidade</span></div><b>›</b></Link><Link href="/aluno/comunidade"><div><strong>Minhas publicações</strong><span>Veja e interaja com a comunidade</span></div><b>›</b></Link><Link href="/aluno/biblioteca"><div><strong>Atividades enviadas</strong><span>{submissionsCount || 0} exercícios gravados</span></div><b>›</b></Link><Link href="/aluno/comunidade?tab=seguindo"><div><strong>Seguindo</strong><span>Acompanhe alunos que você segue</span></div><b>›</b></Link></section>
