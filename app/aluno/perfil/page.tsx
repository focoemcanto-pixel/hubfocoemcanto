...
async function safeCount(query: PromiseLike<{ count: number | null }>) {
  const result = await query.catch(() => ({ count: 0 }));
...
export default async function ProfilePage() {
  ...
  const [postsCount, followersCount, followingCount, submissionsCount, reviewsCount, pendingCount, subscriptionsResult, vipProductResult] = await Promise.all([
    ...
    profileId ? admin.from('subscriptions').select('*').eq('profile_id', profileId) : Promise.resolve({ data: [] }),
    admin.from('products').select('*').or('slug.ilike.%vip%,name.ilike.%vip%').limit(1).maybeSingle().catch(() => ({ data: null })),
  ]);
  ...
  const vipProduct = (vipProductResult as any)?.data || null;
