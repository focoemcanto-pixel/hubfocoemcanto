...
async function waitForQueuedRender(supabase: ReturnType<typeof createAdminClient>, jobId: string) {
  for (let attempt = 1; attempt <= 18; attempt += 1) {
    await sleep(3000);
    const { data, error } = await supabase.from('duet_render_jobs').select('id,status,output_url,error_message,created_at').eq('id', jobId).maybeSingle();
...
async function renderWithWorker(params: { video: File; voice: File; referenceUrl: string; voiceVolume: number; referenceVolume: number; referenceOffsetMs: number }) {
...
export async function POST(request: Request) {
  try {
    ...
    if (!lessonSlug || !(video instanceof File) || !(voice instanceof File) || !referenceUrl) return NextResponse.json({ error: 'missing_payload' }, { status: 400 });
    const supabase = createAdminClient();
    const resolved = await resolveExerciseAndProfile(supabase, lessonSlug, email);
    if ('error' in resolved) return resolved.error;
    if (reviewRequested && !resolved.canRequestReview && visibility !== 'community') return NextResponse.json({ error: 'vip_required', detail: 'Avaliação do professor é exclusiva para assinantes VIP.' }, { status: 403 });
    const now = Date.now();
    ...
    const { data: job, error: jobError } = await supabase.from('duet_render_jobs').insert({ profile_id: resolved.profile.id, exercise_id: resolved.exercise.id, status: 'pending', source_video_url: videoUpload.url, source_voice_url: voiceUpload.url, reference_url: referenceUrl, source_video_path: videoUpload.path, source_voice_path: voiceUpload.path, output_path: outputPath, caption, visibility, review_requested: reviewRequested, voice_volume: voiceVolume, reference_volume: referenceVolume, reference_offset_ms: referenceOffsetMs, render_meta: { created_from: 'hub_duet_recorder', mode: 'server_render_queue' } }).select('id,status,created_at').single();
    ...
    try {
      ...
      if (renderedBytes) {
        ...
        const { error: updateError } = await supabase.from('duet_render_jobs').update({ status: 'completed', output_url: output.url, output_path: output.path, completed_at: new Date().toISOString(), render_meta: { mode: 'worker_sync', bytes: renderedBytes.byteLength } }).eq('id', job.id);
    ...
    if (queuedResult?.status === 'completed' && queuedResult.output_url) return NextResponse.json({ ok: true, job_id: job.id, status: 'completed', output_url: queuedResult.output_url, created_at: job.created_at });
    if (queuedResult?.status === 'failed') return NextResponse.json({ ok: true, job_id: job.id, status: 'failed', error_message: queuedResult.error_message || 'Renderização no servidor falhou.', created_at: job.created_at });
