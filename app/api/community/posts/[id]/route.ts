...
function isCommunityAdmin(profile?: { email?: string | null; role?: string | null } | null) {
  const email = String(profile?.email || '').trim().toLowerCase();
  return ADMIN_EMAILS.has(email) || String(profile?.role || '').toLowerCase() === 'admin';
}
...
async function currentProfile() {
  ...
  const { data } = await supabase.from('profiles').select('id,email,name,role').eq('email', email).maybeSingle();
  return data || null;
}
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const canDelete = post.profile_id === profile.id || isCommunityAdmin(profile);
  if (!canDelete) return NextResponse.json({ error: 'not_allowed' }, { status: 403 });
  ...
  const query = supabase.from('community_posts').delete().eq('id', id);
  const { data: removed, error: removeError } = await (isCommunityAdmin(profile) ? query : query.eq('profile_id', profile.id)).select('id').maybeSingle();
