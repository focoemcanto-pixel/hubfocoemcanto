...
async function compressOnce(file: File, profile: CompressionProfile, options: CompressOptions, planIndex: number, planLength: number): Promise<File> {
  ...
  return withObjectUrl(file, async (url) => {
    ...
    video.src = url; video.muted = false; video.volume = 0; video.playsInline = true; video.preload = 'metadata'; video.crossOrigin = 'anonymous';
    ...
    const { targetWidth, targetHeight, videoBitsPerSecond, audioBitsPerSecond } = targetFor(profile, video.videoWidth || 1920, video.videoHeight || 1080, originalDuration);
    debug('metadata', { profile, targetWidth, targetHeight, videoBitsPerSecond, audioBitsPerSecond, originalDuration });
    ...
    debug('blob generated', { profile, size: blob.size, targetWidth, targetHeight });
