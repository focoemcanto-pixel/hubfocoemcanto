...
export class DuetPreviewEngine {
  ...
  async prepare() {
    ...
    this.timelineDuration = Math.max(this.refs.visual.duration || 0, this.voiceBuffer?.duration || 0, this.referenceBuffer?.duration || this.fallbackReference?.duration || 0); this.prepared = true;
  }
  async play() {
  ...
  private async playMediaElementFallback() { const visual = this.refs.visual; const voice = this.fallbackVoice || this.refs.voice; const reference = this.fallbackReference || this.refs.reference; const faders = this.audio.getFaders(); const offsetSeconds = this.referenceOffsetMs / 1000; try { visual.pause(); voice.pause(); reference.pause(); } catch {}; try { visual.currentTime = 0; } catch {}; try { voice.currentTime = 0; } catch {}; try { reference.currentTime = offsetSeconds < 0 ? Math.abs(offsetSeconds) : 0; } catch {}; await Promise.all([waitForSeek(visual), waitForSeek(voice), waitForSeek(reference)]); visual.muted = true; visual.volume = 0; voice.volume = 1; voice.muted = false; reference.volume = 1; reference.muted = false; this.playing = true; this.timelineStartAt = this.audio.context.currentTime; this.voiceStartedAt = this.timelineStartAt; this.referenceStartedAt = offsetSeconds >= 0 ? this.timelineStartAt + offsetSeconds : this.timelineStartAt; this.audio.stopReferenceBuffer(); const playVisual = () => visual.play().catch(() => undefined); const playVoice = () => faders.voice > 0 ? voice.play().catch(() => undefined) : Promise.resolve(); const playReference = () => faders.reference > 0 ? reference.play().catch(() => undefined) : Promise.resolve(); if (offsetSeconds > 0 && faders.reference > 0) { void Promise.all([playVisual(), playVoice()]); this.delayedStartTimer = window.setTimeout(() => { if (!this.playing) return; reference.play().catch(() => undefined); }, Math.round(offsetSeconds * 1000)); return; } void Promise.all([playVisual(), playVoice(), playReference()]); this.startVisualClockGuard(); }
