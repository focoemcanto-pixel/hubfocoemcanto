...
async function blobToAudioBuffer(blob: Blob, sampleRate: number) {
  ...
  try { return await context.decodeAudioData((await blob.arrayBuffer()).slice(0)); } finally { await context.close().catch(() => undefined); }
}
async function urlToAudioBuffer(url: string, sampleRate: number) {
  ...
  return blobToAudioBuffer(await response.blob(), sampleRate);
}
...
function audioBufferToWav(buffer: AudioBuffer) {
  ...
  view.setUint16(offset, 1, true); offset += 2; view.setUint16(offset, numberOfChannels, true); offset += 2; view.setUint32(offset, sampleRate, true); offset += 4; view.setUint32(offset, byteRate, true); offset += 4; view.setUint16(offset, blockAlign, true); offset += 2; view.setUint16(offset, 16, true); offset += 2; writeString('data'); view.setUint32(offset, dataSize, true); offset += 4;
  const channels = Array.from({ length: numberOfChannels }, (_, channel) => buffer.getChannelData(channel));
  for (let i = 0; i < length; i += 1) for (let channel = 0; channel < numberOfChannels; channel += 1) { const sample = Math.max(-1, Math.min(1, channels[channel][i])); view.setInt16(offset, sample < 0 ? sample * 0x8000 : sample * 0x7fff, true); offset += 2; }
...
function makeCanvasVideoStream(visual: HTMLVideoElement) {
  const canvas = document.createElement('canvas'); canvas.width = isSafariLike() ? 960 : 1280; canvas.height = isSafariLike() ? 540 : 720;
...
export class DuetRendererEngine {
  ...
  constructor(options: DuetRendererEngineOptions) { this.options = options; }
  ...
  async renderAudio(): Promise<DuetRenderedAudio> {
    ...
    const context = new OfflineAudioContext({ numberOfChannels: 2, length: Math.ceil(duration * sampleRate), sampleRate });
    const voiceSource = context.createBufferSource(); const voiceGain = context.createGain(); const compressor = context.createDynamicsCompressor(); const limiter = context.createDynamicsCompressor();
    configureCompressor(compressor); configureLimiter(limiter);
    voiceSource.buffer = voiceBuffer; voiceGain.gain.value = linearGain(this.options.faders.voice, this.options.preGains?.voice ?? DEFAULT_VOICE_PRE_GAIN);
    voiceSource.connect(compressor).connect(voiceGain).connect(limiter);
    if (referenceBuffer) { const referenceSource = context.createBufferSource(); const referenceGainNode = context.createGain(); referenceSource.buffer = referenceBuffer; referenceGainNode.gain.value = referenceGainValue; referenceSource.connect(referenceGainNode).connect(limiter); referenceSource.start(delayReferenceSeconds); }
  ...
  async renderVideo(): Promise<DuetRenderedVideo> {
    ...
    const audioDestination = audioContext.createMediaStreamDestination(); const audioSource = audioContext.createBufferSource(); audioSource.buffer = renderedAudio.audioBuffer; audioSource.connect(audioDestination);
    ...
    const mimeType = recorderMimeType(); const recorder = new MediaRecorder(outputStream, { ...(mimeType ? { mimeType } : {}), videoBitsPerSecond: isSafariLike() ? 2500000 : 5200000, audioBitsPerSecond: 256000 });
    const chunks: Blob[] = [];
