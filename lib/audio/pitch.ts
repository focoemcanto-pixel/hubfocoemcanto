...
function normalizeFrequency(frequency: number) {
  let normalized = frequency;
  while (normalized > MAX_VOICE_FREQUENCY && normalized / 2 >= MIN_VOICE_FREQUENCY) normalized /= 2;
  return normalized;
}
...
function mpmPitch(buffer: Float32Array, sampleRate: number) {
  ...
  while (position <= maxLag && nsdf[position] > 0) position += 1;
  while (position <= maxLag) {
    while (position <= maxLag && nsdf[position] <= 0) position += 1;
    if (position > maxLag) break;

    let maxTau = position;
    let maxValue = nsdf[position];
    while (position <= maxLag && nsdf[position] > 0) {
      if (nsdf[position] > maxValue) {
        maxValue = nsdf[position];
        maxTau = position;
      }
      position += 1;
    }
  ...
  const frequency = normalizeFrequency(sampleRate / parabolicPeak(nsdf, selected.tau));
...
function yinPitch(buffer: Float32Array, sampleRate: number) {
  ...
  for (let tau = minTau; tau <= maxTau; tau += 1) {
    if (cmnd[tau] < 0.16) {
      ...
      return frequency >= MIN_VOICE_FREQUENCY && frequency <= MAX_VOICE_FREQUENCY
        ? { frequency, confidence: 1 - cmnd[tau] }
        : null;
    }
  }
  ...
  if (bestTau > 0 && bestValue < 0.34) {
    ...
    return frequency >= MIN_VOICE_FREQUENCY && frequency <= MAX_VOICE_FREQUENCY
      ? { frequency, confidence: 1 - bestValue }
      : null;
  }
...
function normalizedCorrelationPitch(buffer: Float32Array, sampleRate: number) {
  ...
  return frequency >= MIN_VOICE_FREQUENCY && frequency <= MAX_VOICE_FREQUENCY
    ? { frequency, confidence: bestCorrelation }
    : null;
}
...
export function detectPitch(buffer: Float32Array, sampleRate: number): PitchFrame | null {
  ...
  // Gate leve: segura ruído ambiente, mas não apaga notas graves, falsete leve ou ataques curtos.
  if (rms < 0.0012 || peak < 0.01) return null;
...
export function classifyVoice(params: {
  ...
  if (low == null || high == null || low > high) {
    return { classification: 'Indefinida', confidence: 0.55 };
  }

  const span = high - low;
  const coherent = params.lowestMidi == null
    || params.highestMidi == null
    || (low >= params.lowestMidi && high <= params.highestMidi);
