...
export class DuetBufferEngine {
  ...
  async toggle() { if (this.isPlaying) { this.pause(true); return false; } await this.play(); return true; }

  async play() {
    const activeVoiceBuffer = this.getSettings().noiseReduction ? this.cleanVoiceBuffer : this.voiceBuffer;
    if (!this.bus || !activeVoiceBuffer || !this.referenceBuffer || !this.video) return;
    this.stopSources();
    this.applySettings();
  ...
  pause(storeOffset = true, reset = false) {
    ...
    this.stopSyncLoop();
    this.stopSources();
    if (this.video) { this.video.pause(); this.video.playbackRate = 1; if (reset) this.video.currentTime = 0; }
    this.isPlaying = false;
  }

  destroy() { this.pause(false); this.bus?.ctx.close().catch(() => undefined); this.bus = null; this.voiceBuffer = null; this.cleanVoiceBuffer = null; this.referenceBuffer = null; }

  currentTime() { if (!this.playback || !this.bus || !this.isPlaying) return this.offset; return Math.max(0, this.bus.ctx.currentTime - this.playback.startedAt); }
  ...
  private makeBus(ctx: AudioContext, destination: AudioNode): Bus {
    ...
    compressor.connect(dry).connect(limiter); compressor.connect(delay).connect(wet).connect(limiter); limiter.connect(destination); referenceInput.connect(referenceGain).connect(destination);
  ...
  private applyPreset(nodes: Bus, selected: VoicePreset) {
    ...
    const presets: Record<VoicePreset, { highpass: number; bodyGain: number; presenceGain: number; airGain: number; threshold: number; ratio: number; delay: number; wet: number; dry: number; }> = {
