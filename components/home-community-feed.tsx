...
export function HomeCommunityFeed({ initialPosts, hasVipAccess = false, vipCheckoutUrl = DEFAULT_VIP_CHECKOUT }: Props) {
  ...
  async function addComment(id: string, formEl: HTMLFormElement) {
    ...
    if (!response.ok || !data?.ok) {
      ...
      setComments((value) => ({ ...value, [id]: previousComments }));
  ...
  return <>
    <div className="home-insta-feed instagram-mobile-feed">{posts.map((post) => <article className={`home-insta-post instagram-post-card ${removing === post.id ? 'post-removing' : ''}`} id={`post-${post.id}`} key={post.id}>
      ...
      <div className="home-post-media instagram-reel-media">{post.mediaUrl ? <><video ref={(node) => { refs.current[post.id] = node; }} src={post.mediaUrl} muted={!soundOn[post.id]} loop playsInline preload="auto" onLoadedData={(event) => { if (!soundOn[post.id]) event.currentTarget.play().catch(() => undefined); }} /><button className="home-sound-toggle instagram-sound-toggle" type="button" onClick={() => toggleSound(post.id)}>{soundOn[post.id] ? <Volume2 size={20} /> : <VolumeX size={20} />}</button>{post.exerciseTitle ? <div className="instagram-music-chip">♪ {post.exerciseTitle}</div> : null}</> : <div className="home-post-placeholder"><strong>{post.exerciseTitle || 'Publicação da comunidade'}</strong></div>}</div>
      ...
      <div className="instagram-engagement-line"><strong>{post.likesCount}</strong> curtidas</div><p className="home-post-caption instagram-bottom-caption"><strong>{post.authorName}</strong> {post.caption || 'Compartilhou uma prática.'}</p><button type="button" className="instagram-view-comments" onClick={() => setSheet(post.id)}>Ver todos os {post.commentsCount} comentários</button>{(comments[post.id] || []).slice(-2).map((c, i) => <p className="home-local-comments" key={c.id || i}><strong>{c.authorName}</strong> {c.text}</p>)}
    </article>)}</div>
