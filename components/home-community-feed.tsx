...
type FeedPost = { id: string; authorId?: string | null; authorName: string; authorAvatarUrl?: string | null; createdAt?: string | null; exerciseTitle?: string | null; exerciseSlug?: string | null; caption?: string | null; mediaUrl?: string | null; likesCount: number; commentsCount: number; canDelete?: boolean; isFollowing?: boolean };
...
export function HomeCommunityFeed({ initialPosts }: { initialPosts: FeedPost[] }) {
  ...
  if (!posts.length) {
    return <div className="empty-community-feed"><h3>Nenhuma postagem real ainda.</h3><p>Quando os alunos publicarem exercícios na comunidade, eles aparecerão aqui automaticamente.</p><Link className="premium-button gold" href="/aluno/comunidade" prefetch>Criar primeira postagem</Link></div>;
  }
          ...
          return (
            <article className="home-insta-post instagram-post-card" id={`post-${post.id}`} key={post.id}>
              <header className="home-post-head instagram-post-head">
                ...
                <div className="instagram-author-copy"><strong>{post.authorName || 'Aluno VIP'}</strong><span>{timeAgo(post.createdAt)}</span></div>
                ...
                <div className="home-post-options">
                  ...
                  {openMenu === post.id ? <div className="post-options-popover instagram-options-sheet"><Link href={`/aluno/comunidade#post-${post.id}`} prefetch>Ver publicação</Link>{post.exerciseSlug ? <Link href={`/aluno/aula/${post.exerciseSlug}`} prefetch>Ver aula vinculada</Link> : null}{post.exerciseSlug ? <Link href={`/aluno/atividade/${post.exerciseSlug}`} prefetch>Gravar dueto</Link> : null}<button type="button" onClick={() => sharePost(post.id)}>Compartilhar</button><button type="button" onClick={() => navigator.clipboard?.writeText(`${window.location.origin}/aluno/comunidade#post-${post.id}`)}>Copiar link</button>{post.canDelete ? <button className="danger-option" type="button" disabled={removingPost === post.id} onClick={() => removePost(post.id)}>{removingPost === post.id ? 'Excluindo...' : 'Excluir publicação'}</button> : null}</div> : null}
                </div>
              </header>

              <div className="home-post-media instagram-reel-media" onClick={() => toggleVideo(post.id)}>
                {post.mediaUrl ? <><video ref={(node) => { videoRefs.current[post.id] = node; }} data-src={post.mediaUrl} muted={!soundOn[post.id]} loop playsInline preload="none" controls={false} /><button className="home-sound-toggle instagram-sound-toggle" type="button" onClick={(event) => { event.stopPropagation(); toggleSound(post.id); }} aria-label="Ativar som">{soundOn[post.id] ? <Volume2 size={20} /> : <VolumeX size={20} />}</button>{post.exerciseTitle ? <div className="instagram-music-chip">♪ {post.exerciseTitle}</div> : null}</> : <div className="home-post-placeholder"><div><span>▶</span><strong>{post.exerciseTitle || 'Publicação da comunidade'}</strong><small>Vídeo ainda não vinculado a este post</small></div></div>}
              </div>

              <div className="home-icon-actions instagram-action-row"><div className="instagram-action-left"><button type="button" className={liked[post.id] ? 'liked' : ''} onClick={() => likePost(post.id)} aria-label="Curtir"><Heart size={30} fill={liked[post.id] ? 'currentColor' : 'none'} /></button><button type="button" onClick={() => setCommentSheetPostId(post.id)} aria-label="Comentar"><MessageCircle size={30} /></button><button type="button" onClick={() => sharePost(post.id)} aria-label="Enviar"><Send size={30} /></button><button type="button" className={reposted[post.id] ? 'active-action' : ''} onClick={() => setReposted((current) => ({ ...current, [post.id]: !current[post.id] }))} aria-label="Repostar"><Repeat2 size={29} /></button></div><button type="button" className={`save-button ${saved[post.id] ? 'active-action' : ''}`} onClick={() => setSaved((current) => ({ ...current, [post.id]: !current[post.id] }))} aria-label="Salvar"><Bookmark size={31} fill={saved[post.id] ? 'currentColor' : 'none'} /></button></div>
              ...
              {post.exerciseSlug ? <div className="home-lesson-actions instagram-lesson-actions"><Link href={`/aluno/aula/${post.exerciseSlug}`} prefetch>Ver aula</Link><Link className="primary" href={`/aluno/atividade/${post.exerciseSlug}`} prefetch>Gravar dueto</Link></div> : null}
