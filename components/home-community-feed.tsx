...
export function HomeCommunityFeed({ initialPosts }: { initialPosts: FeedPost[] }) {
  ...
  async function followAuthor(authorId?: string | null) { if (!authorId) return; const previousFollow = following[authorId]; const next = !previousFollow; setFollowing((v) => ({ ...v, [authorId]: next })); const r = await postForm('/api/community/follows', { following_id: authorId, following: String(next) }); const data = await r.json().catch(() => null); if (!r.ok || !data?.ok) { setFollowing((v) => ({ ...v, [authorId]: !!previousFollow })); notice(data?.detail || 'Não foi possível seguir. Execute o SQL de follows.'); return; } if (data.self) { setFollowing((v) => ({ ...v, [authorId]: false })); notice('Esse é o seu próprio perfil.'); return; } notice(data.following ? 'Agora você está seguindo.' : 'Você deixou de seguir.'); }
