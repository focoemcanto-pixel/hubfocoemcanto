    ...
    <div className="community-comment-sheet-backdrop" onClick={onClose} role="presentation">
      <section
        ...
        onTouchMove={(event) => {
          ...
          setDragY(Math.max(0, event.touches[0].clientY - dragStart));
        }}
        onTouchEnd={() => {
...
export function HomeCommunityFeed({ initialPosts, hasVipAccess = false, vipCheckoutUrl = DEFAULT_VIP_CHECKOUT, currentProfileId = null }: Props) {
  ...
  function closeComments() { setCommentPost(null); setComments([]); setCommentText(''); setCommentsLoading(false); setCommentSending(false); }
  ...
  async function likePost(id: string) { const next = !liked[id]; setLiked((v) => ({ ...v, [id]: next })); setPosts((list) => list.map((p) => p.id === id ? { ...p, likesCount: Math.max(0, p.likesCount + (next ? 1 : -1)) } : p)); const r = await postForm('/api/community/likes', { post_id: id, liked: String(next) }); if (!r.ok) notice('Não foi possível curtir.'); }
  async function savePost(id: string) { const next = !saved[id]; setSaved((v) => ({ ...v, [id]: next })); notice(next ? 'Salvo nos favoritos.' : 'Removido dos favoritos.'); const r = await postForm('/api/community/saves', { post_id: id, saved: String(next) }); if (!r.ok) notice('Não foi possível salvar.'); }
  async function followAuthor(authorId?: string | null) { if (!authorId || authorId === effectiveProfileId) return; const next = !following[authorId]; setFollowing((value) => ({ ...value, [authorId]: next })); const r = await postForm('/api/community/follows', { following_id: authorId, following: String(next) }); const data = await r.json().catch(() => null); if (!r.ok || data?.error) { setFollowing((value) => ({ ...value, [authorId]: !next })); notice(data?.detail || 'Não foi possível seguir.'); return; } notice(next ? 'Agora você está seguindo.' : 'Você deixou de seguir.'); }
  async function openComments(post: FeedPost) { setCommentPost(post); setComments([]); setCommentText(''); setCommentsLoading(true); const response = await fetch(`/api/community/comments?post_id=${encodeURIComponent(post.id)}`, { headers: { accept: 'application/json' } }); const data = await response.json().catch(() => null); if (response.ok && data?.comments) setComments(data.comments); else notice('Não foi possível carregar comentários.'); setCommentsLoading(false); }
  async function submitComment(event: FormEvent) { event.preventDefault(); if (!commentPost || !commentText.trim() || commentSending) return; setCommentSending(true); const r = await postForm('/api/community/comments', { post_id: commentPost.id, comment: commentText.trim() }); const data = await r.json().catch(() => null); if (!r.ok || !data?.ok) { notice(data?.detail || 'Não foi possível comentar.'); setCommentSending(false); return; } setComments((list) => [...list, data.comment]); setPosts((list) => list.map((p) => p.id === commentPost.id ? { ...p, commentsCount: data.comments_count ?? p.commentsCount + 1 } : p)); setCommentPost((post) => post ? { ...post, commentsCount: data.comments_count ?? post.commentsCount + 1 } : post); setCommentText(''); setCommentSending(false); }
  ...
  function toggleSound(id: string) { setSoundOn((v) => ({ ...v, [id]: !v[id] })); }
