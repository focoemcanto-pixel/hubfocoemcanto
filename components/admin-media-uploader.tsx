...
type QueueStatus = 'queued' | 'uploading' | 'linked' | 'done' | 'error';
type Item = { id: string; file: File; name: string; relativePath: string; size: number; status: QueueStatus; progress: number; attempts: number; uid?: string; error?: string; url?: string };
...
type StreamStatus = { state?: string; received?: boolean; ready?: boolean; message?: string; error?: string; duration?: number | null };
...
const RETRY_LIMIT = 5;
const DIRECT_STREAM_LIMIT = 190 * 1024 * 1024;
...
async function waitForReady(uid: string, originalDuration: number, update: (message: string, progress: number) => void) {
  for (let i = 0; i < 100; i += 1) {
    ...
    if (json.ready || json.received) {
      if (duration > 0 && duration < originalDuration * 0.95) {
        throw new Error(`Cloudflare recebeu vídeo incompleto: ${Math.round(duration)}s de ${Math.round(originalDuration)}s. Exclua esse UID no Stream e reenvie um arquivo abaixo de ${formatBytes(DIRECT_STREAM_LIMIT)}.`);
      }
      update(`Cloudflare confirmou vídeo pronto. Duração: ${Math.round(duration)}s.`, 92);
      return;
    }
    update(`Aguardando processamento completo no Cloudflare... (${state})`, Math.min(90, 55 + Math.round(i * 0.35)));
  }
  throw new Error('Cloudflare não confirmou o vídeo como pronto. O Hub não vinculou para evitar vídeo incompleto.');
}

async function xhrPut(url: string, file: File, onProgress: (value: number) => void) { await new Promise<void>((resolve, reject) => { const xhr = new XMLHttpRequest(); xhr.open('PUT', url); if (file.type) xhr.setRequestHeader('Content-Type', file.type); xhr.upload.onprogress = (event) => event.lengthComputable && onProgress(Math.round((event.loaded / event.total) * 100)); xhr.onload = () => xhr.status >= 200 && xhr.status < 300 ? resolve() : reject(new Error((xhr.responseText || '').trim() || `Upload falhou (${xhr.status}).`)); xhr.onerror = () => reject(new Error('Upload interrompido pela conexão.')); xhr.send(file); }); }
...
export function AdminMediaUploader({ productId, productName, modules = [], migrationOnly = false, totalLessons = 0, migratedLessons = 0, driveLessons = 0 }: Props = {}) {
  ...
  async function mapFolder(files: FileList | null) {
    ...
    try {
      ...
      setStreamItems(pendingFiles.map(toItem));
      setFolderMap(json);
  ...
  async function uploadStreamItem(item: Item) {
    ...
    while (attempts < RETRY_LIMIT) {
      try {
        ...
        const originalDuration = await readLocalDuration(uploadFile).catch(() => 0);
        if (!originalDuration || originalDuration <= 0) { patchStream(item.id, { status: 'error', progress: 0, attempts, error: 'Não foi possível validar a duração local deste vídeo. O Hub bloqueou o envio para evitar arquivo cortado.' }); return; }
        const fileHash = await sha256File(uploadFile).catch(() => '');
        if (uploadFile.size > DIRECT_STREAM_LIMIT) { patchStream(item.id, { status: 'error', progress: 0, attempts, error: `Upload direto bloqueado por segurança. Este arquivo tem ${formatBytes(uploadFile.size)} e o limite seguro é ${formatBytes(DIRECT_STREAM_LIMIT)}. Envie pelo painel Cloudflare Stream ou comprima externamente para abaixo desse limite.` }); return; }
        patchStream(item.id, { status: 'uploading', progress: 40, attempts, error: 'Enviando arquivo original sem recompressão...' });
        const create = await fetch('/api/admin/media/stream-upload-url', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileName: item.name, contentType: uploadFile.type || item.file.type || 'video/mp4', productId, moduleId, relativePath: item.relativePath, size: uploadFile.size, originalSize: item.file.size, fileHash, compressionProfile: 'none', forceNewUpload: true }) });
        ...
        patchStream(item.id, { status: done.linked ? 'linked' : 'done', progress: 100, attempts, uid, error: done.createdExercise ? 'Aula criada automaticamente.' : 'Arquivo original vinculado.' });
        return;
