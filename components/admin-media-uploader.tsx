...
const RETRY_LIMIT = 5;
const DIRECT_STREAM_LIMIT = 320 * 1024 * 1024;
...
async function waitForReady(uid: string, originalDuration: number, update: (message: string, progress: number) => void) {
  for (let i = 0; i < 100; i += 1) {
    ...
    if (json.ready || json.received) {
      if (originalDuration > 20 && duration > 0 && duration < originalDuration * 0.95) {
        throw new Error(`Cloudflare recebeu vídeo incompleto: ${Math.round(duration)}s de ${Math.round(originalDuration)}s. Exclua esse UID no Stream e reenvie o arquivo original.`);
      }
  ...
  async function uploadStreamItem(item: Item) {
    ...
    while (attempts < RETRY_LIMIT) {
      try {
        ...
        if (uploadFile.size > DIRECT_STREAM_LIMIT) { patchStream(item.id, { status: 'error', progress: 0, attempts, error: `Compressão local desativada por segurança. Este arquivo tem ${formatBytes(uploadFile.size)} e precisa ser enviado pelo painel Cloudflare Stream ou por um compressor externo confiável.` }); return; }
