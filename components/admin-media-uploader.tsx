...
export function AdminMediaUploader({ productId, productName, modules = [], migrationOnly = false, totalLessons = 0, migratedLessons = 0, driveLessons = 0 }: Props = {}) {
  ...
  return <>
    <section className="card admin-section media-migration-card">
      ...
      <div className="admin-help-box"><strong>Destino desta importação</strong><p className="muted">Stream não faz upload pelo navegador nesta etapa. Ele busca vídeos já enviados no painel Cloudflare e vincula ao módulo. R2 faz upload de arquivos auxiliares.</p><div className="media-migration-toolbar"><button className={`admin-clean-button ${uploadDestination === 'stream' ? 'primary' : 'secondary'}`} type="button" onClick={() => setUploadDestination('stream')}><Video size={16} /> Importar do Stream</button><button className={`admin-clean-button ${uploadDestination === 'r2' ? 'primary' : 'secondary'}`} type="button" onClick={() => setUploadDestination('r2')}><UploadCloud size={16} /> Upload para R2</button></div></div>
    </section>

    {uploadDestination === 'stream' ? <section className="card admin-section media-migration-card">
      <div className="section-heading"><div><p className="eyebrow">Cloudflare Stream</p><h2>Importar vídeos das aulas</h2><p className="muted">Suba os vídeos pelo painel Cloudflare Stream. Depois clique aqui para vincular automaticamente às aulas do módulo escolhido.</p></div><span className={`admin-clean-pill ${syncError ? 'danger' : syncResult ? 'success' : 'warning'}`}>{syncing ? 'Importando...' : syncResult ? 'Importado' : 'Pronto'}</span></div>
      ...
      <div className="admin-help-box"><strong><Video size={16} /> Como usar</strong><p className="muted">Envie seus vídeos no painel do Cloudflare Stream. Mantenha nomes parecidos com as aulas, escolha o módulo aqui e sincronize. Isso evita os resets de conexão do upload pelo navegador.</p></div>
    ...
    {uploadDestination === 'r2' ? <section className="card admin-section media-migration-card">
      ...
      <div className="media-migration-toolbar"><input ref={fileInputRef} type="file" multiple style={{ display: 'none' }} onChange={(event) => addR2Files(event.target.files)} /><input ref={folderInputRef} type="file" multiple {...({ webkitdirectory: '', directory: '' } as Record<string, string>)} style={{ display: 'none' }} onChange={(event) => addR2Files(event.target.files)} /><button className="admin-clean-button secondary" type="button" onClick={() => fileInputRef.current?.click()}>Importar arquivo</button><button className="admin-clean-button secondary" type="button" onClick={() => folderInputRef.current?.click()}><FolderUp size={16} /> Importar pasta</button><button className="admin-clean-button primary" type="button" onClick={() => startR2Upload(false)} disabled={r2Running || !destinationModuleId || !r2Items.some((item) => item.status === 'queued' || item.status === 'error')}>{r2Running ? <><Loader2 size={16} className="premium-video-spinner" /> Enviando {r2Stats.overall}%</> : <><UploadCloud size={16} /> Enviar para R2</>}</button><button className="admin-clean-button secondary" type="button" onClick={() => setR2Items([])} disabled={r2Running}>Limpar fila</button></div>
