...
type Item = { id: string; file: File; name: string; relativePath: string; size: number; status: QueueStatus; progress: number; attempts: number; uid?: string; error?: string; compressedSize?: number; appliedCompressionProfile?: CompressionProfile; url?: string };
...
type SyncExercise = { id: string; title?: string | null; module_id?: string | null };
type SyncUnmatched = { uid: string; name: string; status: string; score: number };
type SyncResult = { total?: number; linked?: number; unmatchedCount?: number; errorsCount?: number; unmatched?: SyncUnmatched[]; exercises?: SyncExercise[]; message?: string; error?: string };

const RETRY_LIMIT = 5;
const DIRECT_STREAM_LIMIT = 180 * 1024 * 1024;
const AGGRESSIVE_COMPRESSION_LIMIT = 180 * 1024 * 1024;
const ULTRA_COMPRESSION_LIMIT = 350 * 1024 * 1024;
...
function automaticProfile(fileSize: number, selected: CompressionProfile) { if (fileSize > ULTRA_COMPRESSION_LIMIT) return 'ultra' as const; if (fileSize > AGGRESSIVE_COMPRESSION_LIMIT && selected !== 'ultra') return 'aggressive' as const; return selected; }
...
export function AdminMediaUploader({ productId, productName, modules = [], migrationOnly = false, totalLessons = 0, migratedLessons = 0, driveLessons = 0 }: Props = {}) {
  ...
  async function uploadStreamItem(item: Item) {
    ...
    while (attempts < RETRY_LIMIT) {
      try {
        attempts += 1;
        const effectiveProfile = automaticProfile(item.file.size, compressionProfile);
        const forcedAggressive = effectiveProfile !== compressionProfile && (effectiveProfile === 'aggressive' || effectiveProfile === 'ultra');
        patchStream(item.id, { status: 'compressing', progress: 1, attempts, appliedCompressionProfile: effectiveProfile, error: forcedAggressive ? 'Compressão agressiva aplicada por tamanho do arquivo.' : 'Preparando vídeo...' });
        const fileHash = await sha256File(item.file).catch(() => '');
        const uploadFile = await compressVideoForUpload(item.file, { enabled: compressionEnabled || forcedAggressive, profile: effectiveProfile, onProgress: (progress, label) => patchStream(item.id, { status: 'compressing', progress: Math.min(35, Math.max(1, Math.round(progress * 0.35))), attempts, error: `${forcedAggressive ? 'Compressão agressiva aplicada por tamanho do arquivo. ' : ''}${label || ''}`.trim() }) });
        const compressed = uploadFile.size < item.file.size;
        const sizeSummary = `${formatBytes(item.file.size)} → ${formatBytes(uploadFile.size)}`;
        if (uploadFile.size > DIRECT_STREAM_LIMIT) {
          patchStream(item.id, { status: 'error', progress: 0, attempts, compressedSize: compressed ? uploadFile.size : undefined, appliedCompressionProfile: effectiveProfile, error: `Mesmo após compressão, este vídeo ficou grande demais para envio direto pelo Hub. Envie pelo painel Cloudflare Stream e depois clique em Buscar vídeos do Cloudflare. Tamanho original: ${formatBytes(item.file.size)}. Tamanho comprimido: ${formatBytes(uploadFile.size)}.` });
          return;
        }
        patchStream(item.id, { status: 'uploading', progress: 40, attempts, compressedSize: compressed ? uploadFile.size : undefined, appliedCompressionProfile: effectiveProfile, error: compressed ? `${forcedAggressive ? 'Compressão agressiva aplicada por tamanho do arquivo. ' : ''}Comprimido: ${sizeSummary}` : 'Criando URL segura...' });
        const create = await fetch('/api/admin/media/stream-upload-url', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileName: item.name, contentType: uploadFile.type || item.file.type || 'video/mp4', productId, moduleId, relativePath: item.relativePath, size: uploadFile.size, originalSize: item.file.size, fileHash, compressionProfile: effectiveProfile }) });
...
function QueueList({ items }: { items: Item[] }) { if (!items.length) return null; return <div className="admin-list media-migration-results">{items.slice(0, 160).map((item) => <div className="admin-row" key={item.id}><div><span className={`admin-clean-pill ${item.status === 'linked' || item.status === 'done' ? 'success' : item.status === 'error' ? 'danger' : 'warning'}`}>{item.status === 'linked' ? <><Check size={14} /> Vinculado</> : item.status === 'done' ? <><Check size={14} /> Salvo</> : item.status === 'error' ? <><XCircle size={14} /> Falhou</> : item.status === 'compressing' ? 'Comprimindo' : item.status === 'uploading' ? 'Enviando' : 'Na fila'}</span><h3>{item.name}</h3><p className="muted">{item.relativePath} · {formatBytes(item.size)}{item.compressedSize ? ` → ${formatBytes(item.compressedSize)}` : ''} · tentativas {item.attempts}/{RETRY_LIMIT}{item.uid ? ` · UID ${item.uid}` : ''}{item.appliedCompressionProfile === 'aggressive' || item.appliedCompressionProfile === 'ultra' ? ` · Perfil ${item.appliedCompressionProfile === 'ultra' ? 'ultra agressivo' : 'agressivo'}` : ''}{item.compressedSize ? ` · Economia ${formatBytes(Math.max(0, item.size - item.compressedSize))}` : ''}{item.error ? ` · ${item.error}` : ''}</p></div><strong>{item.progress}%</strong></div>)}</div>; }
