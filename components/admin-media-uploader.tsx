...
export function AdminMediaUploader({ productId, productName, modules = [], migrationOnly = false, totalLessons = 0, migratedLessons = 0, driveLessons = 0 }: Props = {}) {
  ...
  return <>
    <section className="card admin-section media-migration-card">
      <div className="section-heading"><div><p className="eyebrow">Destino da biblioteca</p><h2>{productName || 'Produto'}</h2><p className="muted">Escolha o módulo antes de enviar vídeos para o Stream, importar vídeos já existentes ou subir materiais auxiliares ao R2.</p></div><span className="admin-clean-pill warning">{selectedModule?.title || 'Selecione um módulo'}</span></div>
    ...
    <section className="card admin-section media-migration-card">
      <div className="section-heading"><div><p className="eyebrow">Cloudflare Stream</p><h2>Upload das aulas</h2><p className="muted">Use para vídeos principais do curso. O Hub envia para o Stream e tenta vincular automaticamente pelo nome da aula no módulo escolhido.</p></div><span className={`admin-clean-pill ${streamStats.failed ? 'danger' : streamStats.done ? 'success' : streamRunning ? 'success' : 'warning'}`}>{streamRunning ? 'Enviando...' : streamStats.done ? 'Uploads salvos' : 'Pronto'}</span></div>
    ...
    <section className="card admin-section media-migration-card">
      <div className="section-heading"><div><p className="eyebrow">Sincronização Stream</p><h2>Importar vídeos já enviados</h2><p className="muted">Use quando os vídeos já foram enviados direto no painel Cloudflare Stream. O Hub busca no Stream e vincula ao módulo selecionado.</p></div><span className={`admin-clean-pill ${syncError ? 'danger' : syncResult ? 'success' : 'warning'}`}>{syncing ? 'Importando...' : syncResult ? 'Importado' : 'Pronto'}</span></div>
