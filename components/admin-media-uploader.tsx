...
function mediaFolder(file: File) {
  ...
  if (file.type.startsWith('video/')) return 'videos/auxiliares';
  return 'files';
}
...
async function xhrPut(url: string, file: File, onProgress: (n: number) => void) {
  await new Promise<void>((resolve, reject) => {
    ...
    xhr.onerror = () => reject(new Error('Upload interrompido.'));
    xhr.send(file);
  });
}
...
export function AdminMediaUploader({ productId, productName, modules = [], migrationOnly = false, totalLessons = 0, migratedLessons = 0, driveLessons = 0 }: Props = {}) {
  ...
  const r2Stats = useMemo(() => {
    ...
    const uploading = r2Items.filter((item) => item.status === 'uploading').length;
    const overall = total ? Math.round(r2Items.reduce((sum, item) => sum + item.progress, 0) / total) : 0;
    return { total, done, failed, uploading, overall };
  }, [r2Items]);

  function setR2Item(id: string, patch: Partial<R2Item>) {
    setR2Items((current) => current.map((item) => item.id === id ? { ...item, ...patch } : item));
  }
  ...
  async function uploadOneR2(item: R2Item) {
    try {
      ...
      const response = await fetch('/api/admin/media/signed-upload', {
        ...
        body: JSON.stringify({ fileName: item.name, contentType: item.file.type || 'application/octet-stream', folder: mediaFolder(item.file), relativePath: item.relativePath, productId }),
      });
      ...
      setR2Item(item.id, { status: 'error', error: err instanceof Error ? err.message : 'Erro desconhecido ao enviar.', progress: 0 });
    }
  }
  ...
  return <>
    <section className="card admin-section media-migration-card">
      <div className="section-heading">
        <div><p className="eyebrow">Cloudflare Stream</p><h2>{productName ? `Biblioteca Stream · ${productName}` : 'Biblioteca Stream'}</h2><p className="muted">Suba os vídeos no painel Cloudflare Stream. Depois o Hub encontra pelo nome e vincula às aulas.</p></div>
        <span className={`admin-clean-pill ${syncError ? 'danger' : syncResult ? 'success' : 'warning'}`}>{syncing ? 'Sincronizando...' : syncResult ? 'Sincronizado' : 'Pronto'}</span>
      </div>
      <div className="admin-grid admin-section">
        <article className="admin-stat"><span>Aulas</span><strong>{totalLessons}</strong><p className="muted">Conteúdos do produto.</p></article>
        <article className="admin-stat"><span>Drive atual</span><strong>{driveLessons}</strong><p className="muted">Aguardam Stream.</p></article>
        <article className="admin-stat"><span>Otimizadas</span><strong>{migratedLessons}</strong><p className="muted">Já têm origem interna.</p></article>
      </div>
    ...
    <section className="card admin-section media-migration-card">
      <div className="section-heading">
        <div><p className="eyebrow">Cloudflare R2</p><h2>Upload de áudios, imagens e arquivos</h2><p className="muted">Selecione arquivos ou uma pasta. Use R2 para áudios de treino, capas, PDFs e materiais auxiliares.</p></div>
        <span className={`admin-clean-pill ${r2Stats.failed ? 'danger' : r2Stats.done ? 'success' : r2Running ? 'success' : 'warning'}`}>{r2Running ? 'Enviando...' : r2Stats.done ? 'Uploads salvos' : 'Pronto'}</span>
      </div>
      <div className="admin-grid admin-section">
        <article className="admin-stat"><span>Na fila</span><strong>{r2Stats.total}</strong><p className="muted">Arquivos selecionados.</p></article>
        <article className="admin-stat"><span>Enviados</span><strong>{r2Stats.done}</strong><p className="muted">Salvos no R2.</p></article>
        <article className="admin-stat"><span>Falhas</span><strong>{r2Stats.failed}</strong><p className="muted">Podem ser reenviadas.</p></article>
      </div>
      ...
      <div className="media-migration-toolbar">
        <input ref={fileInputRef} type="file" multiple style={{ display: 'none' }} onChange={(event) => addR2Files(event.target.files)} />
        <input ref={folderInputRef} type="file" multiple {...({ webkitdirectory: '', directory: '' } as Record<string, string>)} style={{ display: 'none' }} onChange={(event) => addR2Files(event.target.files)} />
        <button className="admin-clean-button secondary" type="button" onClick={() => fileInputRef.current?.click()} disabled={r2Running}>Selecionar arquivos</button>
        <button className="admin-clean-button secondary" type="button" onClick={() => folderInputRef.current?.click()} disabled={r2Running}><FolderUp size={16} /> Selecionar pasta</button>
        <button className="admin-clean-button primary" type="button" onClick={() => startR2Upload(false)} disabled={r2Running || !r2Items.some((item) => item.status === 'queued' || item.status === 'error')}>{r2Running ? <><Loader2 size={16} className="premium-video-spinner" /> Enviando {r2Stats.overall}%</> : <><UploadCloud size={16} /> Enviar para R2</>}</button>
        <button className="admin-clean-button secondary" type="button" onClick={() => setR2Items([])} disabled={r2Running}>Limpar fila</button>
      </div>
