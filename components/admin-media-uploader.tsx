...
type MigrationResult = { id: string; title?: string | null; status: string; reason?: string; detail?: string; mediaUrl?: string; folder?: string; moduleTitle?: string | null };
...
function slug(value?: string | null) {
  return String(value || 'produto').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'produto';
}
...
function pillClass(status: string) {
  if (status === 'r2' || status === 'migrated') return 'admin-clean-pill success';
  if (status === 'drive' || status === 'skipped') return 'admin-clean-pill warning';
...
export function AdminMediaUploader({ productId, productName, migrationOnly = false, totalLessons = 0, migratedLessons = 0, driveLessons = 0 }: AdminMediaUploaderProps = {}) {
  ...
  async function refreshMigrationStatus() {
    ...
    try {
      ...
      setMigrationError(err instanceof Error ? err.message : 'Erro ao carregar biblioteca.');
    } finally {
  ...
  function toggleModule(module: ModuleSummary) {
    const ids = module.lessons.filter((lesson) => lesson.status === 'drive').map((lesson) => lesson.id);
    setSelected((current) => {
      const next = new Set(current);
  ...
  function selectAllPending() {
    setSelected(new Set(pendingLessons.map((lesson) => lesson.id)));
  }
  ...
  async function migrateOne(exerciseId: string) {
    const response = await fetch('/api/admin/media/migrate-drive', {
      ...
      body: JSON.stringify({ exerciseId, productId, limit: 1, force: true }),
    });
  ...
  async function migrateSelected() {
    if (!selectedPending.length) {
      setMigrationError('Selecione pelo menos uma aula pendente no Drive.');
  ...
  const migrationCard = (
    <section className="card admin-section media-migration-card">
      <div className="section-heading">
        <div>
          <p className="eyebrow">Central de migração</p>
          <h2>{productName ? `Drive → R2 · ${productName}` : 'Drive → R2'}</h2>
          <p className="muted">Selecione aulas, módulos ou o produto inteiro. O Hub baixa do Drive, envia ao R2 e mantém os cortes salvos.</p>
        </div>
        <span className="admin-clean-pill">{statusLoading ? 'Atualizando...' : `${percent}% no R2`}</span>
      </div>

      <div className="admin-grid admin-section">
        <article className="admin-stat"><span>Total</span><strong>{library.total || '—'}</strong><p className="muted">Aulas vinculadas.</p></article>
        <article className="admin-stat"><span>No R2</span><strong>{library.migrated}</strong><p className="muted">Arquivos reais no domínio R2.</p></article>
        <article className="admin-stat"><span>Selecionadas</span><strong>{selectedPending.length}</strong><p className="muted">Prontas para migrar.</p></article>
      </div>

      <div className="progress media-migration-progress"><span style={{ width: `${percent}%` }} /></div>

      <div className="media-migration-toolbar">
        <button className="admin-clean-button primary" type="button" onClick={migrateSelected} disabled={migrationStatus === 'running' || selectedPending.length === 0}>{migrationStatus === 'running' ? 'Migrando selecionadas...' : `Migrar selecionadas (${selectedPending.length})`}</button>
        ...
        <button className="admin-clean-button secondary" type="button" onClick={() => setSelected(new Set())}>Limpar seleção</button>
        <button className="admin-clean-button secondary" type="button" onClick={refreshMigrationStatus} disabled={statusLoading}>Atualizar mapa</button>
      </div>

      <div className="media-migration-filters">
        ...
        <button type="button" className={filter === 'r2' ? 'active' : ''} onClick={() => setFilter('r2')}>No R2</button>
      </div>

      <div className="admin-help-box">
        <strong>Destino organizado</strong>
        <p className="muted">Cada arquivo vai para a pasta do produto e do módulo correto.</p>
        <code>{destinationPreview}</code>
      </div>

      {migrationStatus === 'running' ? <p className="admin-save-success">Migração em andamento. Não feche esta tela.</p> : null}
      {migrationStatus === 'done' ? <p className="admin-save-success">Migração concluída. O mapa foi atualizado.</p> : null}
      {migrationError ? <p className="admin-save-error">{migrationError}</p> : null}

      <div className="media-library-map">
        {library.modules.map((module) => {
          const pendingIds = module.lessons.filter((lesson) => lesson.status === 'drive').map((lesson) => lesson.id);
          const moduleSelected = pendingIds.length > 0 && pendingIds.every((id) => selected.has(id));
          const visibleLessons = module.lessons.filter((lesson) => filter === 'all' ? true : filter === 'pending' ? lesson.status === 'drive' : lesson.status === 'r2');
          if (!visibleLessons.length && filter !== 'all') return null;
          const modulePercent = module.total > 0 ? Math.round((module.migrated / module.total) * 100) : 0;
          return (
            <article className="media-module-card" key={module.id}>
              <header>
                <div>
                  <span className={module.pending === 0 ? 'admin-clean-pill success' : module.migrated ? 'admin-clean-pill warning' : 'admin-clean-pill danger'}>{module.pending === 0 ? 'Concluído' : module.migrated ? 'Parcial' : 'Pendente'}</span>
                  <h3>{module.title}</h3>
                  <p>{module.migrated}/{module.total} no R2 · {module.pending} pendentes · {modulePercent}%</p>
                </div>
                <button type="button" className="admin-clean-button secondary" onClick={() => toggleModule(module)} disabled={!pendingIds.length}>{moduleSelected ? 'Remover módulo' : `Selecionar módulo (${pendingIds.length})`}</button>
              </header>
              <div className="media-lesson-list">
                {visibleLessons.map((lesson) => (
                  <label className={`media-lesson-row ${lesson.status}`} key={lesson.id}>
                    <input type="checkbox" checked={selected.has(lesson.id)} disabled={lesson.status !== 'drive'} onChange={() => toggleLesson(lesson)} />
                    <span className={pillClass(lesson.status)}>{statusLabel(lesson.status)}</span>
                    <strong>{lesson.title || lesson.slug || 'Aula sem título'}</strong>
                    <small>{lesson.status === 'r2' ? 'Já está no Cloudflare R2' : lesson.status === 'drive' ? 'Pronta para migrar do Drive' : 'Sem link de mídia'}</small>
                  </label>
                ))}
              </div>
            </article>
        ...
        {!library.modules.length ? <div className="admin-help-box"><strong>Nenhum módulo encontrado</strong><p className="muted">O produto ainda não retornou módulos vinculados para migração.</p></div> : null}
      </div>
        ...
        <div className="admin-list media-migration-results">
          {migrationResults.map((item, index) => (
            <div className="admin-row" key={`${item.id}-${index}`}>
              <div>
                <span className={pillClass(item.status)}>{resultLabel(item.status)}</span>
  ...
  return (
    <>
      <section className="card admin-section">
        <div className="section-heading"><div><p className="eyebrow">Cloudflare R2</p><h2>Upload de mídia</h2><p className="muted">Envie vídeos, áudios e imagens para o bucket configurado nas variáveis do projeto.</p></div></div>
        <div className="admin-form-grid">
          <label>Arquivo<input type="file" accept="video/*,audio/*,image/*,application/pdf" onChange={(event) => setFile(event.target.files?.[0] || null)} /></label>
          ...
          <button className="button" type="button" onClick={upload} disabled={!file || status === 'signing' || status === 'uploading'}>{status === 'signing' ? 'Preparando...' : status === 'uploading' ? `Enviando ${progress}%` : 'Enviar para R2'}</button>
        </div>
        {status === 'uploading' ? <div className="progress"><span style={{ width: `${progress}%` }} /></div> : null}
        {error ? <p className="error-text">{error}</p> : null}
        {result ? <div className="admin-result-box"><p className="eyebrow">Upload concluído</p><strong>URL pública</strong><code>{result.publicUrl}</code><button className="button secondary" type="button" onClick={() => navigator.clipboard?.writeText(result.publicUrl)}>Copiar URL</button></div> : null}
      </section>
      {migrationCard}
    </>
  );
