...
async function xhrPostStreamServer(item: StreamItem, productId: string, moduleId: string, onProgress: (n: number) => void) {
  return await new Promise<{ uid: string; linked?: boolean }>((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    const form = new FormData();
    form.append('file', item.file, item.name);
    form.append('fileName', item.name);
    form.append('contentType', item.file.type || 'video/mp4');
    form.append('relativePath', item.relativePath);
    form.append('productId', productId);
    form.append('moduleId', moduleId);
    form.append('size', String(item.size));
    xhr.open('POST', '/api/admin/media/stream-server-upload');
    xhr.upload.onprogress = (event) => event.lengthComputable && onProgress(Math.min(95, Math.round((event.loaded / event.total) * 90)));
    xhr.onload = () => {
      const json = JSON.parse(xhr.responseText || '{}');
      if (xhr.status >= 200 && xhr.status < 300) resolve(json);
      else reject(new Error(json?.message || `Upload falhou (${xhr.status}).`));
    };
  ...
  async function uploadOneStream(item: StreamItem) {
    ...
    while (attempt < RETRY_LIMIT) {
      try {
        ...
        const completedJson = await xhrPostStreamServer(item, productId, destinationModuleId, (progress) => setStreamItem(item.id, { progress, attempts: attempt }));
        setStreamItem(item.id, { status: completedJson?.linked ? 'linked' : 'done', progress: 100, attempts: attempt, uid: completedJson.uid, url: `https://iframe.videodelivery.net/${completedJson.uid}`, error: '' });
        return;
      } catch (err) {
