...
function blobStatus(label: string, blob?: Blob | null, chunks?: number) {
  ...
  return <li style={{ display: 'flex', justifyContent: 'space-between', gap: 12, padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,.08)' }}><span>{ok ? '✅' : '⚠️'} {label}</span><strong>{ok ? `${Math.round((blob?.size || 0) / 1024)} KB` : 'vazio'}{typeof chunks === 'number' ? ` · ${chunks} chunks` : ''}</strong></li>;
}
...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  async function buildUploadBlob() {
    ...
    try {
      const rendered = await renderFinalDuetVideo({
        ...
        referenceBlob: null,
        referenceSource,
        settings: { voiceVolume: 100, referenceVolume: 100, preset: 'natural', latencyMs: 0, noiseReduction: false },
      });
      if (rendered?.size > 1000) return rendered;
    } catch {}
    setRenderStatus('Não consegui renderizar com a referência original. Enviando o bruto como fallback...');
  ...
  async function submit() {
    ...
    try {
      ...
      setRenderStatus(''); setStep('posted');
    } catch (err: any) {
      setError(err?.message || 'Não consegui enviar sua atividade.'); setStep('review');
    }
  }
  ...
  return <main style={{ minHeight: '100dvh', background: '#050507', color: '#fff', padding: 18 }}><section style={{ maxWidth: 980, margin: '0 auto', display: 'grid', gap: 18 }}>
    ...
    {renderStatus ? <div style={{ ...box, color: '#f5c76b' }}>{renderStatus}</div> : null}
    ...
    {result ? <section style={box}><h2 style={{ marginTop: 0 }}>Diagnóstico da gravação</h2><ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>{blobStatus('Vídeo da câmera', result.cameraBlob, result.diagnostics.cameraChunks)}{blobStatus('Vídeo do canvas', result.canvasBlob, result.diagnostics.canvasChunks)}{blobStatus('Voz gravada', result.voiceBlob, result.diagnostics.voiceChunks)}{blobStatus('Referência separada (diagnóstico)', result.referenceBlob, result.diagnostics.referenceChunks)}{blobStatus('Vídeo final bruto', result.mixedBlob, result.diagnostics.mixedChunks)}</ul><p style={{ opacity: .7 }}>O envio final usa a referência original, não esta faixa regravada.</p></section> : null}
