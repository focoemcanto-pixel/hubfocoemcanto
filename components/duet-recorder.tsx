...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  async function startRecording() {
    ...
    try {
      ...
      setStep('recording'); setStatus('Gravando com referência monitorada pelo AudioEngine e vídeo da referência mudo.');
    } catch (err) {
  ...
  async function stopRecording() {
    ...
    try {
      const recording = await engine.stop(); recorderEngineRef.current = null; await stopReferenceMonitor(); setResult(recording); setStep('review'); setStatus('Gravação pronta. Abra o preview para mixar voz e referência com faders reais.');
  ...
  async function runReferenceFaderTest() {
    ...
    try {
      ...
      setVoiceVolume(oldVoice); setReferenceVolume(oldReference); engine.setFaders({ voice: oldVoice, reference: oldReference }); setDiagnostic(['TESTE DO FADER DA REFERÊNCIA', 'Voz forçada em 0% durante a medição.', ...rows, '', 'Leitura: se os dB mudam mas o ouvido não muda, existe áudio fora do mixer. Se os dB não mudam, o problema está no roteamento/gain da referência.'].join('\n'));
  ...
  return <main style={shell}><section style={wrap}>
    ...
    {result ? <section style={{ ...glass, padding: 18 }}><h2 style={{ marginTop: 0, fontSize: 22 }}>Diagnóstico técnico</h2><div style={{ display: 'grid', gap: 8, color: 'rgba(255,255,255,.78)' }}><span>✅ Vídeo composto: <strong>{formatSize(result.canvasBlob)}</strong></span><span>✅ Voz limpa: <strong>{formatSize(result.voiceBlob)}</strong></span><span>✅ Publicação segura iPhone: <strong>{formatSize(result.safePublishBlob)}</strong></span><span>✅ Referência: <strong>arquivo original via AudioEngine</strong></span><span>ℹ️ Câmera bruta: <strong>{formatSize(result.cameraBlob)}</strong></span></div></section> : null}
