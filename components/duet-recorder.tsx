...
import { AlertTriangle, CheckCircle2, Headphones, Lock, Mic, Music2, Pause, Play, RefreshCcw, Send, SlidersHorizontal, Sparkles, UploadCloud, Users, Video, Wand2, X } from 'lucide-react';
...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  async function uploadBlobForSubmit(postToCommunity: boolean) {
    ...
    try { const rendered = await renderMixBlob(PUBLISH_RENDER_TIMEOUT_MS); if (rendered?.size && rendered.size >= 1000) { setPublishProgress(55); return rendered; } } catch {}
    setPublishProgress(55); setRenderStatus('Finalizando publicação com a gravação completa...'); return fallback;
  }
