...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  function runDiagnostic() {
    ...
    setDiagnostic([
      ...
      `voiceGain=${snapshot.voiceGain.toFixed(4)} · voiceDb=${snapshot.voiceDb.toFixed(1)} dBFS · voiceTime=${snapshot.voiceTime.toFixed(2)} · paused=${snapshot.voicePaused}`,
      `referenceGain=${snapshot.referenceGain.toFixed(4)} · referenceDb=${snapshot.referenceDb.toFixed(1)} dBFS · referenceTime=${snapshot.referenceTime.toFixed(2)} · paused=${snapshot.referencePaused}`,
  ...
  async function buildUploadBlob() {
    ...
    setStatus(`Renderizando final com faders: voz ${voiceVolume}% · referência ${referenceVolume}%...`);
    const renderer = new DuetRendererEngine({ visualBlob: result.canvasBlob, voiceBlob: result.voiceBlob, referenceUrl: referenceSource, faders: { voice: voiceVolume, reference: referenceVolume } });
  ...
  return <main style={shell}><section style={wrap}>
    ...
    {step === 'review' ? <section style={{ ...glass, padding: 18, display: 'grid', gap: 16 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}><div><h2 style={{ margin: 0, fontSize: 24 }}>Mixagem com PreviewEngine</h2><p style={{ margin: '4px 0 0', color: 'rgba(255,255,255,.62)' }}>Vídeo mudo, voz e referência em trilhas independentes.</p></div><SlidersHorizontal color="#f5c76b" /></div><label style={{ display: 'grid', gap: 8 }}><span style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 800 }}><span>Minha voz</span><span>{voiceVolume}%</span></span><input type="range" min="0" max="200" value={voiceVolume} onChange={(event) => setVoice(Number(event.target.value))} /></label><label style={{ display: 'grid', gap: 8 }}><span style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 800 }}><span>Referência original</span><span>{referenceVolume}%</span></span><input type="range" min="0" max="200" value={referenceVolume} onChange={(event) => setReference(Number(event.target.value))} /></label><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><button style={mutedPill} onClick={() => { setVoice(100); setReference(100); }}>Reset</button><button style={mutedPill} onClick={autoMix}>Auto Mix</button><button style={mutedPill} onClick={runDiagnostic}>Medir engine</button><button style={goldPill} onClick={playing ? pausePreview : playPreview}>{playing ? <Pause size={18} /> : <Play size={18} />}{playing ? 'Pausar mix' : 'Ouvir mix'}</button></div></section> : null}
