...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  const monitorReferenceRef = useRef<HTMLAudioElement | null>(null);
  ...
  useEffect(() => () => {
    ...
    monitorEngineRef.current?.close().catch(() => undefined);
  }, []);

  async function startReferenceMonitor() {
    if (!referenceSource || !monitorReferenceRef.current) return;
    await monitorEngineRef.current?.close().catch(() => undefined);
    const audio = monitorReferenceRef.current;
    audio.pause();
    audio.src = referenceSource;
    audio.preload = 'auto';
    audio.volume = 0;
    audio.muted = false;
    audio.load();
    ...
    engine.connectReferenceElement(audio);
  ...
  async function stopReferenceMonitor() {
    try { monitorReferenceRef.current?.pause(); } catch {}
    await monitorEngineRef.current?.close().catch(() => undefined);
  ...
  async function ensurePreviewEngine() {
    ...
    const engine = new DuetPreviewEngine({ visual: previewVisualRef.current, voice: previewVoiceRef.current, reference: previewReferenceRef.current }, {
      visualBlob: result.canvasBlob,
      voiceBlob: result.voiceBlob,
      referenceUrl: referenceSource,
      initialFaders: { voice: voiceVolume, reference: referenceVolume },
      preGains: { voice: 3.2, reference: 0.08 },
      referenceOffsetMs,
    });
  ...
  async function playPreview() {
    ...
    try {
      ...
      setPlaying(false);
      setError(`Não consegui iniciar o preview: ${errorText(err)}`);
    }
  }
  ...
  function setVoice(value: number) {
    ...
    previewEngineRef.current?.setFaders({ voice: value });
  }

  function setReference(value: number) {
    setReferenceVolume(value);
    previewEngineRef.current?.setFaders({ reference: value });
  }

  function setReferenceOffset(value: number) {
    setReferenceOffsetMsState(value);
    previewEngineRef.current?.setReferenceOffsetMs(value);
    setStatus(value === 0 ? 'Sincronia da referência zerada.' : `Sincronia ajustada: ${value > 0 ? 'referência atrasa' : 'referência adianta'} ${Math.abs(value)}ms. Toque novamente para ouvir do início.`);
  }

  function autoMix() {
    setVoiceVolume(110);
    setReferenceVolume(70);
    previewEngineRef.current?.autoMix();
    setStatus('Auto Mix aplicado: voz presente e referência em apoio.');
  }
  ...
  function runDiagnostic() {
    ...
    setDiagnostic([
      'DIAGNÓSTICO DO PREVIEW ENGINE',
      `context=${snapshot.contextState}`,
      `voiceGain=${snapshot.voiceGain.toFixed(4)} · voiceDb=${snapshot.voiceDb.toFixed(1)} dBFS · voiceTime=${snapshot.voiceTime.toFixed(2)} · paused=${snapshot.voicePaused} · drift=${snapshot.voiceDriftMs}ms`,
      `referenceGain=${snapshot.referenceGain.toFixed(4)} · referenceDb=${snapshot.referenceDb.toFixed(1)} dBFS · referenceTime=${snapshot.referenceTime.toFixed(2)} · paused=${snapshot.referencePaused} · drift=${snapshot.referenceDriftMs}ms`,
      `referenceOffset=${snapshot.referenceOffsetMs}ms`,
      `masterDb=${snapshot.masterDb.toFixed(1)} dBFS · visualTime=${snapshot.visualTime.toFixed(2)} · visualPaused=${snapshot.visualPaused}`,
    ].join('\n'));
  }
  ...
  async function runReferenceFaderTest() {
    ...
    try {
      ...
      const rows: string[] = [];
      setVoiceVolume(0);
      engine.setFaders({ voice: 0 });
      for (const level of levels) {
        setReferenceVolume(level);
        engine.setFaders({ reference: level });
        await sleep(650);
        const snapshot = engine.getDiagnostic();
        rows.push(`${level}% → gain=${snapshot.referenceGain.toFixed(5)} · referenceDb=${snapshot.referenceDb.toFixed(1)} dBFS · masterDb=${snapshot.masterDb.toFixed(1)} dBFS · paused=${snapshot.referencePaused}`);
      }
      setVoiceVolume(oldVoice);
      setReferenceVolume(oldReference);
      engine.setFaders({ voice: oldVoice, reference: oldReference });
      setDiagnostic([
        'TESTE DO FADER DA REFERÊNCIA',
        'Voz forçada em 0% durante a medição.',
        ...rows,
        '',
        'Leitura: se os dB mudam mas o ouvido não muda, existe áudio fora do mixer. Se os dB não mudam, o problema está no roteamento/gain da referência.',
      ].join('\n'));
    } catch (err) {
      setError(`Não consegui medir o fader da referência: ${errorText(err)}`);
    }
  }
  ...
  async function reset() {
    ...
    await previewEngineRef.current?.close().catch(() => undefined);
    previewEngineRef.current = null;
    setResult(null); setError(''); setStatus(''); setDiagnostic(''); setPostedHref(''); setStep('intro'); setPlaying(false);
  }
  ...
  async function submit() {
    ...
    try {
      ...
      setError(`Não consegui enviar sua atividade: ${errorText(err)}`);
      setStep('review');
    }
  }
