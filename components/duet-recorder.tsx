...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  function finishRecording(stream: MediaStream, mediaRecorder: MediaRecorder) {
    ...
    window.setTimeout(() => {
      ...
      if (recorder.visualChunksRef.current.length && recorder.micChunksRef.current.length) {
        ...
        recorder.prepareEngine(voiceBlob, referenceBlob).then(() => { recorder.setVisualUrl(URL.createObjectURL(visualBlob)); applyAutoMix(true); }).catch(() => { recorder.setVisualUrl(null); recorder.setAudioReady(false); setShowMixer(false); recorder.setError('A prévia completa está disponível. Não consegui liberar a mixagem ao vivo desta gravação.'); });
      }
    }, 1200);
  ...
  return <div className="duet-recording-premium">
    ...
    <section className="duet-video-stage">
      ...
      {recorder.previewUrl ? <><video ref={recorder.previewRef} className="duet-final-video" src={reviewPreviewSrc || recorder.previewUrl} playsInline muted={useLiveMixPreview} controls={!useLiveMixPreview} onLoadedMetadata={() => recorder.engineRef.current?.setVideo(recorder.previewRef.current)} onEnded={() => { recorder.engineRef.current?.pause(false, true); recorder.setIsPlaying(false); }} />{useLiveMixPreview ? <button type="button" className="duet-stage-play" onClick={togglePlayback}>{recorder.isPlaying ? <Pause size={38} fill="currentColor" /> : <Play size={42} fill="currentColor" />}</button> : null}</> : <canvas ref={recorder.canvasRef} className="duet-canvas" width={1280} height={720} />}
    ...
    {recorder.step === 'caption' ? <div className="duet-caption-sheet"><button type="button" className="duet-sheet-close" onClick={() => recorder.setStep('review')}><X size={22} /></button><h2>Legenda do seu dueto</h2><textarea value={caption} onChange={(e) => setCaption(e.target.value)} placeholder="Escreva uma legenda curta para a comunidade..." maxLength={220} /><button type="button" onClick={() => submitDuet(caption, { postToCommunity, sendForReview })} disabled={recorder.isSubmitting}>{recorder.isSubmitting ? 'Publicando...' : finalButtonText()}</button></div> : null}
