...
const PUBLISH_RENDER_TIMEOUT_MS = 22000;
...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl }: Props) {
  ...
  function chooseDevice(deviceId: string, devices = audioDevices) { setSelectedAudioDeviceId(deviceId); const device = devices.find((item) => item.deviceId === deviceId); recorder.setLatencyMs(estimateDuetLatencyMs(device?.label)); }
  ...
  async function refreshAudioDevices(preferPhoneMic = false) {
    ...
    try {
      ...
      if (!selectedAudioDeviceId || !devices.some((device) => device.deviceId === selectedAudioDeviceId)) chooseDevice(preferPhoneMic ? preferredPhoneMicDeviceId(devices) : devices[0]?.deviceId || '', devices);
  ...
  async function unlockAudioSetup() {
    ...
    setIsLoadingDevices(true);
    try { const probe = await navigator.mediaDevices.getUserMedia({ audio: true }); probe.getTracks().forEach((track) => track.stop()); setAudioSetupUnlocked(true); await refreshAudioDevices(true); }
  ...
  function resetLocalState() {
    recorder.setPreviewUrl(null); recorder.setVisualUrl(null); recorder.setAudioReady(false); recorder.setError('');
    ...
    recorder.finalBlobRef.current = null; recorder.visualBlobRef.current = null; recorder.voiceBlobRef.current = null; recorder.referenceBlobRef.current = null;
    premiumRenderCacheRef.current = null; setCaption(''); setPostCommunity(false); setShowMixer(false); setRenderStatus(''); setIsAutoMixing(false); setAutoMixMessage('');
  }

  async function startCountdown() {
    resetLocalState(); recorder.cleanup();
    if (!recorder.canRecord) { recorder.setError('Seu navegador não liberou câmera/microfone. Tente pelo Chrome ou Safari atualizado.'); return; }
    if (!referenceSource) { recorder.setError('Essa atividade ainda não tem vídeo de referência vinculado.'); return; }
  ...
  async function beginRecording(stream: MediaStream) {
    const canvas = recorder.canvasRef.current; const reference = recorder.referenceRef.current; const camera = recorder.cameraRef.current;
  ...
  async function applyAutoMix(isInitial = false) {
    const voiceBlob = recorder.voiceBlobRef.current; const referenceBlob = recorder.referenceBlobRef.current;
    ...
    } catch { setAutoMixMessage(isInitial ? '' : 'Não consegui analisar automaticamente. Ajuste manualmente.'); }
    finally { setIsAutoMixing(false); }
  }

  function fallbackUploadBlob() { return recorder.finalBlobRef.current; }

  async function renderMixBlob(timeoutMs: number) {
    const visualBlob = recorder.visualBlobRef.current; const voiceBlob = recorder.voiceBlobRef.current; const referenceBlob = recorder.referenceBlobRef.current;
    ...
    const timeoutPromise = new Promise<Blob>((_, reject) => window.setTimeout(() => reject(new Error('render_timeout')), timeoutMs));
  ...
  async function renderPremiumInBackground(submissionId: string) {
    ...
    if (!needsPremiumRender()) { setRenderStatus(''); return; }
    ...
    try {
      ...
      const fileType = rendered.type || 'video/webm'; const data = new FormData();
  ...
  async function uploadBlobForSubmit(forceCommunity: boolean) {
    const stableRecording = fallbackUploadBlob();
    if (stableRecording?.size) {
      if (forceCommunity) setRenderStatus('Publicando gravação estável...');
      return stableRecording;
    }
    if (!forceCommunity) return null;
    try {
      setRenderStatus('Preparando vídeo completo para a comunidade...');
      const rendered = await renderMixBlob(PUBLISH_RENDER_TIMEOUT_MS);
      if (rendered?.size && rendered.size >= 1000) return rendered;
    } catch {
      recorder.setError('Não consegui preparar a publicação. Tente novamente.');
      return null;
    }
    return null;
  }
  ...
  async function submitDuet(finalCaption: string, forceCommunity = false) {
    recorder.setIsSubmitting(true); recorder.setError(''); setRenderStatus(forceCommunity ? 'Preparando publicação...' : 'Publicando agora...');
    const blob = await uploadBlobForSubmit(forceCommunity);
    if (!blob) { recorder.setIsSubmitting(false); setRenderStatus(''); if (!recorder.error) recorder.setError('Grave o dueto antes de enviar.'); return; }
    try {
      const data = new FormData(); const fileType = blob.type || 'video/webm';
      data.set('lesson_slug', lessonSlug); data.set('caption', finalCaption); data.set('visibility', forceCommunity ? 'community' : 'private'); data.set('voice_volume', String(recorder.voiceVolume)); data.set('reference_volume', String(recorder.referenceVolume)); data.set('voice_preset', recorder.preset); data.set('noise_reduction', String(recorder.noiseReduction));
      ...
      if (!response.ok) { const json = await response.json().catch(() => null); recorder.setError(json?.detail || json?.message || 'Não consegui enviar sua atividade.'); recorder.setIsSubmitting(false); setRenderStatus(''); return; }
      const json = await response.json().catch(() => null); const submissionId = String(json?.id || '');
      recorder.setIsSubmitting(false); setRenderStatus(''); recorder.setStep('posted');
      if (submissionId && !forceCommunity) void renderPremiumInBackground(submissionId);
    } catch { recorder.setIsSubmitting(false); setRenderStatus(''); recorder.setError('Não consegui enviar sua atividade.'); }
  }
  ...
  return (
    <div className="duet-remix-studio real-duet-studio premium-duet-studio reels-duet-editor smule-duet-studio">
      ...
      <section className="duet-control-bar premium-duet-control-bar reels-review-actions">{recorder.step === 'recording' ? <><span className="recording-dot">● Gravando dueto</span><button className="button danger" onClick={stopRecording}>Finalizar gravação</button></> : null}{recorder.step === 'review' ? <><button className="button secondary" onClick={reset} disabled={recorder.isSubmitting}><RefreshCcw size={16} /> Regravar</button><label className="community-toggle review-community-toggle"><input type="checkbox" checked={postCommunity} onChange={(event) => setPostCommunity(event.target.checked)} disabled={recorder.isSubmitting} /> Publicar também na comunidade</label><button className="button" onClick={() => postCommunity ? recorder.setStep('caption') : submitDuet('', false)} disabled={recorder.isSubmitting}><UploadCloud size={16} /> {postCommunity ? 'Continuar' : recorder.isSubmitting ? (renderStatus || 'Publicando...') : 'Enviar para avaliação'}</button></> : null}</section>
      ...
      {recorder.step === 'posted' ? <section className="posted-box duet-posted-box premium-duet-note"><CheckCircle2 size={28} /><div><h2>Atividade enviada</h2><p>Sua gravação já foi publicada.</p><a className="button secondary" href={`/aluno/aula/${lessonSlug}`}>Voltar para aula</a></div></section> : null}
    </div>
  );
}
