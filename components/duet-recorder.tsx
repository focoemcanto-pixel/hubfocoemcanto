...
import { AlertTriangle, Camera, CheckCircle2, ChevronDown, CircleHelp, Crown, Headphones, Lock, Mic, Music2, Pause, Play, RefreshCcw, RotateCw, Send, SlidersHorizontal, Sparkles, UploadCloud, Users, Video, Wand2, X } from 'lucide-react';
...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  async function startCountdown() {
    ...
    try {
      ...
      setAudioSetupUnlocked(true);
      refreshAudioDevices(false).catch(() => undefined);
      recorder.drawFrame(); recorder.setStep('countdown');
      ...
      recorder.setStep('intro'); recorder.setError('O vídeo ou a câmera não carregaram. Feche a aba, abra novamente e permita câmera/microfone.');
    }
  }
  async function beginRecording(stream: MediaStream) {
    ...
    canvas.width = safari ? 960 : quality === '1080p' ? 1920 : 1280;
    canvas.height = safari ? 540 : quality === '1080p' ? 1080 : 720;
    try {
      await camera.play(); reference.pause(); reference.currentTime = 0; reference.muted = false; await reference.play(); recorder.startDraw();
    } catch {
      recorder.setStep('intro'); recorder.setError('O navegador bloqueou o início do dueto. Toque novamente em iniciar.'); return;
    }
  ...
  return <div className="duet-recording-premium">
    ...
    <header className="duet-premium-top"><a href={`/aluno/aula/${lessonSlug}`}><span>←</span> Voltar para a aula</a>{isRecording ? <button type="button" onClick={stopRecording}>Finalizar gravação</button> : <button type="button" onClick={() => recorder.setStep('review')} disabled={!recorder.previewUrl}>Finalizar gravação</button>}</header>

    <section className="duet-hero-card">
      <div className="duet-hero-copy"><p>Atividade prática</p><h1>Grave seu dueto</h1><span>Aula: {lessonTitle}</span></div>
      <div className="duet-hero-mic">🎙️</div>
      <div className="duet-hero-tips"><div><Headphones size={34} /><strong>Use fone de ouvido</strong><small>Isso garante melhor captação da sua voz e performance.</small></div><button type="button" onClick={() => !canSendForReview && setShowVipModal(true)}><Crown size={30} /><strong>{canSendForReview ? 'Modo VIP' : 'Modo gratuito'}</strong><small>{canSendForReview ? 'Grave e envie para avaliação.' : 'Grave duetos e poste na comunidade.'}</small><span>›</span></button></div>
    </section>
    ...
    <section className="duet-video-stage">
      ...
      {recorder.step === 'intro' ? <div className="duet-empty-stage"><Video size={46} /><p>O vídeo do seu dueto<br />aparecerá aqui</p></div> : null}
    ...
    <section className="duet-control-grid">
      <button type="button" onClick={toggleCameraFacing}><Camera size={24} /><strong>Câmera</strong><small>{cameraFacing === 'user' ? 'Frontal' : 'Traseira'}</small><ChevronDown size={18} /></button>
      <button type="button" onClick={() => setQuality((value) => value === '1080p' ? '720p' : '1080p')}><Video size={24} /><strong>Qualidade</strong><small>{quality}</small><ChevronDown size={18} /></button>
      <button type="button" onClick={audioSetupUnlocked ? () => refreshAudioDevices(true) : unlockAudioSetup} disabled={isLoadingDevices}><Mic size={24} /><strong>Áudio</strong><small>{selectedAudioDevice ? selectedAudioDevice.label.replace(/\s*\([^)]*\)/g, '').slice(0, 20) : 'Microfone padrão'}</small><ChevronDown size={18} /></button>
      <button type="button" onClick={() => setShowTips((value) => !value)}><CircleHelp size={24} /><strong>Dicas</strong><small>{showTips ? 'Ocultar dicas' : 'Ver dicas'}</small><ChevronDown size={18} /></button>
    </section>
    ...
    <section className="duet-action-console">
      <button type="button" onClick={previewBase}><Play size={30} fill="currentColor" /><span>{isPreviewingBase ? 'Pausar base' : 'Ouvir base'}</span></button>
      <button type="button" className="record-main" onClick={isRecording ? stopRecording : startCountdown} disabled={recorder.step === 'loading' || recorder.step === 'countdown' || recorder.step === 'review' || recorder.step === 'caption' || recorder.step === 'posted'}><Video size={32} fill="currentColor" /><strong>{isRecording ? 'Finalizar' : recorder.step === 'loading' ? 'Preparando...' : 'Iniciar gravação'}</strong><small>{isRecording ? 'Toque para finalizar' : 'Toque para começar'}</small></button>
      <button type="button" onClick={toggleCameraFacing}><RotateCw size={30} /><span>Virar câmera</span></button>
    </section>
...
const duetCss = `.duet-recording-premium{min-height:100dvh;margin:-24px -16px 0;padding:calc(30px + env(safe-area-inset-top)) 18px calc(28px + env(safe-area-inset-bottom));background:radial-gradient(circle at 78% 9%,rgba(245,199,107,.12),transparent 24%),linear-gradient(180deg,#111418 0%,#050608 48%,#020304 100%);color:#fff}.duet-premium-top{display:flex;align-items:center;justify-content:space-between;gap:18px;max-width:820px;margin:0 auto 26px}.duet-premium-top a,.duet-premium-top button{border:0;background:transparent;color:#fff;text-decoration:none;font-weight:900;font-size:15px;display:flex;align-items:center;gap:8px}.duet-premium-top a span{font-size:28px}.duet-premium-top button{color:#f5c76b}.duet-premium-top button:disabled{opacity:.35}.duet-hero-card,.duet-video-stage,.duet-action-console,.duet-duration-line,.duet-tips-panel,.duet-publish-card,.duet-caption-card,.duet-posted-card{max-width:820px;margin-left:auto;margin-right:auto}.duet-hero-card{position:relative;overflow:hidden;border:1px solid rgba(245,199,107,.26);border-radius:24px;padding:36px 30px 22px;background:radial-gradient(circle at 84% 24%,rgba(245,199,107,.18),transparent 31%),linear-gradient(135deg,rgba(255,255,255,.055),rgba(255,255,255,.018));box-shadow:0 24px 80px rgba(0,0,0,.28)}.duet-hero-card:after{content:'';position:absolute;right:90px;top:95px;width:220px;height:2px;background:linear-gradient(90deg,transparent,rgba(245,199,107,.45),transparent);box-shadow:0 0 32px rgba(245,199,107,.45)}.duet-hero-copy{position:relative;z-index:2}.duet-hero-copy p,.duet-section-label{color:#f5c76b;text-transform:uppercase;letter-spacing:.22em;font-weight:950;font-size:13px}.duet-hero-copy h1{font-family:Georgia,'Times New Roman',serif;font-size:clamp(40px,8vw,58px);line-height:1;margin:18px 0 16px;letter-spacing:-.05em}.duet-hero-copy span{font-size:18px;color:rgba(255,255,255,.75)}.duet-hero-mic{position:absolute;right:62px;top:24px;font-size:132px;filter:sepia(1) saturate(1.8) drop-shadow(0 0 34px rgba(245,199,107,.28));opacity:.88}.duet-hero-tips{position:relative;z-index:2;margin-top:58px;display:grid;grid-template-columns:1fr 1fr;gap:14px}.duet-hero-tips>div,.duet-hero-tips>button{min-height:86px;text-align:left;border:1px solid rgba(255,255,255,.08);border-radius:15px;background:rgba(255,255,255,.045);color:#fff;padding:18px;display:grid;grid-template-columns:44px 1fr 18px;gap:14px;align-items:center}.duet-hero-tips>div{grid-template-columns:44px 1fr}.duet-hero-tips svg{color:#f5c76b}.duet-hero-tips strong{display:block;color:#f5c76b;font-size:17px}.duet-hero-tips small{display:block;margin-top:7px;color:rgba(255,255,255,.68);line-height:1.35}.duet-section-label{max-width:820px;margin:28px auto 14px}.duet-video-stage{position:relative;aspect-ratio:16/11;border:1px solid rgba(245,199,107,.62);border-radius:23px;background:#020304;overflow:hidden;box-shadow:inset 0 0 60px rgba(0,0,0,.65),0 18px 70px rgba(0,0,0,.28)}.duet-hidden-source{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}.duet-canvas,.duet-final-video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;background:#020304}.duet-empty-stage,.duet-stage-overlay,.duet-countdown{position:absolute;inset:0;display:grid;place-items:center;text-align:center;color:rgba(255,255,255,.52);font-size:17px}.duet-empty-stage svg{margin:0 auto 18px;color:rgba(255,255,255,.52)}.duet-stage-overlay span{border:1px solid rgba(245,199,107,.25);border-radius:999px;background:rgba(0,0,0,.62);color:#f5c76b;padding:10px 16px;font-weight:900}.duet-stage-overlay.recording span{color:#ff6565;border-color:rgba(255,101,101,.34)}.duet-countdown{font-size:110px;color:#f5c76b;text-shadow:0 0 45px rgba(245,199,107,.62);background:rgba(0,0,0,.45)}.duet-stage-play{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:82px;height:82px;border-radius:50%;border:2px solid rgba(255,255,255,.84);background:rgba(0,0,0,.35);color:#fff}.duet-video-chip{position:absolute;left:18px;bottom:18px;border-radius:999px;background:rgba(0,0,0,.58);border:1px solid rgba(245,199,107,.22);padding:9px 13px;display:flex;gap:8px;align-items:center;color:#f5c76b;font-weight:850}.duet-control-grid{max-width:820px;margin:0 auto 26px;display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.duet-control-grid button{position:relative;min-height:82px;border:1px solid rgba(245,199,107,.22);border-radius:15px;background:linear-gradient(135deg,rgba(255,255,255,.055),rgba(255,255,255,.02));color:#fff;text-align:left;padding:16px 32px 14px 18px}.duet-control-grid svg{color:#f5c76b}.duet-control-grid strong{display:block;margin-top:8px;color:#f5c76b;font-size:15px}.duet-control-grid small{display:block;margin-top:8px;color:rgba(255,255,255,.72);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.duet-control-grid button>svg:last-child{position:absolute;right:12px;bottom:18px}.duet-tips-panel{border:1px solid rgba(255,255,255,.10);border-radius:16px;background:rgba(255,255,255,.04);padding:16px 18px;margin-bottom:18px;color:rgba(255,255,255,.72)}.duet-tips-panel p{display:flex;gap:10px;align-items:flex-start;margin:8px 0}.duet-tips-panel svg{color:#f5c76b;flex:0 0 auto}.duet-tips-panel .warning{color:#ffd98c}.duet-action-console{border:1px solid rgba(255,255,255,.09);border-radius:21px;background:linear-gradient(135deg,rgba(255,255,255,.055),rgba(255,255,255,.018));padding:23px 22px;display:grid;grid-template-columns:1fr 1.4fr 1fr;align-items:center;text-align:center}.duet-action-console button{border:0;background:transparent;color:#fff;display:grid;place-items:center;gap:10px;font-size:14px}.duet-action-console button>svg{background:rgba(255,255,255,.08);border-radius:50%;padding:14px;width:62px;height:62px}.duet-action-console span{color:rgba(255,255,255,.78)}.record-main{position:relative}.record-main svg{width:82px!important;height:82px!important;border:2px solid #f5c76b!important;background:rgba(245,199,107,.12)!important;color:#f5c76b!important;box-shadow:0 0 28px rgba(245,199,107,.16)}.record-main strong{color:#f5c76b;font-size:18px}.record-main small{color:rgba(255,255,255,.66)}.duet-duration-line{margin-top:18px;border:1px solid rgba(255,255,255,.08);border-radius:999px;background:rgba(255,255,255,.035);display:grid;grid-template-columns:auto 1fr auto;gap:16px;align-items:center;padding:12px 18px;color:rgba(255,255,255,.70);font-size:14px}.duet-duration-line i{height:5px;border-radius:999px;background:rgba(255,255,255,.08);overflow:hidden}.duet-duration-line b{display:block;height:100%;border-radius:inherit;background:#f5c76b}.duet-premium-error{max-width:820px;margin:16px auto;border:1px solid rgba(255,91,91,.35);border-radius:16px;background:rgba(255,91,91,.08);color:#ffb4b4;padding:14px 16px}.duet-publish-progress,.duet-after-record-actions,.duet-publish-card,.duet-caption-card,.duet-posted-card{max-width:820px;margin:18px auto}.duet-publish-progress div{height:8px;border-radius:999px;background:rgba(255,255,255,.09);overflow:hidden}.duet-publish-progress span{display:block;height:100%;background:#f5c76b}.duet-after-record-actions{display:grid;grid-template-columns:1fr 1fr;gap:12px}.duet-after-record-actions button,.duet-publish-card button,.duet-caption-card button,.duet-posted-card a{border:1px solid rgba(245,199,107,.28);border-radius:15px;background:rgba(255,255,255,.04);color:#f5c76b;text-decoration:none;padding:14px 18px;font-weight:900;display:flex;align-items:center;justify-content:center;gap:8px}.duet-publish-card,.duet-caption-card,.duet-posted-card{border:1px solid rgba(245,199,107,.22);border-radius:21px;background:rgba(255,255,255,.035);padding:22px}.duet-publish-card label{display:flex;align-items:center;gap:13px;border:1px solid rgba(255,255,255,.10);border-radius:15px;padding:14px;margin:12px 0}.duet-publish-card label.active{border-color:rgba(245,199,107,.48);background:rgba(245,199,107,.08)}.duet-publish-card input{width:18px;height:18px}.duet-publish-card small{display:block;color:rgba(255,255,255,.60)}.duet-caption-card textarea{width:100%;min-height:120px;border:1px solid rgba(255,255,255,.12);border-radius:15px;background:rgba(0,0,0,.22);color:#fff;padding:14px}.duet-caption-card div,.duet-posted-card div div{display:flex;gap:12px;margin-top:14px}.vip-lock-backdrop{position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,.68);display:grid;place-items:center;padding:22px}.vip-lock-modal{position:relative;max-width:430px;border-radius:24px;border:1px solid rgba(245,199,107,.35);background:#111;color:#fff;padding:28px}.vip-lock-close{position:absolute;right:14px;top:14px;background:transparent;border:0;color:#fff}.vip-lock-cta{display:block;text-align:center;border-radius:999px;background:#f5c76b;color:#171009;text-decoration:none;font-weight:950;padding:14px;margin-top:18px}.vip-lock-later{width:100%;margin-top:10px;background:transparent;border:0;color:#ddd}@media(max-width:720px){.duet-recording-premium{margin:-16px -12px 0;padding:calc(24px + env(safe-area-inset-top)) 14px calc(24px + env(safe-area-inset-bottom))}.duet-hero-card{padding:30px 20px 20px}.duet-hero-copy h1{font-size:42px}.duet-hero-mic{right:16px;top:36px;font-size:92px;opacity:.72}.duet-hero-tips{grid-template-columns:1fr;margin-top:42px}.duet-video-stage{aspect-ratio:1/1.02}.duet-control-grid{grid-template-columns:repeat(2,1fr)}.duet-action-console{grid-template-columns:1fr 1.25fr 1fr;padding:20px 10px}.duet-action-console button>svg{width:56px;height:56px}.record-main svg{width:78px!important;height:78px!important}.duet-duration-line{grid-template-columns:1fr auto}.duet-duration-line i{grid-column:1/-1;grid-row:2}.duet-premium-top{padding:0 4px}.duet-premium-top a,.duet-premium-top button{font-size:14px}}`;
