...
async function posterDataUrlToFile(dataUrl?: string | null) {
  if (!dataUrl || !dataUrl.startsWith('data:image/')) return null;
  const blob = await (await fetch(dataUrl)).blob();
  if (blob.size < 300) return null;
  return new File([blob], `dueto-poster.${blob.type.includes('png') ? 'png' : 'jpg'}`, { type: blob.type || 'image/jpeg' });
}
...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true, vipCheckoutUrl = DEFAULT_VIP_CHECKOUT }: Props) {
  ...
  useEffect(() => () => { try { recorderEngineRef.current?.cleanup(); } catch {}; previewEngineRef.current?.close().catch(() => undefined); }, []);

  function applyReferenceOffset(value: number, message?: string) { setReferenceOffsetMsState(value); previewEngineRef.current?.setReferenceOffsetMs(value); if (message) setStatus(message); }
  ...
  function setReferenceOffset(value: number) { applyReferenceOffset(value, 'Sincronia ajustada. Toque em ouvir mix para conferir do início.'); }
  ...
  async function mapAudioDevices() {
    ...
    try {
      ...
      setAudioDevices(inputs); setShowMicChoices(true); if (!selectedAudioDeviceId && inputs[0]) setSelectedAudioDeviceId(inputs[0].deviceId);
    } catch (err) { setError(`Não consegui mapear os microfones: ${errorText(err)}`); }
  }

  async function applySmartMix(recording = result) {
    if (!recording?.voiceBlob || !referenceSource) { setVoice(110); setReference(70); setPreset('studio'); setStatus('Auto Mix aplicado.'); return; }
    try {
      setStatus('Analisando sua gravação para ajustar a mixagem...');
  ...
  async function startRecording() {
    setError(''); setStatus(''); setPostedHref(''); setResult(null); setPostingProgress(0); setPlaying(false); setShowMicChoices(false); setReferenceOffsetMsState(0);
    ...
    try {
      ...
      recorderEngineRef.current = engine; setStatus('Preparando câmera e referência...'); await engine.prepare(); setStep('countdown');
      for (let value = 4; value >= 1; value -= 1) { setCountdown(value); await sleep(1000); }
      setCountdown(0); await engine.start(); setStep('recording'); setStatus('Gravando agora. Cante junto com a referência e finalize quando terminar.');
    } catch (err) { setCountdown(0); try { recorderEngineRef.current?.cleanup(); } catch {}; recorderEngineRef.current = null; setStep('intro'); setError(`Não consegui iniciar a gravação: ${errorText(err)}`); }
  }
  ...
  async function createServerRenderJob() {
    ...
    const json = await response.json().catch(() => null); const jobId = String(json?.job_id || '');
    if (json?.status === 'completed' && json?.output_url) { setPostingProgress(88); return String(json.output_url); }
    if (!jobId) return null;
    for (let attempt = 1; attempt <= 18; attempt += 1) { setPostingProgress(Math.min(84, 28 + attempt * 3)); await sleep(3000); const statusResponse = await fetch(`/api/duet-render/status/${jobId}`, { cache: 'no-store' }); const statusJson = await statusResponse.json().catch(() => null); const job = statusJson?.job; if (job?.status === 'completed' && job.output_url) { setPostingProgress(88); return String(job.output_url); } if (job?.status === 'failed') throw new Error(job.error_message || 'Renderização no servidor falhou.'); }
    return null;
  }

  async function saveRenderedUrl(fileUrl: string) {
    const response = await fetch('/api/submissions/duet', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ lesson_slug: lessonSlug, caption: caption || 'Minha prática do dueto.', visibility: postCommunity ? 'community' : 'private', review_requested: canSendForReview && sendForReview, file_url: fileUrl, poster_data_url: result?.posterDataUrl || null }) });
  ...
  async function buildUploadBlob() {
    ...
    const renderer = new DuetRendererEngine({ visualBlob: result.canvasBlob, voiceBlob: result.voiceBlob, referenceUrl: referenceSource, faders: { voice: voiceVolume, reference: referenceVolume }, preGains: { voice: VOICE_PRE_GAIN, reference: REFERENCE_PRE_GAIN }, referenceOffsetMs });
    const rendered = await renderer.renderVideo(); if (!rendered.blob || rendered.blob.size < 1000) throw new Error(`render_empty:${rendered.blob?.size || 0}`); setPostingProgress(80); return rendered.blob;
  }
  async function submit() {
    ...
    try {
      ...
      const posterFile = await posterDataUrlToFile(result.posterDataUrl);
  ...
  return <main style={shell}><section style={wrap}>
    ...
    {step === 'posting' ? <section style={{ ...glass, padding: 22, display: 'grid', gap: 14 }}><strong style={{ fontSize: 22 }}>Publicando seu dueto</strong><p style={{ margin: 0, color: 'rgba(255,255,255,.62)' }}>{status || 'Preparando sua publicação...'}</p><div style={{ height: 10, borderRadius: 999, background: 'rgba(255,255,255,.12)', overflow: 'hidden' }}><i style={{ display: 'block', height: '100%', width: `${postingProgress}%`, background: 'linear-gradient(90deg,#f8d47b,#f3bd49)' }} /></div></section> : null}
    {step === 'review' ? <section style={{ ...glass, padding: 16, display: 'grid', gap: 14 }}><p style={{ ...label, margin: 0 }}><SlidersHorizontal size={15} /> Mixagem</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 8 }}>{(['natural','studio','worship','coral'] as Preset[]).map((key) => <button type="button" key={key} style={cardOption(preset === key)} onClick={() => applyPreset(key)}><span><strong>{presets[key].title}</strong><br/><small style={{ color: 'rgba(255,255,255,.58)' }}>{presets[key].text}</small></span></button>)}</div><label>Minha voz: {voiceVolume}%<input type="range" min="0" max="160" value={voiceVolume} onChange={(e) => setVoice(Number(e.target.value))} style={{ width: '100%' }} /></label><label>Referência: {referenceVolume}%<input type="range" min="0" max="160" value={referenceVolume} onChange={(e) => setReference(Number(e.target.value))} style={{ width: '100%' }} /></label><label>Sincronia da referência: {referenceOffsetMs}ms<input type="range" min="-300" max="300" step="10" value={referenceOffsetMs} onChange={(e) => setReferenceOffset(Number(e.target.value))} style={{ width: '100%' }} /></label></section> : null}
    {step === 'review' ? <section style={{ ...glass, padding: 16, display: 'grid', gap: 12 }}><label style={cardOption(postCommunity)}><input type="checkbox" checked={postCommunity} onChange={(e) => setPostCommunity(e.target.checked)} /> Postar na comunidade</label><label style={cardOption(sendForReview)}><input type="checkbox" checked={sendForReview} onChange={(e) => canSendForReview ? setSendForReview(e.target.checked) : setShowVipModal(true)} /> Enviar para avaliação do professor</label><textarea value={caption} onChange={(e) => setCaption(e.target.value)} style={{ minHeight: 88, borderRadius: 18, border: '1px solid rgba(255,255,255,.12)', background: 'rgba(255,255,255,.06)', color: '#fff', padding: 14 }} /><button style={goldPill} onClick={submit}><Send size={18} /> Publicar atividade</button></section> : null}
    {step === 'posted' ? <section style={{ ...glass, padding: 22, textAlign: 'center', display: 'grid', gap: 12 }}><CheckCircle2 size={44} color="#f5c76b" style={{ margin: '0 auto' }} /><h2 style={{ margin: 0 }}>Dueto publicado!</h2>{postedHref ? <a href={postedHref} style={{ color: '#f5c76b', fontWeight: 900 }}>Ver na comunidade</a> : null}<button style={mutedPill} onClick={reset}>Gravar outro</button></section> : null}
    {showVipModal ? <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.72)', zIndex: 999, display: 'grid', placeItems: 'center', padding: 20 }} onClick={() => setShowVipModal(false)}><section style={{ ...glass, maxWidth: 430, padding: 26 }} onClick={(e) => e.stopPropagation()}><button onClick={() => setShowVipModal(false)} style={{ float: 'right', border: 0, background: 'transparent', color: '#fff' }}><X /></button><Lock color="#f5c76b" /><h2>Exclusivo VIP</h2><p>Assine para enviar suas atividades para avaliação.</p><a href={vipCheckoutUrl} style={{ ...goldPill, textDecoration: 'none' }}>Assinar VIP</a></section></div> : null}
  </section></main>;
}
