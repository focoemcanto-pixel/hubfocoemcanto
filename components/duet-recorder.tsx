...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  const [previewMixBlob, setPreviewMixBlob] = useState<Blob | null>(null);
  const [mixerDirty, setMixerDirty] = useState(false);
  const [mixing, setMixing] = useState(false);
  const previewBlob = previewMixBlob || result?.canvasBlob || null;
  const previewUrl = useMemo(() => blobUrl(previewBlob), [previewBlob]);

  async function startRecording() {
    setError(''); setResult(null); setPostedHref(''); setRenderStatus(''); setPreviewMixBlob(null); setMixerDirty(false);
  ...
  async function stopRecording() {
    ...
    try {
      ...
      setResult(recording);
      setPreviewMixBlob(null);
      setMixerDirty(true);
      setRenderStatus('Aplique o mix para ouvir o preview com os volumes escolhidos.');
      setStep('review');
    } catch (err: any) {
  ...
  function reset() { setResult(null); setError(''); setPostedHref(''); setRenderStatus(''); setPreviewMixBlob(null); setMixerDirty(false); setStep('intro'); }
  function markMixerDirty() { setMixerDirty(true); setPreviewMixBlob(null); }
  function setVoice(value: number) { setVoiceVolume(value); markMixerDirty(); }
  function setReference(value: number) { setReferenceVolume(value); markMixerDirty(); }
  function autoMix() { setVoiceVolume(115); setReferenceVolume(85); markMixerDirty(); setRenderStatus('Auto Mix aplicado. Toque em Aplicar mix para ouvir.'); }

  async function renderCurrentMix() {
    if (!result?.canvasBlob || !result?.voiceBlob || !referenceSource) throw new Error('mix_payload_missing');
    setMixing(true);
    setRenderStatus(`Renderizando preview: voz ${voiceVolume}% · referência ${referenceVolume}%...`);
    try {
      ...
      setPreviewMixBlob(rendered);
      setMixerDirty(false);
      setRenderStatus(`Mix aplicado no preview: voz ${voiceVolume}% · referência ${referenceVolume}%.`);
      return rendered;
    } finally {
      setMixing(false);
    }
  ...
  async function buildUploadBlob() {
    ...
    if (previewMixBlob && !mixerDirty) return previewMixBlob;
    try {
      return await renderCurrentMix();
    } catch (err) {
  ...
  async function applyMixPreview() {
    setError('');
    try { await renderCurrentMix(); }
    catch (err) { setError(`Não consegui aplicar o mix: ${errorText(err)}`); }
  }
  ...
  return <main style={shell}><section style={wrap}>
    ...
    <section style={{ ...glass, overflow: 'hidden', padding: 0 }}><div style={{ position: 'relative', aspectRatio: '16/9', background: '#000', display: 'grid', placeItems: 'center' }}><video ref={referenceRef} playsInline muted style={{ display: 'none' }} /><video ref={cameraRef} playsInline muted autoPlay style={{ display: 'none' }} />{previewUrl && step !== 'recording' ? <video key={previewUrl} src={previewUrl} playsInline controls style={{ width: '100%', height: '100%', objectFit: 'contain' }} /> : <canvas ref={canvasRef} width={1280} height={720} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}{step === 'intro' ? <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', textAlign: 'center', padding: 24, background: 'radial-gradient(circle,rgba(0,0,0,.18),rgba(0,0,0,.55))' }}><div style={{ display: 'grid', placeItems: 'center', gap: 12 }}><span style={{ width: 78, height: 78, borderRadius: 999, display: 'grid', placeItems: 'center', background: 'rgba(245,199,107,.16)', border: '1px solid rgba(245,199,107,.45)', color: '#f5c76b' }}><Video size={34} /></span><strong style={{ fontSize: 18 }}>Toque para iniciar câmera e referência</strong><small style={{ color: 'rgba(255,255,255,.62)' }}>A gravação será montada com a referência original.</small></div></div> : null}{step === 'recording' ? <div style={{ position: 'absolute', top: 14, left: 14, background: '#e11d48', borderRadius: 999, padding: '8px 13px', fontWeight: 950, boxShadow: '0 0 24px rgba(225,29,72,.5)' }}>● Gravando</div> : null}</div></section>
    ...
    {step === 'review' ? <section style={{ ...glass, padding: 18, display: 'grid', gap: 16 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}><div><h2 style={{ margin: 0, fontSize: 24 }}>Mixagem final</h2><p style={{ margin: '4px 0 0', color: 'rgba(255,255,255,.62)' }}>{mixerDirty ? 'Ajuste os volumes e toque em Aplicar mix para ouvir o resultado.' : 'O preview acima já está usando este mix.'}</p></div><SlidersHorizontal color="#f5c76b" /></div><label style={{ display: 'grid', gap: 8 }}><span style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 800 }}><span>Minha voz</span><span>{voiceVolume}%</span></span><input type="range" min="0" max="200" value={voiceVolume} onChange={(event) => setVoice(Number(event.target.value))} /></label><label style={{ display: 'grid', gap: 8 }}><span style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 800 }}><span>Referência original</span><span>{referenceVolume}%</span></span><input type="range" min="0" max="200" value={referenceVolume} onChange={(event) => setReference(Number(event.target.value))} /></label><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><button style={mutedPill} onClick={() => { setVoice(100); setReference(100); }}>Reset</button><button style={mutedPill} onClick={autoMix}>Auto Mix</button><button style={goldPill} disabled={mixing} onClick={applyMixPreview}>{mixing ? 'Aplicando...' : 'Aplicar mix'}</button></div></section> : null}
    {result ? <section style={{ ...glass, padding: 18 }}><h2 style={{ marginTop: 0, fontSize: 22 }}>Diagnóstico técnico</h2><div style={{ display: 'grid', gap: 8, color: 'rgba(255,255,255,.78)' }}><span>✅ Voz gravada: <strong>{formatSize(result.voiceBlob)}</strong></span><span>✅ Vídeo canvas: <strong>{formatSize(result.canvasBlob)}</strong></span><span>✅ Preview mixado: <strong>{formatSize(previewMixBlob)}</strong></span><span>ℹ️ Bruto da gravação: <strong>{formatSize(result.mixedBlob)}</strong></span></div></section> : null}
