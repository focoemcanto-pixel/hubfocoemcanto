...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  function syncElementAudio(nextVoice = voiceVolume, nextReference = referenceVolume) {
    ...
    if (voice) {
      voice.volume = 0;
      voice.muted = false;
      if (nextVoice <= 0) { try { voice.pause(); } catch {} }
      else if (isPlaying) { if (visual && Number.isFinite(visual.currentTime)) voice.currentTime = visual.currentTime; voice.play().catch(() => undefined); }
    }
    if (reference) {
      reference.volume = 0;
      reference.muted = false;
      if (nextReference <= 0) { try { reference.pause(); } catch {} }
      else if (isPlaying) { if (visual && Number.isFinite(visual.currentTime)) reference.currentTime = visual.currentTime; reference.play().catch(() => undefined); }
    }
  }
  ...
  function ensureLiveMixer() {
    ...
    voice.volume = 0; voice.muted = false;
    reference.volume = 0; reference.muted = false;
  ...
  async function startLivePreview() {
    ...
    try {
      ...
      setLivePlaying(true);
      setRenderStatus(`Preview normalizado: voz ${voiceVolume}% · referência ${referenceVolume}%. Áudio nativo zerado; só o mixer toca.`);
