...
import { AlertTriangle, Camera, CheckCircle2, ChevronDown, CircleHelp, Crown, Headphones, Lock, Mic, Music2, Pause, Play, RefreshCcw, RotateCw, SlidersHorizontal, UploadCloud, Users, Video, X } from 'lucide-react';
...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  function needsPremiumRender() {
    return recorder.voiceVolume !== DEFAULT_VOICE_VOLUME || recorder.referenceVolume !== DEFAULT_REFERENCE_VOLUME || recorder.preset !== DEFAULT_PRESET || recorder.noiseReduction;
  }
  ...
  function resetLocalState() {
    ...
    setPostedSummary('Seu dueto foi processado conforme as opções escolhidas.');
    setShowMixer(false);
  ...
  async function previewBase() {
    ...
    try {
      ...
      recorder.setError('Toque novamente para liberar o áudio da base no navegador.');
    }
  }
  ...
  function finishRecording(stream: MediaStream, mediaRecorder: MediaRecorder) {
    ...
    window.setTimeout(() => {
      ...
      if (recorder.visualChunksRef.current.length && recorder.micChunksRef.current.length) {
        ...
        recorder.prepareEngine(voiceBlob, null).then(() => {
          ...
          recorder.setAudioReady(false);
          setShowMixer(false);
          recorder.setError('A prévia está disponível. A mixagem ao vivo não foi liberada para esta gravação.');
        });
      }
    }, 1200);
    setShowMixer(false);
  ...
  async function renderMixBlob(timeoutMs: number) {
    ...
    const rendered = await Promise.race([
      ...
      new Promise<Blob>((_, reject) => window.setTimeout(() => reject(new Error('render_timeout')), timeoutMs)),
    ]);
  ...
  return <div className="duet-recording-premium">
    ...
    <header className="duet-premium-top">
      <a href={`/aluno/aula/${lessonSlug}`}><span>←</span> Voltar para a aula</a>
      {isRecording ? <button type="button" onClick={stopRecording}>Finalizar gravação</button> : <button type="button" onClick={() => recorder.previewUrl && recorder.setStep('review')} disabled={!recorder.previewUrl}>Assistir gravação</button>}
    </header>
    ...
    <section className="duet-hero-card">
      ...
      <div className="duet-hero-tips">
        <div><Headphones size={28} /><span><strong>Use fone de ouvido</strong><small>Isso garante melhor captação da sua voz e performance.</small></span></div>
        <button type="button" onClick={() => !canSendForReview && setShowVipModal(true)}><Crown size={28} /><span><strong>{canSendForReview ? 'Modo VIP' : 'Modo gratuito'}</strong><small>{canSendForReview ? 'Grave e envie para avaliação.' : 'Grave duetos e poste na comunidade.'}</small></span><b>›</b></button>
      </div>
    </section>
    ...
    <section className="duet-video-stage">
      ...
      {recorder.previewUrl ? <>
        ...
        {useLiveMixPreview ? <button type="button" className="duet-stage-play" onClick={togglePlayback}>{recorder.isPlaying ? <Pause size={38} fill="currentColor" /> : <Play size={42} fill="currentColor" />}</button> : null}
      </> : <canvas ref={recorder.canvasRef} className="duet-canvas" width={1280} height={720} />}
      {recorder.step === 'intro' ? <div className="duet-empty-stage"><div><Video size={42} /><p>O vídeo do seu dueto<br />aparecerá aqui</p></div></div> : null}
    ...
    <section className="duet-control-grid">
      <button type="button" onClick={toggleCameraFacing}><Camera size={22} /><strong>Câmera</strong><small>{cameraFacing === 'user' ? 'Frontal' : 'Traseira'}</small><ChevronDown size={18} /></button>
      <button type="button" onClick={() => setQuality((value) => value === '1080p' ? '720p' : '1080p')}><Video size={22} /><strong>Qualidade</strong><small>{quality}</small><ChevronDown size={18} /></button>
      <button type="button" onClick={audioSetupUnlocked ? () => refreshAudioDevices(true) : unlockAudioSetup} disabled={isLoadingDevices}><Mic size={22} /><strong>Áudio</strong><small>{selectedAudioDevice ? selectedAudioDevice.label.replace(/\s*\([^)]*\)/g, '').slice(0, 20) : 'Microfone padrão'}</small><ChevronDown size={18} /></button>
      <button type="button" onClick={() => setShowTips((value) => !value)}><CircleHelp size={22} /><strong>Dicas</strong><small>{showTips ? 'Ocultar dicas' : 'Ver dicas'}</small><ChevronDown size={18} /></button>
    </section>

    <section className="duet-action-console sticky-actions">
      <button type="button" onClick={handleLeftAction} disabled={recorder.step === 'loading' || recorder.step === 'countdown'}>{hasRecording ? (recorder.isPlaying ? <Pause size={27} fill="currentColor" /> : <Play size={27} fill="currentColor" />) : <Play size={27} fill="currentColor" />}<span>{hasRecording ? (recorder.isPlaying ? 'Pausar gravação' : 'Assistir') : (isPreviewingBase ? 'Pausar base' : 'Ouvir base')}</span></button>
      <button type="button" className="record-main" onClick={handlePrimaryRecordAction} disabled={recorder.step === 'loading' || recorder.step === 'countdown' || recorder.step === 'caption' || recorder.step === 'posted'}>{hasRecording && !isRecording ? <RefreshCcw size={30} /> : <Video size={30} fill="currentColor" />}<strong>{isRecording ? 'Finalizar' : recorder.step === 'loading' ? 'Preparando...' : hasRecording ? 'Regravar' : 'Iniciar gravação'}</strong><small>{isRecording ? 'Toque para finalizar' : hasRecording ? 'Grave outra versão' : 'Toque para começar'}</small></button>
      <button type="button" onClick={toggleCameraFacing}><RotateCw size={27} /><span>Virar câmera</span></button>
    </section>
    ...
    {recorder.step === 'review' ? <div className="duet-after-record-actions">
      {canUseMixer ? <button type="button" onClick={() => setShowMixer((value) => !value)} disabled={recorder.isSubmitting}><SlidersHorizontal size={16} /> {showMixer ? 'Ocultar mixagem' : 'Editar mixagem'}</button> : null}
      <button type="button" className="duet-continue-button" onClick={() => setShowPublishOptions(true)} disabled={recorder.isSubmitting}>Continuar</button>
    </div> : null}
