...
import { analyzeDuetAutoSync } from '@/lib/audio-engine/duet-auto-sync';
...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  async function startRecording() {
    ...
    try {
      ...
      setCountdown(0); try { recorderEngineRef.current?.cleanup(); } catch {} recorderEngineRef.current = null; setStep('intro'); setError(`Não consegui iniciar a gravação: ${errorText(err)}`);
    }
  }
  ...
  async function stopRecording() {
    ...
    try {
      const recording = await engine.stop(); recorderEngineRef.current = null; setResult(recording);
      const markerOffset = recording.markerOffsetMs || 0;
      if (markerOffset) setReferenceOffset(markerOffset);
      setStep('review'); setStatus(markerOffset ? `Marker aplicado: referência atrasada em ${markerOffset}ms. Você ainda pode refinar em Sincronizar automaticamente.` : 'Gravação pronta. Ajuste a mixagem antes de enviar.');
  ...
  async function reset() { pausePreview(); await previewEngineRef.current?.close().catch(() => undefined); previewEngineRef.current = null; setResult(null); setError(''); setStatus(''); setPostedHref(''); setStep('intro'); setPostingProgress(0); setCountdown(0); setPlaying(false); setSyncing(false); }

  async function autoSync() {
    if (!result?.voiceBlob || !referenceSource) return setError('Grave o dueto antes de sincronizar.');
    setError(''); setSyncing(true); pausePreview(); setStatus('Analisando áudio para sincronizar...');
    try {
      const sync = await analyzeDuetAutoSync({ referenceUrl: referenceSource, voiceBlob: result.voiceBlob, markerExpectedMs: result.markerOffsetMs || 220 });
      setReferenceOffset(sync.offsetMs);
      const label = sync.strategy === 'marker' ? 'marker/beep' : 'waveform';
      const confidence = Math.round(sync.confidence * 100);
      setStatus(confidence > 0 ? `Sincronização por ${label}: ${sync.offsetMs > 0 ? '+' : ''}${sync.offsetMs}ms (${confidence}% de confiança).` : 'Não consegui detectar com segurança. Ajuste manualmente em Sincronia.');
    } catch (err) { setError(`Não consegui sincronizar automaticamente: ${errorText(err)}`); }
    finally { setSyncing(false); }
  }

  async function createServerRenderJob() {
    if (!result?.canvasBlob || !result?.voiceBlob || !referenceSource) return null;
    const data = new FormData(); data.set('lesson_slug', lessonSlug); data.set('caption', caption || 'Minha prática do dueto.'); data.set('visibility', postCommunity ? 'community' : 'private'); data.set('review_requested', String(canSendForReview && sendForReview)); data.set('voice_volume', String(voiceVolume)); data.set('reference_volume', String(referenceVolume)); data.set('reference_offset_ms', String(referenceOffsetMs)); data.set('reference_url', referenceSource); data.set('video', new File([result.canvasBlob], `${lessonSlug}-visual.${(result.canvasBlob.type || '').includes('webm') ? 'webm' : 'mp4'}`, { type: result.canvasBlob.type || 'video/mp4' })); data.set('voice', new File([result.voiceBlob], `${lessonSlug}-voice.${(result.voiceBlob.type || '').includes('webm') ? 'webm' : 'm4a'}`, { type: result.voiceBlob.type || 'audio/mp4' }));
    setStatus('Preparando seu vídeo...'); setPostingProgress(22); const response = await fetch('/api/duet-render/jobs', { method: 'POST', body: data }); if (!response.ok) { const json = await response.json().catch(() => null); throw new Error(json?.detail || json?.message || 'Não consegui preparar o vídeo.'); }
    const json = await response.json().catch(() => null); const jobId = String(json?.job_id || ''); if (json?.status === 'completed' && json?.output_url) { setPostingProgress(88); return String(json.output_url); } if (!jobId) return null;
