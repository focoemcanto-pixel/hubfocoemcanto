...
function errorText(error: unknown) {
  return error instanceof Error ? `${error.name}: ${error.message}` : String(error || 'erro_desconhecido');
}
...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  function applyReferenceOffset(value: number, message?: string) {
  ...
  async function startRecording() {
    ...
    setPlaying(false);
    setShowMicChoices(false);
    setReferenceOffsetMsState(0);
  ...
  async function stopRecording() {
    ...
    try {
      ...
      if (markerOffset) {
        applyReferenceOffset(markerOffset, `Sincronização automática aplicada: referência atrasada em ${markerOffset}ms.`);
      } else {
  ...
  async function createServerRenderJob() {
    ...
    if (json?.status === 'completed' && json?.output_url) {
      setPostingProgress(88);
      return String(json.output_url);
    }
  ...
  async function saveRenderedUrl(fileUrl: string) {
    ...
    if (!response.ok) {
      const json = await response.json().catch(() => null);
      throw new Error(json?.detail || json?.message || 'Não consegui salvar sua publicação.');
    }
  ...
  async function buildUploadBlob() {
    ...
    if (isSafariLike() && result.safePublishBlob && result.safePublishBlob.size > 1000) {
      setStatus('Finalizando envio...');
      setPostingProgress(72);
      return result.safePublishBlob;
    }
    setStatus('Finalizando sua mixagem...');
    setPostingProgress(56);
  ...
  async function submit() {
    ...
    setStep('posting');
    setError('');
    setStatus('Enviando sua atividade...');
    setPostingProgress(10);
    pausePreview();
    try {
      if (isSafariLike()) {
