...
export function DuetRecorder({ lessonTitle, lessonSlug, referenceUrl, canSendForReview = true }: Props) {
  ...
  async function startRecording() {
    ...
    try {
      ...
      await engine.prepare(); await startReferenceMonitor(); await engine.start();
      setStep('recording'); setStatus('Gravando agora. Cante junto com a referência e finalize quando terminar.');
    } catch (err) {
      await stopReferenceMonitor(); try { recorderEngineRef.current?.cleanup(); } catch {}; recorderEngineRef.current = null; setStep('intro'); setError(`Não consegui iniciar a gravação: ${errorText(err)}`);
    }
  }
  ...
  async function stopRecording() {
    ...
    try {
      const recording = await engine.stop(); recorderEngineRef.current = null; await stopReferenceMonitor(); setResult(recording); setStep('review'); setStatus('Gravação pronta. Ajuste a mixagem antes de enviar.');
  ...
  async function reset() { pausePreview(); await previewEngineRef.current?.close().catch(() => undefined); previewEngineRef.current = null; setResult(null); setError(''); setStatus(''); setPostedHref(''); setStep('intro'); setPostingProgress(0); setPlaying(false); }
  ...
  async function buildUploadBlob() {
    ...
    if (isSafariLike() && result.safePublishBlob && result.safePublishBlob.size > 1000) {
      setStatus('Finalizando envio...'); setPostingProgress(72);
