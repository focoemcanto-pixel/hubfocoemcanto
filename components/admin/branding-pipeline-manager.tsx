...
import type { BrandingAssetSize } from '@/lib/data/admin-settings';
...
function drawAsset(image: HTMLImageElement, asset: Asset, size: BrandingAssetSize) {
  const width = Math.max(40, Math.round(size.width || asset.width));
  const height = Math.max(20, Math.round(size.height || asset.height));
...
export function BrandingPipelineManager({ initial, initialSizes }: { initial: BrandingState; initialSizes?: Partial<SizeMap> }) {
  const [values, setValues] = useState<BrandingState>(initial);
  const [sizes, setSizes] = useState<SizeMap>(() => normalizeSizes(initialSizes));
  ...
  function updateSize(key: AssetKey, field: 'width' | 'height', value: string) {
    const numeric = Math.max(40, Math.min(4000, Number(value) || 40));
  ...
  return (
    <>
      ...
      <div className="branding-pipeline">
        {ASSETS.map((asset) => <input key={asset.sizeField} type="hidden" name={`${asset.sizeField}Json`} value={JSON.stringify(sizes[asset.key])} />)}
        ...
        <div className="branding-pipeline-head">
          <div><p className="branding-pipeline-kicker">◇ Premium</p><h2 className="branding-pipeline-title">Identidade visual premium</h2><p className="branding-pipeline-sub">Gerencie os ativos visuais do Hub. Use uma imagem matriz ou envie cada formato separadamente.</p></div>
          <div className="branding-pipeline-actions"><label className="branding-btn gold">↥ {loading ? 'Processando...' : 'Subir imagem matriz'}<input type="file" accept="image/png,image/jpeg,image/webp" className="branding-hidden" disabled={loading} onChange={(event) => { const file = event.target.files?.[0]; event.currentTarget.value = ''; if (file) void generate(file); }} /></label><button type="button" onClick={() => { setValues(EMPTY); setMessage('Uploads removidos. Clique em Salvar alterações.'); }} disabled={loading} className="branding-btn danger">⌫ Remover tudo</button></div>
        </div>
        <div className="branding-tip"><span className="branding-tip-icon">✧</span><div><strong>Dica premium</strong><p>Edite o tamanho antes de subir a imagem matriz para gerar e também controlar a exibição da logo na Home e na sala de aula.</p></div></div>
        ...
        <div className="branding-assets-list">
          {previews.map((asset) => (
            <div key={asset.key} className="branding-asset-row">
              ...
              <div className="branding-size-control"><span>Tamanho / exibição</span><div className="branding-size-inputs"><input type="number" value={asset.size.width} min={40} max={4000} onChange={(e) => updateSize(asset.key, 'width', e.target.value)} /><em>x</em><input type="number" value={asset.size.height} min={20} max={4000} onChange={(e) => updateSize(asset.key, 'height', e.target.value)} /><em>px</em></div></div>
