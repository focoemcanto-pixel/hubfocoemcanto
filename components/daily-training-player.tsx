...
function BreathPractice({ step }: { step: DailyTrainingStep }) {
  ...
  function readBreath(analyser: AnalyserNode) {
    ...
    const loop = () => {
      ...
      if (isSustained) {
        ...
        setMessage(heldRef.current > 0 ? 'O S quebrou, retome para continuar contando' : 'Comece o S para iniciar a contagem');
      }
  ...
  return (
    <section className="breath-screen">
      ...
      <strong className="breath-time">{Math.floor(heldSeconds)} seg</strong>
