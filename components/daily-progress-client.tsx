...
export function DailyProgressClient() {
  ...
  const metrics = useMemo(() => {
    const base = completedCount ? Math.min(100, Math.max(20, conclusion)) : 0;
    return [
      ['Afinação', base],
      ['Respiração', progress.completedExercises.includes(2) ? Math.max(base, 75) : Math.min(base, 45)],
      ['Precisão', completedCount ? Math.min(100, base + 8) : 0],
      ['Resistência', Math.min(100, Math.round((progress.totalSeconds / 600) * 100))],
    ] as const;
  }, [completedCount, conclusion, progress.completedExercises, progress.totalSeconds]);
