...
function formatSemitones(value: string) {
  const semitones = Number(value || 0);
  return `${semitones > 0 ? '+' : ''}${semitones} semitom(ns)`;
}

function buildSummary(form: FormState) {
  return [
    `🎶 Estudo de Repertório — ${form.songName || 'Música sem nome'}`,
    '',
    `Link: ${form.youtubeUrl || 'Não informado'}`,
    `Tom original: ${form.originalKey}`,
    `Meu tom: ${form.studyKey}`,
    `Transposição: ${formatSemitones(form.semitones)}`,
    form.bpm ? `BPM: ${form.bpm}` : 'BPM: não definido',
    '',
    'Observações:',
    form.notes || 'Sem observações.',
    '',
    'Preparado no Hub Foco em Canto.',
  ].join('\n');
}

function studyToForm(study: Study): FormState {
  return {
    songName: study.song_name,
    youtubeUrl: study.youtube_url,
    originalKey: study.original_key,
    studyKey: study.study_key,
    semitones: String(study.semitone_transposition),
    bpm: study.bpm ? String(study.bpm) : '',
    notes: study.notes || '',
  };
}
...
export function RepertoireStudyClient({ initialStudies }: { initialStudies: Study[] }) {
  ...
  function startNewStudy() {
    ...
    setSummary('');
    setStatus('Novo estudo iniciado.');
  }
  ...
  function loadStudy(study: Study) {
    ...
    setStatus(`Editando: ${study.song_name}`);
  }

  function duplicateStudy(study: Study) {
    setForm({ ...studyToForm(study), songName: `${study.song_name} — nova versão` });
    setSelectedStudyId(null);
    setSummary('');
    setStatus('Versão duplicada. Ajuste o tom e salve como novo estudo.');
  }

  async function saveStudy() {
    setSaving(true);
    setStatus(selectedStudyId ? 'Atualizando estudo...' : 'Salvando estudo...');
    const nextSummary = summary || generate();
    const payload = {
      ...(selectedStudyId ? { id: selectedStudyId } : {}),
      songName: form.songName,
      youtubeUrl: form.youtubeUrl,
      originalKey: form.originalKey,
      studyKey: form.studyKey,
      semitoneTransposition: Number(form.semitones || 0),
    ...
    try {
      const response = await fetch('/api/repertoire-studies', {
        method: selectedStudyId ? 'PUT' : 'POST',
      ...
      setSummary(data.study.summary || nextSummary);
  ...
  async function deleteStudy(study: Study) {
    const confirmed = window.confirm(`Excluir o estudo "${study.song_name}"?`);
    if (!confirmed) return;

    setDeletingId(study.id);
    setStatus('Excluindo estudo...');
    try {
      ...
      setStatus(error instanceof Error ? error.message : 'Não foi possível excluir.');
    } finally {
      setDeletingId(null);
    }
  }
  ...
  async function copySummary() {
    ...
    setStatus('Resumo copiado. Agora é só colar no grupo da banda.');
  }

  return <div className="repertoire-layout">
    <section className="repertoire-panel main-panel">
      <div className="panel-heading">
        <p className="eyebrow">Caderno digital de repertório</p>
        <h2>{selectedStudy ? 'Edite seu estudo salvo' : 'Monte seu estudo da música'}</h2>
        <span>Sem baixar áudio ou vídeo: use o YouTube incorporado como referência.</span>
      </div>
