...
import { AudioTransposePanel } from './audio-transpose-panel';
...
function transposeKey(originalKey: string, semitones: number) { const mode = getKeyMode(originalKey); const list = mode === 'minor' ? minorNotes : majorNotes; const index = list.indexOf(originalKey); return index < 0 ? originalKey : list[normalizeIndex(index + semitones)]; }
...
export function RepertoireStudyClient({ initialStudies }: { initialStudies: Study[] }) {
  ...
  function setOriginalKey(nextKey: string) { setForm((current) => ({ ...current, originalKey: nextKey, studyKey: transposeKey(nextKey, semitoneValue) })); setSummary(''); }
  function adjustTranspose(delta: number) { const next = clamp(semitoneValue + delta, -12, 12); setForm((current) => ({ ...current, semitones: String(next), studyKey: transposeKey(current.originalKey, next) })); setSummary(''); }
  function resetTranspose() { setForm((current) => ({ ...current, semitones: '0', studyKey: transposeKey(current.originalKey, 0) })); setSummary(''); }
  ...
  async function runYouTubeSearch(query: string, silent = false) {
    ...
    setSearching(true); if (!silent) setStatus('Pesquisando no YouTube...');
    try {
      ...
      setResults(data.items || []); setResultsOpen(true);
  ...
  async function searchYouTube(event?: FormEvent) { event?.preventDefault(); await runYouTubeSearch(searchQuery, false); }
  ...
  async function saveStudy() {
    ...
    const payload = { ...(selectedStudyId ? { id: selectedStudyId } : {}), songName: form.songName, youtubeUrl: form.youtubeUrl, originalKey: form.originalKey, studyKey: form.studyKey, semitoneTransposition: semitoneValue, bpm: form.bpm ? Number(form.bpm) : null, notes: form.notes, summary: nextSummary };
    try {
  ...
  return <div className="repertoire-layout">
    <section className="repertoire-panel main-panel">
      <div className="panel-heading"><p className="eyebrow">Caderno digital de repertório</p><h2>{selectedStudy ? 'Edite seu estudo salvo' : 'Monte seu estudo da música'}</h2><span>Pesquise no YouTube dentro do Hub, escolha o vídeo e ajuste o tom de forma simples.</span></div>
