...
type SingLive = { targetIndex: number; detected: NoteName | null; listening: boolean; locked: boolean };
...
function noteY(note: NoteName | null) {
  if (!note) return 50;
  const positions: Record<NoteName, number> = { B: 6, 'A#': 13, A: 20, 'G#': 27, G: 34, 'F#': 41, F: 48, E: 55, 'D#': 62, D: 69, 'C#': 76, C: 83 };
...
export function DailyEarTrainingPlayer({ step, exercise }: { step: DailyTrainingStep; exercise: TrainingExercise }) {
  ...
  const [singLive, setSingLive] = useState<SingLive>({ targetIndex: 0, detected: null, listening: false, locked: false });
  ...
  function mark(value: Status) {
    ...
    window.setTimeout(() => {
      ...
      setRhythmHits([]);
      setSingDone([]);
      setSingLive({ targetIndex: 0, detected: null, listening: false, locked: false });
  ...
  async function listenFor(note: NoteName, index: number) {
    if (singLive.listening || singLive.locked) return;
    setSingLive({ targetIndex: index, detected: null, listening: true, locked: false });
    try {
      ...
      while (performance.now() - started < 2600) {
        ...
        setSingLive((old) => ({ ...old, detected, listening: true }));
        if (detected === note) okFrames += 1;
        else okFrames = Math.max(0, okFrames - 1);
        if (okFrames >= 5) break;
        await sleep(90);
      }
      stream.getTracks().forEach((track) => track.stop());
      await ctx.close().catch(() => null);
      if (okFrames < 5) return mark('wrong');
      const next = [...singDone];
      next[index] = true;
      setSingDone(next);
      setSingLive({ targetIndex: index, detected: note, listening: false, locked: true });
      if (next.filter(Boolean).length === challenge.sing.notes.length) window.setTimeout(() => mark('right'), 450);
      else window.setTimeout(() => setSingLive({ targetIndex: index + 1, detected: null, listening: false, locked: false }), 420);
    } catch {
      setSingLive((old) => ({ ...old, listening: false }));
      setMessage('microfone');
      window.setTimeout(() => setMessage(''), 900);
    }
  }

  async function startSingChallenge() {
    if (singLive.listening) return;
    await playSequence(challenge.sing.notes);
    const nextIndex = singDone[0] ? 1 : 0;
    await listenFor(challenge.sing.notes[nextIndex], nextIndex);
  }
  ...
  return (
    <section className="ear-screen">
      ...
      {key === 'sing' ? <Sing challenge={challenge} startSingChallenge={startSingChallenge} listenFor={listenFor} singDone={singDone} live={singLive} /> : null}
    ...
    <div className="ear-body different-body">
      ...
      <div className="different-grid">
        ...
        <div className="note-choices">
          {challenge.different.first.map((note, index) => <div className="reference-note" key={`ref-${note}-${index}`}><span>♪</span><b>{note}</b></div>)}
          {challenge.different.second.map((note, index) => <button type="button" key={`answer-${note}-${index}`} className={selected[0] === String(index) ? 'selected' : ''} onClick={() => setSelected([String(index)])} aria-label={`Nota ${index + 1} do segundo conjunto`}><span>♪</span><i /></button>)}
        </div>
      </div>
...
function Sing({ challenge, startSingChallenge, listenFor, singDone, live }: { challenge: Challenge; startSingChallenge: () => Promise<void>; listenFor: (note: NoteName, index: number) => void; singDone: boolean[]; live: SingLive }) {
  return (
    <div className="ear-body sing-body">
      <h2>Ouça e cante as mesmas notas</h2>
      <div className="pitch-board sing-board">
        {['B', 'A#', 'A', 'G#', 'G', 'F#', 'F', 'E', 'D#', 'D', 'C#', 'C'].map((note) => <div key={note}><span>{note}</span><i /></div>)}
        {challenge.sing.notes.map((note, index) => <button type="button" key={`${note}-${index}`} className={`target-note target-${index} ${singDone[index] ? 'done' : ''} ${live.targetIndex === index ? 'current' : ''}`} style={{ top: `${noteY(note)}%`, left: `${index ? 74 : 32}%` }} onClick={() => listenFor(note, index)}>{singDone[index] ? '✓' : note}</button>)}
        {live.detected ? <span className="voice-dot" style={{ top: `${noteY(live.detected)}%`, left: `${live.targetIndex ? 74 : 32}%` }} /> : null}
      </div>
      <button type="button" className="sound-big" onClick={startSingChallenge}>🔊</button>
      <small className="sing-hint">{live.listening ? 'centralize a bolinha' : 'toque e cante'}</small>
    </div>
  );
}
