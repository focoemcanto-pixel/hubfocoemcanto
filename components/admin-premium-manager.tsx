    ...
    <div className="premium-client-column">
      ...
      <article className="premium-panel premium-subscriber-panel">
        ...
        <div className="premium-subscriber-list">
          {visible.map((row) => <article className={`premium-subscriber-card ${row.state.tone} renewal-${row.renewalTone}`} key={row.key}><div className="premium-member-main"><span className={`premium-status ${row.state.tone}`}>{row.state.label}</span><h3>{row.student.name || 'Sem nome'}</h3><p>{row.student.email}</p><small>{row.student.whatsapp || 'WhatsApp não informado'}</small></div><div className="premium-renewal-box"><span>{row.state.active ? 'Próxima renovação' : 'Venceu em'}</span><strong>{row.renewalDateLabel}</strong><em>{row.state.active ? row.renewalLabel : row.accessReason}</em></div><div className="premium-plan-box"><span>Financeiro</span><strong>{row.amountLabel} / mês</strong><small>{row.method} · {row.productName}</small></div><div className="premium-event-box"><span>Último webhook</span><strong className={`event-${row.lastEventTone}`}>{row.lastEventLabel}</strong><small>{row.lastEventDate}</small></div><div className="premium-actions premium-actions-console"><span className={row.state.remove ? 'remove-tag' : row.state.tone === 'late' ? 'late-tag' : row.estimated ? 'late-tag' : 'keep-tag'}>{row.state.remove ? 'remover do grupo' : row.estimated ? 'data estimada' : row.state.action}</span>{row.whatsapp ? <a className="button secondary" href={row.whatsapp} target="_blank" rel="noreferrer">WhatsApp</a> : null}</div></article>)}
