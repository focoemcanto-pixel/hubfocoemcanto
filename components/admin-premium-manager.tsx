...
export function AdminPremiumManager({ rows, removeEmails, lateEmails }: Props) {
  ...
  const counts = useMemo(() => {
    ...
    rows.forEach((row) => { if (row.state.active) base.ativos += 1; if (row.renewalTone === 'late' || row.renewalTone === 'pending') base.vencendo += 1; if (row.estimated) base.revisar += 1; if (row.state.tone === 'late') base.atrasados += 1; if (row.state.tone === 'pending') base.pendentes += 1; if (row.state.remove) base.remover += 1; });
    return base;
  }, [rows]);
  const courseCounts = useMemo(() => {
    const base: Record<string, number> = { todos: rows.length };
    COURSE_ACCESS.forEach((course) => { base[course.key] = rows.filter((row) => row.courseKey === course.key).length; });
    return base;
  }, [rows]);
  const visible = useMemo(() => {
    const term = query.trim().toLowerCase();
    return rows.filter((row) => {
      const matchesStatus = filter === 'todos' || rowGroup(row) === filter || (filter === 'ativos' && row.state.active);
  ...
  return (
    <div className="premium-client-column">
      ...
      <section className="premium-filter-bar premium-filter-bar-console no-refresh-filter course-filter-pills">
        ...
        {COURSE_ACCESS.slice(0, 5).map((course) => <button className={courseFilter === course.key ? 'active' : ''} key={course.key} onClick={() => setCourseFilter(course.key)} type="button">{course.shortLabel}<span>{courseCounts[course.key] || 0}</span></button>)}
      </section>
      <input className="student-live-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar assinatura por aluno, e-mail, WhatsApp ou curso..." />
      ...
      <article className="premium-panel premium-subscriber-panel">
        <div className="section-heading"><div><p className="eyebrow">Gestão de acesso</p><h2>Lista de assinaturas</h2></div><span className="pill">{visible.length} registros</span></div>
        <div className="premium-subscriber-list">
          {visible.map((row) => <article className={`premium-subscriber-card ${row.state.tone} renewal-${row.renewalTone}`} key={row.key}><div className="premium-member-main"><span className={`premium-status ${row.state.tone}`}>{row.courseLabel || 'Produto'} · {row.state.label}</span><h3>{row.student.name || 'Sem nome'}</h3><p>{row.student.email}</p><small>{row.student.whatsapp || 'WhatsApp não informado'}</small></div><div className="premium-renewal-box"><span>{row.state.active ? 'Próxima renovação' : 'Venceu em'}</span><strong>{row.renewalDateLabel}</strong><em>{row.state.active ? row.renewalLabel : row.accessReason}</em></div><div className="premium-plan-box"><span>Financeiro</span><strong>{row.amountLabel} / mês</strong><small>{row.method} · {row.productName}</small></div><div className="premium-event-box"><span>Último webhook</span><strong className={`event-${row.lastEventTone}`}>{row.lastEventLabel}</strong><small>{row.lastEventDate}</small></div><div className="premium-actions premium-actions-console"><span className={row.state.remove ? 'remove-tag' : row.state.tone === 'late' ? 'late-tag' : row.estimated ? 'late-tag' : 'keep-tag'}>{row.state.remove ? 'bloquear acesso' : row.estimated ? 'data estimada' : row.state.action}</span>{row.whatsapp ? <a className="button secondary" href={row.whatsapp} target="_blank" rel="noreferrer">WhatsApp</a> : null}</div></article>)}
