...
function brl(value: number) { return new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', notation: value >= 10000 ? 'compact' : 'standard', maximumFractionDigits: value >= 10000 ? 1 : 2 }).format(value || 0); }
...
export function AdminAnalyticsPanel({ metrics }: { metrics: Metrics }) {
  ...
  return <section className="analytics-admin-panel"><style dangerouslySetInnerHTML={{ __html: css }} />
    <div className="exec-grid">
      <article className="exec-card"><span>Receita hoje</span><strong>{brl(metrics.revenueToday)}</strong><p>{metrics.newSubscriptions} nova(s) assinatura(s)</p></article>
      <article className="exec-card"><span>Receita mês</span><strong>{brl(metrics.revenueMonth)}</strong><p>{metrics.renewals} renovação(ões) no período</p></article>
      <article className="exec-card"><span>MRR</span><strong>{brl(metrics.mrr)}</strong><p>{metrics.activeSubscribers} assinantes ativos</p></article>
      ...
      <article className="exec-card"><span>Ticket médio</span><strong>{brl(metrics.avgTicket)}</strong><p>baseado em compras ativas</p></article>
    ...
    <section className="health-strip">
      <div className="health-pill"><span>DAU</span><strong>{metrics.dau}</strong></div>
      <div className="health-pill"><span>WAU</span><strong>{metrics.wau}</strong></div>
      <div className="health-pill"><span>MAU</span><strong>{metrics.mau}</strong></div>
    </section>

    <div className="analytics-two">
      <article className="funnel-card"><p className="eyebrow">Funil {metrics.periodLabel}</p><h2>Jornada do aluno</h2>{metrics.funnel.map((step) => <div className="funnel-row" key={step.key}><label>{step.label}</label><div className="bar"><i style={{ width: `${Math.max(3, (step.value / max) * 100)}%` }} /></div><b>{step.value}</b><small>{step.rate !== undefined ? pct(step.rate) : ''}</small></div>)}</article>
      <article className="opportunity-card"><p className="eyebrow">🔥 Oportunidades</p><h2>Onde mais vende VIP</h2>{metrics.topBlocked.length ? metrics.topBlocked.map((item) => <div className="top-row" key={item.label}><span>{item.label}</span><b>{item.value}</b></div>) : <p className="muted">Sem bloqueios registrados ainda.</p>}</article>
    </div>

    <div className="analytics-three">
      <article className="products-card"><p className="eyebrow">Produtos</p><h2>Receita por produto</h2>{metrics.products.length ? metrics.products.map((item) => <div className="product-row" key={item.label}><div><strong>{item.label}</strong><small>{item.active} ativo(s) · conversão {pct(item.conversion)}</small></div><b>{brl(item.revenue)}</b></div>) : <p className="muted">Sem produtos no período.</p>}</article>
      <article className="activity-card"><p className="eyebrow">Tempo real</p><h2>Atividade recente</h2>{metrics.recent.length ? metrics.recent.map((item, index) => <div className={`activity-row ${item.tone}`} key={`${item.label}-${index}`}><span className="activity-dot" /><div><strong>{item.label}</strong><small>{item.detail}</small></div></div>) : <p className="muted">Sem eventos recentes.</p>}</article>
    </div>
  </section>;
}
