...
export function VocalRangeTest({ profileId, authUserId, initialProfile }: Props) {
  ...
  useEffect(() => { stepRef.current = step; setCaptureReview(false); }, [step]);
  useEffect(() => () => stopMic(), []);
  ...
  function goBackFromCapture() { setCaptureReview(false); if (step === 'lowest') { resetAll(); } else { setStep('lowest'); } }
  function confirmCurrentCapture() {
    if (step === 'lowest') setStep('highest');
    if (step === 'highest') setStep('confirm-range');
  }
  async function save() {
  ...
  const currentCapture = step === 'lowest' ? lowest : highest;
  const captureRange = lowest && highest ? `${lowest.note} — ${highest.note}` : currentCapture?.note;
  ...
  return <div className="vocal-test-shell">
    ...
    {(step === 'lowest' || step === 'highest') && <section className="vocal-stage grid range-capture"><VocalNoteMeter currentMidi={currentMidi} lowestMidi={lowest?.midi} highestMidi={highest?.midi} /><button className="capture-back" onClick={goBackFromCapture} aria-label="Sair da avaliação">←</button><div className="range-copy"><p className="eyebrow">{step === 'lowest' ? 'Etapa 1/3' : 'Etapa 2/3'}</p><h1>{step === 'lowest' ? 'Busque sua nota mais grave' : 'Busque sua nota mais aguda'}</h1><p className="range-helper">{currentCapture ? `${step === 'lowest' ? 'Mais grave' : 'Mais aguda'}: ${currentCapture.note}` : 'Cante e sustente uma nota'}</p>{captureReview && currentCapture ? <div className="capture-result"><span>{step === 'lowest' ? 'Grave encontrado' : 'Agudo encontrado'}</span><strong>{captureRange}</strong><small>{step === 'lowest' ? 'Agora siga para encontrar seu agudo.' : 'Confirme sua extensão e avance.'}</small></div> : null}<div className="actions"><button disabled={!currentCapture} onClick={() => captureReview ? confirmCurrentCapture() : setCaptureReview(true)}>{captureReview ? 'Seguir para próxima etapa' : 'Pressione quando terminar'}</button><button onClick={() => { setCaptureReview(false); step === 'lowest' ? setLowest(null) : setHighest(null); }}><RefreshCw /> Refazer</button></div></div></section>}
