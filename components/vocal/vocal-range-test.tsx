...
export function VocalRangeTest({ profileId, authUserId, initialProfile }: Props) {
  ...
  const stableNote = stableMidi != null ? midiToBrazilianNoteName(stableMidi) : null;
  ...
  return <div className="vocal-test-shell">
    ...
    {step === 'lowest' && <section className="vocal-stage grid range-capture" onClick={finishMapping}>
      <VocalNoteMeter currentMidi={currentMidi} lowestMidi={lowest?.midi} highestMidi={highest?.midi} />
      <button className="capture-back" onClick={(event) => { event.stopPropagation(); resetAll(); }} aria-label="Sair da avaliação">←</button>
      <div className="range-copy">
        <div className="capture-top-card">
          <p className="eyebrow">Etapa 1/3 · Extensão</p>
          <h1>Mapeie sua extensão vocal</h1>
          <p className="range-helper">Cante do grave ao agudo sem forçar. Eu marco automaticamente seu limite mais grave e mais agudo.</p>
        </div>
        <div className="live-note-card" aria-live="polite">
          <span>Nota agora</span>
          <strong>{liveNote}</strong>
          <small>{currentFrequency ? `${Math.round(currentFrequency)} Hz` : 'Aguardando sua voz'}{stableNote ? ` · estável em ${stableNote}` : ''}</small>
        </div>
        {captureReview && <div className="capture-result"><span>Extensão captada</span><strong>{captureRange}</strong><small>Confirme para seguir ou tente novamente.</small></div>}
...
.vocal-stage{max-width:1120px;margin:0 auto;border:1px solid rgba(255,255,255,.12);border-radius:30px;background:linear-gradient(145deg,rgba(255,255,255,.08),rgba(255,255,255,.025));box-shadow:0 28px 90px rgba(0,0,0,.45);padding:24px}.vocal-stage.hero{text-align:center;display:grid;gap:18px;place-items:center}.vocal-stage.grid,.vocal-stage.tessitura-grid{display:grid;grid-template-columns:minmax(360px,1.12fr) minmax(0,.88fr);gap:22px;align-items:stretch}.range-copy{align-self:center}.capture-top-card{display:grid;gap:10px}.capture-back,.live-note-card{display:none}.capture-result{margin:20px 0;padding:18px;border:1px solid rgba(255,255,255,.12);border-radius:22px;background:rgba(0,0,0,.25);display:grid;gap:4px}.capture-result span{color:rgba(255,255,255,.58);font-size:12px;text-transform:uppercase;letter-spacing:.14em}.capture-result strong{font-size:34px;color:#67e8f9}.capture-result small{color:#f5c76b!important}.tessitura-copy{text-align:center;display:grid;gap:14px;place-items:center;align-content:center}.vocal-stage h1{margin:0;font-size:clamp(34px,7vw,58px);letter-spacing:-.06em}.vocal-stage p{margin:0;color:rgba(255,255,255,.72);font-size:18px;line-height:1.45}.eyebrow{color:#67e8f9!important;font-size:13px!important;text-transform:uppercase;letter-spacing:.18em;font-weight:1000}.tip,.vocal-stage small{color:#f5c76b!important}.vocal-stage button,.vocal-stage a{border:0;border-radius:18px;padding:15px 18px;background:linear-gradient(180deg,#ffe29a,#e8ad34);color:#120d05;font-weight:950;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;gap:8px;min-height:54px}.vocal-stage button:disabled{opacity:.45}.actions{display:flex;flex-wrap:wrap;gap:10px;justify-content:center}.actions button:nth-child(n+2){background:rgba(255,255,255,.09);color:#fff;border:1px solid rgba(255,255,255,.12)}.readout{margin:22px 0;padding:20px;border-radius:24px;background:rgba(0,0,0,.28);display:grid;gap:6px}.readout strong,.range-big{font-size:54px;font-weight:1000;letter-spacing:-.05em;color:#67e8f9}.readout span{color:#f5c76b;font-weight:800}.error{color:#ff8a8a}.save-message{color:#86efac}.choice-grid,.result-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;width:100%;max-width:760px}.choice-grid button{background:rgba(255,255,255,.08);color:#fff;border:1px solid rgba(255,255,255,.14)}.choice-grid .selected{border-color:#67e8f9;box-shadow:0 0 0 3px rgba(103,232,249,.12)}.result-grid{grid-template-columns:repeat(2,1fr)}.result-grid article{border:1px solid rgba(255,255,255,.1);border-radius:20px;padding:18px;background:rgba(0,0,0,.25)}.result-grid span{display:block;color:rgba(255,255,255,.62);margin-bottom:7px}.result-grid strong{font-size:24px}blockquote{font-size:22px;color:#fff}
body.vocal-capture-active{overflow:hidden!important}body.vocal-capture-active nav,body.vocal-capture-active footer,body.vocal-capture-active [class*="bottom"],body.vocal-capture-active [class*="Bottom"],body.vocal-capture-active [class*="tab-bar"],body.vocal-capture-active [class*="TabBar"],body.vocal-capture-active [class*="mobile-nav"],body.vocal-capture-active [class*="MobileNav"]{display:none!important}
@media(max-width:760px){
  .vocal-test-shell{padding:0;background:#050507;min-height:100dvh;overflow:hidden}.vocal-stage.grid.range-capture{position:fixed;inset:0;z-index:2147483647;min-height:100dvh;width:100%;max-width:none;margin:0;border:0;border-radius:0;box-shadow:none;background:radial-gradient(circle at 78% 22%,rgba(42,204,221,.18),transparent 28%),radial-gradient(circle at 55% 76%,rgba(245,199,107,.13),transparent 32%),linear-gradient(180deg,#07080a,#050507);padding:0;display:block;overflow:hidden}.range-capture .vocal-meter{position:absolute;inset:0!important;width:100%;height:100dvh!important;min-height:100dvh!important;margin:0;border-radius:0!important}.range-copy{position:absolute;inset:0;z-index:20;pointer-events:none;text-align:left}.capture-back{display:grid;place-items:center;position:absolute;left:18px;top:calc(16px + env(safe-area-inset-top));z-index:40;width:44px;height:44px;min-height:44px;border-radius:999px!important;background:rgba(255,255,255,.075)!important;color:#fff!important;border:1px solid rgba(255,255,255,.14)!important;padding:0!important;font-size:22px!important;backdrop-filter:blur(14px)}.capture-top-card{position:absolute;left:76px;right:16px;top:calc(14px + env(safe-area-inset-top));padding:14px 15px;border:1px solid rgba(255,255,255,.1);border-radius:22px;background:linear-gradient(145deg,rgba(8,10,14,.78),rgba(8,10,14,.42));box-shadow:0 18px 60px rgba(0,0,0,.32);backdrop-filter:blur(18px)}.capture-top-card .eyebrow{font-size:9px!important;letter-spacing:.16em;color:rgba(103,232,249,.9)!important}.capture-top-card h1{font-size:clamp(18px,5.2vw,24px)!important;line-height:1.02!important;letter-spacing:-.04em;color:rgba(255,255,255,.95);text-shadow:0 0 18px rgba(0,0,0,.65)}.range-helper{display:block;color:rgba(255,255,255,.58)!important;font-size:11px!important;line-height:1.25!important}.live-note-card{display:grid;position:absolute;right:16px;top:calc(150px + env(safe-area-inset-top));z-index:24;width:112px;padding:11px;border:1px solid rgba(255,255,255,.1);border-radius:20px;background:rgba(0,0,0,.42);box-shadow:0 18px 48px rgba(0,0,0,.28);backdrop-filter:blur(16px);text-align:center}.live-note-card span{font-size:9px;text-transform:uppercase;letter-spacing:.14em;color:rgba(255,255,255,.5)}.live-note-card strong{font-size:30px;line-height:1;color:#67e8f9;letter-spacing:-.05em}.live-note-card small{font-size:10px!important;line-height:1.15;color:rgba(245,199,107,.86)!important}.capture-result{position:absolute!important;left:18px!important;right:18px!important;bottom:calc(124px + env(safe-area-inset-bottom))!important;z-index:24;gap:4px;text-align:center;border:1px solid rgba(103,232,249,.18);border-radius:24px;background:linear-gradient(145deg,rgba(4,8,12,.82),rgba(4,8,12,.58));padding:13px 14px;backdrop-filter:blur(18px);box-shadow:0 20px 70px rgba(0,0,0,.46)}.capture-result span{font-size:10px;text-transform:uppercase;letter-spacing:.14em;color:rgba(255,255,255,.58)}.capture-result strong{font-size:clamp(24px,7vw,32px);line-height:1;color:#67e8f9}.capture-result small{font-size:11px!important;color:rgba(255,255,255,.62)!important}.range-copy .actions{position:absolute!important;left:18px!important;right:18px!important;bottom:calc(26px + env(safe-area-inset-bottom))!important;top:auto!important;display:grid!important;grid-template-columns:1fr 50px!important;gap:9px!important;pointer-events:auto!important}.range-copy .actions button:first-child{width:100%;min-height:52px!important;border-radius:999px;background:linear-gradient(180deg,#ffe29a,#e8ad34)!important;color:#120d05!important;font-size:13px!important;line-height:1.05;text-align:center;font-weight:1000;padding:0 16px!important;box-shadow:0 16px 38px rgba(232,173,52,.2)}.range-copy .actions button:first-child svg{display:none}.range-copy .actions button:nth-child(2){min-height:52px!important;width:50px!important;border-radius:999px;background:rgba(255,255,255,.08)!important;color:#fff!important;border:1px solid rgba(255,255,255,.14)!important;font-size:0!important;padding:0!important;backdrop-filter:blur(16px)}.range-copy .actions button:nth-child(2) svg{width:20px;height:20px;margin:0}.premium-vocal-meter .marker{height:2px!important;width:34px!important;opacity:.78}.premium-vocal-meter .marker.high,.premium-vocal-meter .marker.low{box-shadow:none!important}.premium-vocal-meter .vocal-meter-scale:after{opacity:.45!important}.vocal-stage:not(.range-capture),.vocal-stage.tessitura-grid{margin:18px 14px;border-radius:24px;padding:18px}.vocal-stage.tessitura-grid{grid-template-columns:1fr}.tessitura-copy{order:2}.tessitura-grid>.vocal-meter{order:1}.choice-grid,.result-grid{grid-template-columns:1fr}
}
`;
