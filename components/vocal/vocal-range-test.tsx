...
import { autoCorrelate, classifyVoice, frequencyToMidi, midiToFrequency, midiToNoteName } from '@/lib/audio/pitch';
...
export function VocalRangeTest({ profileId, authUserId, initialProfile }: Props) {
  ...
  const [lowest, setLowest] = useState<Captured | null>(initialProfile?.lowest_midi ? { note: initialProfile.lowest_note, midi: initialProfile.lowest_midi, frequency: Number(initialProfile.lowest_frequency || midiToFrequency(initialProfile.lowest_midi)) } : null);
  const [highest, setHighest] = useState<Captured | null>(initialProfile?.highest_midi ? { note: initialProfile.highest_note, midi: initialProfile.highest_midi, frequency: Number(initialProfile.highest_frequency || midiToFrequency(initialProfile.highest_midi)) } : null);
  ...
  function capture(kind: 'low' | 'high') { if (!stableMidi || !currentFrequency) return; const item = { midi: stableMidi, note: midiToNoteName(stableMidi), frequency: currentFrequency }; kind === 'low' ? setLowest((old) => !old || item.midi < old.midi ? item : old) : setHighest((old) => !old || item.midi > old.midi ? item : old); }
