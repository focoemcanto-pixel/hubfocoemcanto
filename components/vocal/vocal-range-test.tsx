...
export function VocalRangeTest({ profileId, authUserId, initialProfile }: Props) {
  ...
  return <div className="vocal-test-shell">
    ...
    {(step === 'lowest' || step === 'highest') && <section className="vocal-stage grid range-capture"><VocalNoteMeter currentMidi={currentMidi} lowestMidi={lowest?.midi} highestMidi={highest?.midi} /><div className="range-copy"><p className="eyebrow">{step === 'lowest' ? 'Etapa 1/3' : 'Etapa 2/3'}</p><h1>Cante e alcance suas<br />Notas Mais Altas e Mais Baixas</h1><p className="range-helper">{step === 'lowest' ? '← sua mais baixa' : '← sua mais alta'}</p><div className="readout"><strong>{currentMidi ? midiToBrazilianNoteName(currentMidi) : '—'}</strong><span>{currentFrequency ? `${currentFrequency.toFixed(1)} Hz` : 'Aguardando voz'}</span><small>{step === 'lowest' ? (lowest ? `Mais grave registrada: ${lowest.note}` : 'Sustente a voz para o sistema registrar na régua') : (highest ? `Mais aguda registrada: ${highest.note}` : 'Sustente a voz para o sistema registrar na régua')}</small></div><div className="actions"><button disabled={step === 'lowest' ? !lowest : !highest} onClick={() => step === 'lowest' ? setStep('highest') : setStep('confirm-range')}><Check /> Pressione aqui se terminou</button><button onClick={() => step === 'lowest' ? setLowest(null) : setHighest(null)}><RefreshCw /> Refazer</button></div></div></section>}
