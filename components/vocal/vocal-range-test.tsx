...
export function VocalRangeTest({ profileId, authUserId, initialProfile }: Props) {
  ...
  return <div className="vocal-test-shell">
    ...
    {step === 'lowest' && <section className="vocal-stage grid range-capture">
      ...
      <MobileRangeCapture currentMidi={currentMidi} lowestMidi={lowest?.midi} highestMidi={highest?.midi} liveNote={liveNote} captureReady={captureReady} captureReview={captureReview} captureRange={captureRange} onBack={resetAll} onRetry={retryMapping} onPrimary={() => captureReview ? confirmRangeAndGoToTessitura() : finishMapping()} />
    </section>}
...
function MobileRangeCapture({ currentMidi, lowestMidi, highestMidi, liveNote, captureReady, captureReview, captureRange, onBack, onRetry, onPrimary }: any) {
  ...
  return <div className="mvr-shell">
    <div className="mvr-ruler">
      ...
      {highestMidi != null && <b className="mvr-limit mvr-limit-high" style={{ top: `${percent(highestMidi)}%` }}><em>{midiToBrazilianNoteName(highestMidi)}</em></b>}
      {lowestMidi != null && <b className="mvr-limit mvr-limit-low" style={{ top: `${percent(lowestMidi)}%` }}><em>{midiToBrazilianNoteName(lowestMidi)}</em></b>}
...
function Tessitura({ title, text, midi, phrase, instruction, downLabel, icon, onPlay, onMove, onConfirm }: any) {
  const note = midiToBrazilianNoteName(midi);
  return <section className="vocal-stage tessitura-stage">
    <div className="tessitura-figure" aria-hidden="true"><img src="/vocal/vocal-body-base.png" alt="" draggable={false} /><span /></div>
    <div className="tessitura-copy">
      <p className="tessitura-step">ETAPA 2/3</p>
      <h1>{title}</h1>
      <p className="tessitura-lead">{text}</p>
      <div className="tessitura-note-card">
        <div><span>NOTA SUGERIDA</span><strong>{note}</strong></div>
        <button type="button" onClick={() => onPlay(midi)}><Play /><b>{note}</b><small>Tocar nota</small></button>
      </div>
      <div className="tessitura-sing-card"><i><Mic2 size={28} /></i><div><strong>{instruction || 'Cante a palavra na nota sugerida'}</strong><p>Diga “{phrase || 'eu consigo'}” com qualidade e conforto.</p></div></div>
      <div className="tessitura-tuner"><span>MONITORAMENTO DE AFINAÇÃO</span><div className="tessitura-scale"><b /><i /></div><div className="tessitura-scale-labels"><small>Abaixo</small><small>Na nota</small><small>Acima</small></div><p><Check size={18} /> Afinado! Mantenha a nota...</p></div>
      <div className="actions tessitura-actions"><button onClick={onConfirm}><Check /> Consegui com conforto</button><button onClick={onMove}>{icon === 'up' ? <ArrowUp /> : <ArrowDown />} Difícil / sem qualidade</button><button onClick={onMove}>{icon === 'up' ? <ArrowUp /> : <ArrowDown />} {downLabel}</button></div>
      <div className="tessitura-tip"><Sparkles size={20} /><p>Dica: mantenha o volume moderado e foque na qualidade do som.</p></div>
    </div>
  </section>;
}
