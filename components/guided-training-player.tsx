...
export function GuidedTrainingPlayer({ exercise, autoStart = false }: { exercise: TrainingExercise; compact?: boolean; autoStart?: boolean }) {
  ...
  const registerGlowRef = useRef<HTMLSpanElement | null>(null);
  ...
  const cssVars = { '--voice-y': `${yFromMidi(tuner.midi)}%`, '--voice-visible': tuner.midi == null ? '0' : '1', '--progress': String(progress) } as Vars;
  ...
  useEffect(() => { if (!playing || !controls) return; const id = window.setTimeout(() => setControls(false), 2400); return () => window.clearTimeout(id); }, [playing, controls]);
  ...
  function playClick(context: AudioContext, at: number, strong = false) {
    ...
    oscillator.type = 'square'; oscillator.frequency.value = strong ? 1320 : 880;
    gain.gain.setValueAtTime(0.0001, at); gain.gain.exponentialRampToValueAtTime(strong ? 0.1 : 0.055, at + 0.006); gain.gain.exponentialRampToValueAtTime(0.0001, at + 0.055);
    oscillator.connect(gain); gain.connect(context.destination); oscillator.start(at); oscillator.stop(at + 0.07); oscillatorsRef.current.push(oscillator);
  }
  function scheduleAudioWindow(fromBeat: number, toBeat: number) {
    const context = getAudioContext(); const anchor = audioAnchorRef.current;
    if (!context || anchor == null || toBeat <= nextAudioBeatRef.current) return;
    context.resume().catch(() => null);
    const secondsPerBeat = 60 / bpm; const now = context.currentTime; const startBeat = Math.max(fromBeat, nextAudioBeatRef.current); const endBeat = Math.min(toBeat, totalBeats);
    for (let beatIndex = Math.max(0, Math.ceil(startBeat)); beatIndex < endBeat; beatIndex += 1) { const at = anchor + beatIndex * secondsPerBeat; if (at > now + 0.015) playClick(context, at, beatIndex % 4 === 0); }
    beatNotes.forEach((note) => { if (note.midi == null || note.startBeat < startBeat || note.startBeat >= endBeat) return; const startAt = anchor + note.startBeat * secondsPerBeat; const endAt = anchor + note.endBeat * secondsPerBeat; if (endAt <= now + 0.02) return; void playPianoSample(context, note.midi, Math.max(now + 0.015, startAt), Math.max(now + 0.24, endAt), note.mode === 'guide' ? 0.86 : 1.02); });
    nextAudioBeatRef.current = endBeat;
  }
  async function startPlayback() {
    if (playing || count) { stopAudio(); setPlaying(false); setControls(true); setVoiceTrail([]); return; }
    void startMic(); const context = getAudioContext(); if (!context) { setPlaying(true); return; }
    await context.resume().catch(() => null); stopAudio(); setVoiceTrail([]);
    const start = currentBeat >= totalBeats ? 0 : currentBeat;
    void preloadPianoSamples(context, beatNotes.filter((note) => note.midi != null && note.startBeat >= start && note.startBeat <= start + 36).map((note) => note.midi as number));
  ...
  function moveRegisterGlow(midi: number | null, visible: boolean) { const el = registerGlowRef.current; if (!el) return; el.style.opacity = visible ? '1' : '0.42'; el.style.top = `${registerYFromMidi(midi)}%`; el.style.transform = `translate(-50%,-50%) scale(${visible ? 1.05 : 0.82})`; }
  ...
  return (
    <section className={`exercise-experience ${controls ? 'controls-on' : ''}`} style={cssVars} onPointerDown={showControls}>
      ...
      <div className="exercise-body" aria-hidden="true"><img src="/vocal/vocal-body-base.png" alt="" /><span ref={registerGlowRef} /></div>
      ...
      <div className="control-overlay" onPointerDown={(event) => event.stopPropagation()}><button className="back-btn" type="button" onClick={() => history.back()}>←</button><div className="bpm-control"><button type="button" onClick={() => adjustBpm(-2)}>−</button><span><b>{bpm}</b><small>BPM</small></span><button type="button" onClick={() => adjustBpm(2)}>+</button></div><div className="control-row"><button type="button" onClick={startPlayback}>▶<span>Ouvir base</span></button><button className="main-btn" type="button" onClick={startPlayback}>{playing ? 'Ⅱ' : count ? '×' : '▶'}<span>{playing ? 'Pausar' : count ? 'Cancelar' : 'Iniciar'}</span></button><button type="button" onClick={restart}>↻<span>Repetir</span></button></div></div>
    </section>
  );
}

const css = `.exercise-experience{position:relative;min-height:100dvh;overflow:hidden;color:#fff;background:#050607;touch-action:manipulation;isolation:isolate}.exercise-bg{position:absolute;inset:0;background:radial-gradient(circle at 54% 44%,rgba(245,199,107,.11),transparent 28%),linear-gradient(180deg,#111419 0%,#050608 55%,#030405 100%)}.exercise-bg:after{content:'';position:absolute;inset:0;background-image:linear-gradient(90deg,rgba(255,255,255,.032) 1px,transparent 1px),linear-gradient(rgba(255,255,255,.024) 1px,transparent 1px);background-size:12.5vw 100%,100% 7.2vh;mask-image:linear-gradient(90deg,rgba(0,0,0,.72),#000 24%,#000 82%,rgba(0,0,0,.35))}.minimal-top{position:absolute;left:64px;right:20px;top:2.3dvh;z-index:18;text-align:center;opacity:.78;pointer-events:none}.minimal-top strong{display:block;font-size:clamp(14px,2dvh,18px);font-weight:950;letter-spacing:.12em}.minimal-top span{display:block;margin-top:3px;font-size:11px;color:rgba(255,255,255,.56)}.progress-line{position:absolute;left:72px;right:48px;top:7.1dvh;height:3px;border-radius:999px;background:rgba(255,255,255,.16);z-index:18;overflow:hidden}.progress-line i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,#f7c763,#ffd986)}.exercise-body{position:absolute;z-index:3;left:10%;right:-8%;top:9dvh;bottom:-3dvh;opacity:.28;pointer-events:none;display:grid;place-items:center;background:transparent;mix-blend-mode:screen;mask-image:radial-gradient(ellipse at 53% 54%,#000 0 52%,rgba(0,0,0,.72) 72%,transparent 100%)}.exercise-body img{width:112%;height:112%;object-fit:contain;object-position:center bottom;opacity:.58;filter:drop-shadow(0 0 28px rgba(255,255,255,.035))}.exercise-body span{position:absolute;left:50%;top:58%;width:48%;aspect-ratio:1;border-radius:50%;background:radial-gradient(circle,rgba(245,199,107,.62) 0 11%,rgba(245,199,107,.25) 32%,rgba(245,199,107,.09) 52%,transparent 73%);filter:blur(18px);opacity:.42;transform:translate(-50%,-50%) scale(.82);transition:top .11s linear,opacity .16s ease,transform .16s ease;mix-blend-mode:screen}.wave-glow{position:absolute;left:16%;right:4%;top:35dvh;height:12dvh;z-index:2;opacity:.26;background:repeating-linear-gradient(90deg,transparent 0 8px,rgba(245,199,107,.46) 9px 12px,transparent 13px 20px);mask-image:radial-gradient(ellipse at center,#000 0 24%,rgba(0,0,0,.55) 42%,transparent 80%);filter:drop-shadow(0 0 20px rgba(245,199,107,.18))}.scale-stage{position:absolute;inset:8.6dvh 0 10.8dvh 0;z-index:5}.pitch-ruler{position:absolute;left:4px;top:0;bottom:0;width:56px;display:flex;flex-direction:column;justify-content:space-between;font-size:clamp(6px,.78dvh,10px);font-weight:800;color:rgba(255,255,255,.35);line-height:1}.pitch-ruler span{position:relative}.pitch-ruler span:after{content:'';position:absolute;left:28px;top:50%;width:26px;height:1px;background:rgba(255,255,255,.14)}.pitch-ruler .in-range{color:rgba(245,199,107,.76)}.pitch-ruler .active{color:#ff3131!important;font-size:1.28em;text-shadow:0 0 14px rgba(255,49,49,.85)}.timeline-layer{position:absolute;left:58px;right:0;top:0;bottom:0;overflow:hidden}.hit-line{position:absolute;left:15%;top:0;bottom:0;width:1px;background:linear-gradient(180deg,transparent,#f8ce75 14%,#f8ce75 86%,transparent);box-shadow:0 0 18px rgba(245,199,107,.42);z-index:2}.target-note{position:absolute;height:clamp(8px,1.05dvh,13px);border-radius:999px;background:linear-gradient(180deg,#fff,#e7e1d5);box-shadow:0 0 16px rgba(255,255,255,.34),0 0 28px rgba(245,199,107,.14);transform:translateY(-50%);z-index:4}.voice-trail{position:absolute;inset:0;z-index:7;pointer-events:none}.voice-trail span{position:absolute;height:11px;border-radius:999px;background:linear-gradient(90deg,rgba(255,66,66,0),rgba(255,65,65,.9),rgba(255,245,225,.8));box-shadow:0 0 18px rgba(255,0,0,.68)}.voice-brush{position:absolute;left:15%;top:var(--voice-y);width:21px;height:21px;border-radius:50%;background:radial-gradient(circle,#ffd7d7 0 18%,#ff2020 46%,rgba(255,0,0,.32) 72%,transparent 100%);filter:drop-shadow(0 0 16px rgba(255,0,0,.95));transform:translate(-50%,-50%);opacity:0;z-index:8;will-change:top,opacity}.voice-brush i{position:absolute;inset:6px;border-radius:inherit;background:#fff}.feedback-text{position:absolute;left:80px;right:30px;top:52%;z-index:9;text-align:center;color:#e4b85d;font-size:clamp(23px,4.2vw,38px);font-weight:950;text-shadow:0 0 22px rgba(245,199,107,.36);pointer-events:none}.feedback-text.good{color:#e4b85d}.execution-label{position:absolute;left:24px;right:24px;bottom:calc(max(18px,env(safe-area-inset-bottom)) + 42px);z-index:11;text-align:center;font-size:clamp(28px,7vw,58px);font-weight:950;color:rgba(255,255,255,.9);text-shadow:0 0 26px rgba(255,255,255,.18);pointer-events:none}.control-overlay{position:absolute;inset:0;z-index:30;opacity:0;pointer-events:none;transition:opacity .22s ease;background:linear-gradient(180deg,rgba(0,0,0,.12),transparent 45%,rgba(0,0,0,.52))}.controls-on .control-overlay{opacity:1;pointer-events:auto}.back-btn{position:absolute;left:18px;top:2.2dvh;width:44px;height:44px;border:1px solid rgba(255,255,255,.08);border-radius:50%;background:rgba(255,255,255,.06);color:#fff;font-size:24px;backdrop-filter:blur(14px)}.bpm-control{position:absolute;left:50%;bottom:calc(max(18px,env(safe-area-inset-bottom)) + 118px);transform:translateX(-50%);display:flex;align-items:center;gap:18px;color:#fff}.bpm-control button{width:38px;height:38px;border-radius:50%;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.07);color:#fff;font-size:24px;backdrop-filter:blur(14px)}.bpm-control span{display:grid;place-items:center;min-width:78px}.bpm-control b{font-size:28px;line-height:1}.bpm-control small{font-size:10px;letter-spacing:.18em;color:rgba(255,255,255,.58)}.control-row{position:absolute;left:24px;right:24px;bottom:max(18px,env(safe-area-inset-bottom));display:grid;grid-template-columns:1fr 1.2fr 1fr;align-items:end;gap:18px}.control-row button{border:0;background:transparent;color:#fff;font-weight:700;padding:0;display:grid;place-items:center;gap:8px;font-size:38px;text-shadow:0 0 22px rgba(255,255,255,.2)}.control-row button span{font-size:13px;font-weight:500;color:rgba(255,255,255,.76);text-shadow:none}.control-row .main-btn{width:86px;height:86px;justify-self:center;border:2px solid rgba(255,255,255,.86);border-radius:50%;font-size:42px;background:rgba(255,255,255,.025);backdrop-filter:blur(12px)}.control-row .main-btn span{position:absolute;top:94px}.countdown{position:absolute;inset:0;display:grid;place-items:center;background:rgba(0,0,0,.5);backdrop-filter:blur(10px);z-index:40}.countdown b{font-size:110px;color:#f5c76b;text-shadow:0 0 50px rgba(245,199,107,.8)}@media(max-height:760px){.scale-stage{inset:8dvh 0 10dvh 0}.exercise-body{top:8dvh;bottom:-4dvh}.wave-glow{top:32dvh}.feedback-text{top:50%}.bpm-control{bottom:calc(max(12px,env(safe-area-inset-bottom)) + 102px)}.control-row .main-btn{width:76px;height:76px}.control-row .main-btn span{top:84px}.execution-label{bottom:calc(max(12px,env(safe-area-inset-bottom)) + 36px);font-size:clamp(25px,6.6vw,50px)}}`;
