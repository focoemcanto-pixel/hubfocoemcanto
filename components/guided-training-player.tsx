...
function floatMidiFromFrequency(frequency: number) {
...
function buildBeatNotes(exercise: TrainingExercise): BeatNote[] {
  const originalBeatSeconds = 60 / exercise.bpm;
  return exercise.notes.map((note) => {
    const startBeat = note.start / originalBeatSeconds;
    const durationBeats = note.duration / originalBeatSeconds;
    return {
      ...note,
      startBeat,
      durationBeats,
      endBeat: startBeat + durationBeats,
      midi: noteNameToMidi(note.pitch),
    };
  });
}
...
export function GuidedTrainingPlayer({ exercise }: { exercise: TrainingExercise; compact?: boolean }) {
  ...
  useEffect(() => {
    if (!playing) return;
    stopAudio();
    scheduleAudio(currentBeatRef.current);
  }, [bpm, metro]);
  ...
  function scheduleAudio(fromBeat: number) {
    ...
    if (metro) {
      for (let beatIndex = Math.ceil(fromBeat); beatIndex <= totalBeats; beatIndex += 1) {
        playClick(context, now + Math.max(0, beatIndex - fromBeat) * secondsPerBeat, beatIndex % 4 === 0);
      }
    }
  ...
  function adjustBpm(delta: number) {
    setBpm((old) => clamp(old + delta, 48, 140));
    setControls(true);
  }
  ...
  function listen(analyser: AnalyserNode, context: AudioContext) {
    ...
    const loopPitch = () => {
      ...
      if (!frequency) {
        ...
        smoothMidiRef.current = smoothMidiRef.current == null ? rawMidi : smoothMidiRef.current * 0.12 + rawMidi * 0.88;
  ...
  return (
    <section className={`exercise-experience ${controls ? 'controls-on' : ''}`} style={cssVars} onPointerDown={showControls}>
      ...
      <div className="control-overlay" onPointerDown={(event) => event.stopPropagation()}>
        ...
        <div className="bpm-control" aria-label="Controle de BPM">
          <button type="button" onClick={() => adjustBpm(-2)}>−</button>
          <span><b>{bpm}</b><small>BPM</small></span>
          <button type="button" onClick={() => adjustBpm(2)}>+</button>
        </div>
        <div className="control-row">
          <button type="button" onClick={() => { setCurrentBeat(0); stopAudio(); if (playing) scheduleAudio(0); }}>↺<span>Reiniciar</span></button>
