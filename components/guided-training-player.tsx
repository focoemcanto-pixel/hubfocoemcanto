...
export function GuidedTrainingPlayer({ exercise, autoStart = false }: { exercise: TrainingExercise; compact?: boolean; autoStart?: boolean }) {
  ...
  function listen(analyser: AnalyserNode, context: AudioContext) {
    ...
    const loopPitch = () => {
      ...
      if (!frequency) { silenceRef.current += 1; if (silenceRef.current > 5) { moveBrush(null); if (now - lastUi > 80) { lastUi = now; setTuner((old) => ({ ...old, midi: null, cents: null, feedback: micReady ? 'Cante próximo ao microfone' : 'Aguardando voz' })); setVoiceTrail((old) => old.slice(-3)); } } }
      else { silenceRef.current = 0; const liveMidi = floatMidiFromFrequency(frequency); const target = targetMidiRef.current; const cents = target == null ? null : (liveMidi - target) * 100; const y = yFromMidi(liveMidi); moveBrush(y); trailTickRef.current += 1; if (now - lastUi > 38) { lastUi = now; setVoiceTrail((old) => [...old.slice(-5), { id: Date.now(), y, wobble: (trailTickRef.current % 7) - 3 }]); setTuner({ midi: liveMidi, cents, feedback: cents == null ? 'Aguardando nota' : Math.abs(cents) <= 28 ? 'Perfeito' : cents < 0 ? 'Suba um pouco' : 'Desça um pouco' }); } }
