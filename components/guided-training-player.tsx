  ...
  function listen(analyser: AnalyserNode, context: AudioContext) {
    ...
    const loopPitch = () => {
      ...
      if (!frequency) {
        ...
        const y = yFromMidi(stableMidi);
        trailTickRef.current += 1;
        if (trailTickRef.current % 2 === 0) {
          setVoiceTrail((old) => [...old.slice(-15), { id: Date.now(), y, wobble: (trailTickRef.current % 7) - 3 }]);
        }
        setTuner({
          midi: stableMidi,
          cents,
    ...
    <section className={`exercise-experience ${controls ? 'controls-on' : ''}`} style={cssVars} onPointerDown={showControls}>
      ...
      <div className="control-overlay" onPointerDown={(event) => event.stopPropagation()}>
        ...
        <div className="control-row">
          <button type="button" onClick={() => { setCurrentBeat(0); stopAudio(); setVoiceTrail([]); if (playing) scheduleAudio(0); }}>↺<span>Reiniciar</span></button>
