...
type Vars = CSSProperties & { '--voice-y': string; '--voice-visible': string; '--progress': string; '--active-y': string };
...
export function GuidedTrainingPlayer({ exercise, autoStart = false }: { exercise: TrainingExercise; compact?: boolean; autoStart?: boolean }) {
  ...
  const activeY = yFromMidi(activeMidi);
  const cssVars = { '--voice-y': `${voiceY}%`, '--voice-visible': tuner.midi == null ? '0' : '1', '--progress': String(progress), '--active-y': `${activeY}%` } as Vars;
